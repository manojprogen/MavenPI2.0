...
<%@page contentType="text/html" pageEncoding="UTF-8" import ="java.util.ArrayList,java.util.HashMap,prg.db.Session,prg.db.Container ,com.progen.reportview.bd.PbReportViewerBD,com.progen.reportview.db.PbReportViewerDAO,com.progen.report.PbReportCollection,com.progen.report.PbReportRequestParameter" %>
    ...
    <%
            try {
                ...
                if (session.getAttribute("rowViewIdList") != null) {
                   ...
                   // System.out.println("rowViewByStr::"+rowViewByStr);
                } else {
                ...
                for (int i = 0; i < allViewIds.size(); i++) {
                                        ...
                                        <li class="closed">
                                            ...
                                            <td title="<bean:message bundle="Tooltips" key="changeViewBy.rowViewBy"/>">
                                                <img alt=""  src='<%=imgpathRow%>' onclick="createViewBY('<%=allViewIds.get(i)%>','<%=allViewNames.get(i)%>','rowViewULAO')"/>
                                           </td>
                            ...
                            <tr style="height:50%">
                                <td>
                                    ...
                                    <div style="height:125px;overflow-y:auto" id="RowViewBy">
                                        <ul id="rowViewULAO" class="sortable">
                                            <%//=rowViewByStr%>
                                        </ul>
                                    </div>
                                </td>
...
//                        persist: "cookie"
                    ...
                    $("#RowViewBy").droppable({
                       ...
                       var parentid=  $("#"+ui.draggable.attr('id')).parent().attr('id');//added by sruthi
                        if(parentid=="rowViewULAO")//added by sruthi
                            {
                            }
                        else
                            {
                          if(ui.draggable.attr('id').substring(0,6)=="ViewBy")
                           ...
                           createViewBY(newid,name,"rowViewULAO");
                                }
                                else
                                    {

                                        createViewBY(ui.draggable.attr('id'),ui.draggable.html(),"rowViewULAO");
                                    }
                            }}
                    });
                    $("#ColViewBy").droppable({
...
//                       alert("fromDesigner")
                       $("#ChangeViewbyButton").val("Next")
                        ...
                        //<//%System.out.println("hellojiji");rowViewByStr="";colViewByStr=""; %>
                    createViewBY('<%=allViewIds.get(0)%>','<%=allViewNames.get(0)%>','rowViewULAO')
//                     var allviewByArr = <%//=PbReportCollection.toJavascriptArray(allViewIds)%>;
//                        var allViewByNamesArr = <%//=PbReportCollection.toJavascriptArray(allViewNames)%>
//                        alert("allviewByArr::createViewBY:::"+allviewByArr);
//                         alert("allViewByNamesArr::createViewBY::"+allViewByNamesArr);
//                        createViewBY(allviewByArr,allViewByNamesArr,'rowViewUL');
//                            alert("#####:#####");
                                             // saveViewBy()
//                            alert("#####:#####");
                                              saveViewBy()
...
//               alert("saveViewBy")
               var From="";
                ...
                From=parent.document.getElementById("Designer").value;
                    ...
                    var count=0;
                    var viewByArrayAO=new Array();
                    var rowViewByArrayAO=new Array();
                    var colViewByArray=new Array();

                     var ul = document.getElementById("rowViewULAO");
//                     alert("ul::"+ul)
...
//                             alert("colIds[i].id.replace()"+colIds[i].id.replace("ViewBy",""))
                            rowViewByArrayAO.push(colIds[i].id.replace("ViewBy",""));
                            viewByArrayAO.push(colIds[i].id.replace("ViewBy",""));
                        }

                    }
                }
                         for(var i=0;i<rowViewByArrayAO.length;i++){
                             for(var j=i+1;j<rowViewByArrayAO.length;j++){
                                 if(rowViewByArrayAO[i]==rowViewByArrayAO[j])
                                     count =count+1;
                             }
                         }
                    ...
                    if(colIds!=null && colIds.length!=0){
                        for(var i=0;i<colIds.length;i++){
                         ...
                         for(var i=0;i<colViewByArray.length;i++){
                            for(var j=0;j<rowViewByArrayAO.length;j++){
                                 if(colViewByArray[i]==rowViewByArrayAO[j])
                                     count =count+1;
                             }
                         }
//                           alert("Report Created Successfully");
                    if(colViewByArray.length == 0 && rowViewByArrayAO.length == 0){
                        alert("Please Select Atleast one Row Viewby")
                    }else if(count>0){
                        alert("Please Select different Row Viewby and Col Viewby")
                    }
                    else if(rowViewByArrayAO.length>0 ){
                         if(From!="fromDesigner"){
...
//                        });
                            ...
                            if(From=="fromDesigner"){
                               ...
                               $.post(ctxpath+"/reportTemplateAction.do?templateParam=designerViewbys&ViewByArray="+viewByArrayAO+"&RowViewByArray="+rowViewByArrayAO+"&ColViewByArray="+colViewByArray+"&ChangeView=ChangeViewBy&reportId="+reportId+"&ctxpath="+ctxpath+"&rowViewNamesArr="+rowViewNamesArr+"&colViewNamesArr="+colViewNamesArr, $("#ViewByForm").serialize(),
                             function(data){//
                               parent.$("#editViewByDiv").dialog('close');
...
//                    alert("rowViewByArray::@@@"+rowViewByArray)
                    parent.document.getElementById("REPIds").value = rowViewByArrayAO;
...
//                            parent.$("#measuresDialog").dialog('open');
                        ...
                        if(rowViewByArrayAO=="" || prevREPIds==undefined ){
                         ...
                         $.post(ctxpath+"/reportViewer.do?reportBy=changeViewBy&ViewByArray="+viewByArrayAO+"&RowViewByArray="+rowViewByArrayAO+"&ColViewByArray="+colViewByArray+"&ChangeView=ChangeViewBy&reportId="+reportId+"&ctxpath="+ctxpath+"&rowViewNamesArr="+rowViewNamesArr+"&colViewNamesArr="+colViewNamesArr, $("#ViewByForm").serialize(),
                             function(data){
                                submitChangeViewBy();
                        });
                    }
...
//                 alert("createViewBY::"+elmntId+"::"+elementName)
...
//                        }
                    }else if(tarLoc == "rowViewULAO"){
//                      for(var y in elmntId){
//                            x = ViewByArray.toString();
//                           // alert("X : "+x);
//                            if(x.indexOf(elmntId[y])===-1){
//                            //alert("X : "+x);
//                            ViewByArray.push(elmntId[y]);
//                            RowViewByArray.push(elmntId[y]);
//                            rowViewNamesArr.push(elementName[y]);
//                            var childLI=document.createElement("li");
//                            childLI.id='ViewBy'+elmntId[y];
//                            childLI.style.width='auto';
//                            childLI.style.height='18px';//modified by Dinanath
//                            childLI.style.color='white';
//                            var table=document.createElement("table");
//                            table.id="viewTab"+elmntId[y];
//                            var row=table.insertRow(0);
//                            var cell1=row.insertCell(0);
//                            cell1.style.cursor = "pointer";
//                            cell1.onclick =function(){javascript:deleteColumn('ViewBy'+elmntId[y],'RowViewBy',elementName[y])};
//                            var img=document.createElement("img");
//                            img.src = "<%=delimgpath%>"
//                            cell1.appendChild(img);
//                            var cell2=row.insertCell(1);
//                            cell2.style.color='black';
//                            cell2.innerHTML=elementName[y];
//                            childLI.appendChild(table);
//                            parentUL.appendChild(childLI);
//                        }
//                        }
