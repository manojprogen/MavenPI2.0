...
<%!
    public void getTabData(prg.db.PbReturnObject myret,
            java.util.ArrayList keyFig,
            java.util.ArrayList disCols, JspWriter out) // add from and to
    {
        String colName = null;
        String tmpStr = null;
        String temp = null;
            ...
            for (int j = 0; j < disCols.size(); j++) {
                ...
                if (keyFig.contains(colName)) {
                    if ((oldVals.get(colName)).equals(myret.getFieldValue(i, colName))) {
                        ...
                        ////
                        if (String.valueOf(keyFig.get(0)).equalsIgnoreCase(colName)) {
                            tmpStr = finalBuffer.toString();

                            for(int k=0;k<keyFig.size();k++){
