...
var tableframeObj='iframe1';

function goUp(fromRow,tabId){
    if(fromRow > 1)    {
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&source=L&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
...
function goDown(toRow,rowCount,tabId){
    if(toRow < rowCount)    {
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&source=R&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
    }
}
function goFirst(fromRow,tabId){
    if(fromRow > 1)    {
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&source=S&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
    }
}
function goLast(toRow,rowCount,tabId){
    if(toRow < rowCount)    {
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&source=E&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
    }
}
function doSetFirstRow(obj,evt,maxRow,currStart,tabId){
    if(evt.keyCode == 13 && obj.value != "" && obj.value > 0 && obj.value <= maxRow && obj.value != currStart)    {
        var sVal = obj.value;
        sVal = sVal - 1;
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&rowSearch="+sVal+"&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
    }
}
function setRecordSize(sVal,tabId){
    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&slidePages="+sVal+"&tabId="+tabId;
       var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
}
...
function goRefresh(tabId){
    ...
    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&source=S&refresh=Y&tabId="+tabId;
    var dSrc = document.getElementById(tableframeObj);
...
function doSrchOperation(obj,symbol,colName,evt,tabId,secObj,ctxPath){
//    alert("hi");
//    alert("obj="+obj+"symbol="+symbol+"colName="+colName+"evt="+evt+"tabId="+tabId+"secObj="+"ctxPath="+ctxPath);
    if(evt.keyCode == 13 && obj.value != "" && secObj.value != "")    {
        var srchValue = "";
        var srchCondition = "";
...
//            alert("colName="+colName+"srchCondition="+srchCondition+"srchValue="+srchValue+"tabId="+tabId+"ctxPath="+ctxPath);
    $.ajax({
        ...
        success:function(data)
        {
            ...
            {
                var source ="TableDisplay/pbDisplay.jsp?tabId="+tabId;
                var dSrc = document.getElementById(tableframeObj);
                dSrc.src = source;
            }
        }
    });
//    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&source=S&search="+srchCondition+"&srchValue="+srchValue+"&srchCol="+colName+"&tabId="+tabId;
//    var dSrc = document.getElementById(tableframeObj);
//    dSrc.src = source;
}
...
function doSwap(moveType,colName,tabId){
    ...
    if(moveFromCol != "" && moveToCol != "")    {
        ...
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&swapFrom="+frc+"&swapTo="+toc+"&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
    }
}
...
function doSort(colName,dataType,tabId,sortType){
    ...
    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&source=S&sort="+sortDescend+"&sortColumn="+colName+"&sortDataType="+dataType+"&tabId="+tabId;

    var dSrc = document.getElementById(tableframeObj);

//    $("#"+tableframeObj).attr("src",source);
    dSrc.src =source;
    parent.isGraphExists();
}
...
function showColumnList(modifyColumnsObj,prescolsListObj,prevShownCols,tabId){
    ...
    if(modifyColumnsObj.style.display=='none'){
        ...
        if(presShownCols==prevShownCols){
            ...
            var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&presShownCols="+presShownCols+"&tabId="+tabId;
            var dSrc = document.getElementById(tableframeObj);
            dSrc.src = source;
        }
    }
...
function showTable(tableObj,tabId){
    ...
    if(tableObj.style.display=='' || tableObj.style.display=='block'){
        ...
        //var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&hideTable="+hideTable+"&tabId="+tabId+"&fh1=80&fh2=620";
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&hideTable="+hideTable+"&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        ...
        dSrc = document.getElementById(tableframeObj);
...
function showColumnTotal(dispColTotalObj,tabId,nettotreqobj,grdtotreqobj,avgtotreqobj,overAllMaxObj,overAllMinObj,catMaxObj,catMinObj){
    ...
    if(dispColTotalObj.style.display=='none'){
        ...
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&nettotreq="+nettotreq+"&grdtotreq="+grdtotreq+"&avgtotreq="+avgtotreq+"&overAllMax="+overAllMax+"&overAllMin="+overAllMin+"&catMax="+catMax+"&catMin="+catMin+"&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
    }
}
...
function setFrameHeight(FrameHgt1,FrameHgt2){
    var iframeObj1=document.getElementById(tableframeObj);
...
function transposeTable(tranposeTable,tabId){
    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&tranposeTable="+tranposeTable+"&tabId="+tabId;
    var dSrc = document.getElementById(tableframeObj);
    dSrc.src = source;
}
function formatNumber(colName,disColName,nbrSymbol,REPORTID,ctxPath){
    ...
    $.ajax({
        ...
        success:function(data){
            var source =ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&tabId="+REPORTID;
            var dSrc = document.getElementById(tableframeObj);
            dSrc.src = source;
        }
    });
...
function ifrmesizedynamic(){
    var ifrmesizes = document.getElementById(tableframeObj);
//    alert(screen.width)
...
//    alert(screen.width-270 +'px')
        ...
        if(screen.width > 1400){
            ifrmesizes.style.width='395%';
            ifrmesizes.style.position='relative';
            ifrmesizes.style.overflow='auto';
        }
        else if(screen.width > 1280 && screen.width <=1400 ){
            ifrmesizes.style.width='370%';
            ifrmesizes.style.position='relative';
            ifrmesizes.style.overflow='auto';
        }
        else if(screen.width == 1280 ){
...
function addRuntimeColumn(measType,coltype,coltypeDisp,colName,disColName,REPORTID,ctxPath){
 ...
 if((coltypeDisp.trim()=="(Running Total)")||(coltypeDisp.trim()=="(Rank)")){
    if(!isColumnPresent){
        $.ajax({
...
function goColumnUp(fromColumn,tabId){
    if(fromColumn > 1)    {
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&columnsource=LC&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
...
function goColumnDown(toColumn,ColumnCount,tabId){
    if(toColumn < ColumnCount)    {
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&columnsource=RC&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
    }
}
function goColumnFirst(fromColum,tabId){
    if(fromColum > 1)    {
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&columnsource=SC&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
    }
}
function goColumnLast(toColumn,ColumnCount,tabId){
    if(toColumn < ColumnCount)    {
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&columnsource=EC&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
    }
}
function doSetFirstColumn(obj,evt,maxColumn,currStart,tabId){
    if(evt.keyCode == 13 && obj.value != "" && obj.value > 0 && obj.value <= maxColumn && obj.value != currStart)    {
        var sVal = obj.value;
        sVal = sVal - 1;
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&columnSearch="+sVal+"&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
    }
}
function doSetColumnSize(obj,evt,maxColumn,currSize,tabId){
    if(evt.keyCode == 13 && obj.value != "" && obj.value > 0 && obj.value <= maxColumn && obj.value != currSize)    {
        var sVal = obj.value;
        var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&columnslidePages="+sVal+"&tabId="+tabId;
        var dSrc = document.getElementById(tableframeObj);
        dSrc.src = source;
    }
}
...
function statistics(columnName,disColumnName,tabId,type,ctxPath){
    ...
    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&sourceValue="+type+"&source=S&columnName="+columnName+"&disColumnName="+disColumnName+"&tabId="+tabId;
    var dSrc = document.getElementById(tableframeObj);
    dSrc.src = source;
...
//    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&sourceValue="+type+"&source=S&columnName="+columnName+"&correlationColName="+correlationColID+"&disColumnName="+disColumnName+"&tabId="+tabId;
//    var dSrc = document.getElementById(tableframeObj);
    $.ajax({
...
function toprows(columnName,disColumnName,REPORTID,noofRows,ctxPath){
    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&sourceValue=toprows&source=S&columnName="+columnName+"&topbottomCount="+noofRows+"&disColumnName="+disColumnName+"&tabId="+REPORTID;
    var dSrc = document.getElementById(tableframeObj);
    dSrc.src = source;
    parent.isGraphExists();
...
function rounding(columnName,disColumnName,REPORTID,precision){
    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&sourceValue=rounding&source=S&columnName="+columnName+"&precision="+precision+"&disColumnName="+disColumnName+"&tabId="+REPORTID;
    var dSrc = document.getElementById(tableframeObj);
    dSrc.src = source;
}
...
function bottomrows(columnName,disColumnName,REPORTID,noofRows,ctxPath){
    ...
    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&sourceValue=bottomrows&source=S&columnName="+columnName+"&topbottomCount="+noofRows+"&disColumnName="+disColumnName+"&tabId="+REPORTID;
    var dSrc = document.getElementById(tableframeObj);
    dSrc.src = source;
    parent.isGraphExists();
...
function topRowsPercentWise(columnName,disColumnName,REPORTID,noofRows,ctxPath){
    ...
    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&sourceValue=toprowsPercent&source=S&columnName="+columnName+"&topbottomCount="+noofRows+"&disColumnName="+disColumnName+"&tabId="+REPORTID;
    var dSrc = document.getElementById(tableframeObj);
    dSrc.src = source;
...
function bottomRowsPercentWise(columnName,disColumnName,REPORTID,noofRows,ctxPath){
    ...
    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&sourceValue=bottomrowsPercent&source=S&columnName="+columnName+"&topbottomCount="+noofRows+"&disColumnName="+disColumnName+"&tabId="+REPORTID;
    var dSrc = document.getElementById(tableframeObj);
    dSrc.src = source;
...
function saveReportKPI(ctxPath,PbReportId)
{
    ...
    $.post(ctxPath+'/reportViewer.do?reportBy=saveCreateKPIMeasure&kpiElementId='+kpiElementId+'&measName='+measName+'&kpiName='+kpiName+'&aggType='+aggType+'&PbReportId='+PbReportId+'&timeDet='+timeDet+'&otherTimeDet='+otherTimeDet,
function(data){
    alert("Kpi Created")
...
//                }
          ...
          function changeMeasureVal(cxtPath,factName,id){
             // alert("changeMeasureVal")
              ...
              if(measureCond=="in" || measureCond=="not in" || measureCond=="between"){
            ...
            function saveFactDetails(cxtPath,reportId,measEleId,paramFilterName,folderId,subfolderId){
                ...
                $.post(cxtPath+'/reportViewer.do?reportBy=getParamFilterMbrs&paramFilterName='+paramFilterName+'&folderId='+folderId+'&reportId='+reportId+'&measEleId='+measEleId+'&subfolderId='+subfolderId+'&Type=facts',$("#paramFilterMeasureValFrm").serialize(),
                    function(data){
...
//                else
          ...
          else if(url.indexOf(presentViewById)!=-1)
         {
           ...
           var path=ctxPath+'/reportViewer.do?reportBy=viewReport&REPORTID='+reportId+url;
             submiturls(path);
          }
     }
...
//    $.ajax({
//       url:path+'/reportViewer.do?reportBy=chkforUserPrevilages',
//       success:function(data){
//            var jsonVar=eval('('+data+')')
//            var userType=jsonVar.userType;
//            var isPowerAnalyserEnableforUser=jsonVar.isPAEnableforUser;
...
//        alert("checkCmpMsrids"+checkCmpMsrids.toString())
                ...
                $.post(cxtPath+'/reportViewer.do?reportBy=saveFactFilterDetails&factFilterName='+paramFilterName+'&folderId='+folderId+'&reportId='+reportId+'&measEleId='+measEleId+'&subfolderId='+subfolderId+'&checkCmpMsrids='+checkCmpMsrids,$("#paramFilterMeasureValFrm").serialize(),
                    function(data){
                        parent.$("#paramFilterMeasureValues").dialog('close');
...
//                   filterId.push(a)
  ...
  function reportDrillAssignment(ctxtPath,reportId){
    ...
    $.post(ctxtPath+'/reportViewer.do?reportBy=reportDrillAssignment&reportId='+reportId,
                ...
                if(data!='null'){
                    ...
                    htmlVar+="<td id='singleReportTd"+MsrIds[i]+"'><select id='MsrReport"+MsrIds[i]+"' style='width:150px;' name='MsrReport'>";
                    if(assignRepIds[i]=='0')
                    ...
                    htmlVar+="<option  value='0'>NOT_SELECTED</option>";
                       ...
                       htmlVar+="</select></td>";
                       htmlVar+="<td id='multiReportTd"+MsrIds[i]+"' style='display:none;'><input type='text' value='' onclick=\"getmultiReportIds('"+ctxtPath+"','"+reportId+"',this.id)\" style='background-color:white;width:150px;' id='multireport"+MsrIds[i]+"' name='multireportNames'></td>";
                       htmlVar+="</tr>";
                       ...
                       htmlVar+="<tr><td>&nbsp;</td></tr><tr><td colspan='3'  align='center'><input type='button' value='submit' class='navtitle-hover'  style='width:auto' onclick=\"saveDrillAssignReports('"+reportId+"','"+ctxtPath+"')\"></td></tr>";
                       htmlVar+="</table>";
...
function twoRowViewGrouping(colName,dataType,tabId,ctxPath){
    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&source=Grouping&groupColumn="+colName+"&sortDataType="+dataType+"&tabId="+tabId;
    var dSrc = document.getElementById(tableframeObj);
    dSrc.src = source;
    parent.isGraphExists();
}
...
//                         alert("showSlected"+showSlected)
                 ...
                 html+="<tr>";
                 viewArr=viewbyIdName[i].split("&");
                 html+="<td><input type='text' name='initviewByName' id='initviewByName"+viewArr[1]+"' style='width:100px;' value='"+viewArr[0]+"' readonly></td>"
                 html+="<td><input type='hidden' name='initviewByID' id='initviewByID"+viewArr[1]+"' style='width:100px;' value='"+viewArr[1]+"'></td>"
                 html+="<td><input type='checkbox' name='initcheckReport' id='initcheckReport"+viewArr[1]+"' "+checked+" onclick=\"ViewByrltdReportsForInitialize('"+viewArr[1]+"')\"></td>";
                 checkedList.push(viewArr[1]);
            ...
            $.post(ctxtPath+'/reportViewer.do?reportBy=getReportsforMsrDrill&reportId='+reportId,
             function(data){
               var jsonVar = eval('('+data+')');
                ...
                var htmlVar="<table><tr><td><input type=\"button\" name=\"Done\" class=\"navtitle-hover\" value=\"Done\"  onclick=\"savemultiReportIds('"+ctxtPath+"','"+id+"')\"></td></tr></table>";
  ...
  function multireportMsrDrill(reportId,ctxPath,path,msrId){
      ...
      $.post(ctxPath+'/reportViewer.do?reportBy=getMultiSelectReportNames&path='+encodeURIComponent(path)+"&msrId="+msrid+"&reportId="+reportId,
             function(data){
                    ...
                    var reportIds=jsonVar.reportIds;
                    var reportNames=jsonVar.reportNames;
                    var htmlStr="<table align='center'width='100%'><tr><td>&nbsp;&nbsp;</td></tr>";
                    for(var i=0;i<reportIds.length;i++){
                        var path1=path+"&REPORTID="+reportIds[i];
                       //alert(path1);
                        htmlStr+="<tr><td align='center'><a href=\"javaScript:applymutliReportMsrDrill('"+path1+"')\">"+reportNames[i]+"</a></td></tr>";
                    }
                    htmlStr+="</table>";
                     parent.$("#reportDrillFrm").html(htmlStr);
                     parent.$("#reportDrillDiv").dialog('open');
             });
...
//     alert(colName+''+sortType+''+dataType+''+tabId)
     sortDescend = sortType;
    var source = ctxPath+"/TableDisplay/pbDisplay.jsp?isreportdrillPopUp=true&source=S&sort="+sortDescend+"&sortColumn="+colName+"&sortDataType="+dataType+"&tabId="+tabId+"&sortType=SUBTOTALSORT";
    var dSrc = document.getElementById(tableframeObj);
    dSrc.src = source;
    parent.isGraphExists();
...
//     alert('reportId--'+reportId+' ctxPath--'+ctxPath+' reportUrl'+reportUrl);
//     alert('winWidth'+winWidth+'winHight:'+winHight+'level:'+level)
...
//      alert('winWidth'+winWidth+'winHight:'+winHight+'level:'+level)
//      if(parseInt(level) < 3){
          $("#reportDrillPopUpDiv"+level).dialog({
                        autoOpen: false,
                        height: (winHight),
                        width: (winWidth),
                        position: 'justify',
...
//                        title:dashletName,
             ...
             $("#reportDrillPopUpDiv"+level).dialog('open');
             var loader="<center><img id='imgId"+level+"' src='"+ctxPath+"/images/ajax1.gif' align='top'  width='100px' height='100px  style='position:absolute' /></center>";
             $("#reportDrillPopUpDiv"+level).html(loader);
...
//                alert(dialogeTitle)
                $("#reportDrillPopUpDiv"+level).dialog('option','title',dialogeTitle);
                 var source=ctxPath+'/TableDisplay/pbReportDrillPopUp.jsp?reportId='+presReportId+'&width='+(winWidth-10)+'&height='+(winHight-40)+'&level='+(level+1)+'&isreportdrillPopUp=true';
                 var innerHtml=" <iframe id='reportDrillPopUpIframe"+level+"' width='100%' height='100%' src='"+source+"' scrolling='no' ></iframe>"
                 $("#reportDrillPopUpDiv"+level).html(innerHtml);
//                   frameObj.src=source;
//                 alert(frameObj.src);
             });
//      }else{
//          alert('Sorry...You Cannot Drill Above Three Levels..!!');
//      }

 }
 function setframeObj(objName){
     tableframeObj=objName;
 }