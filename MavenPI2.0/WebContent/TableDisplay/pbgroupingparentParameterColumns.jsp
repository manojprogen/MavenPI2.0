...
<!--        <script type="text/javascript" src="<%=request.getContextPath()%>/javascript/reportDesign.js"></script>-->
        ...
        <script>
            ...
            function saveGrpnew(id1,id2){
                ...
                valList = valList.toString().replace("&gt;",">").replace("&lt;","<")
        ...
        try {
            ...
            if (connectionId != null) {
                ...
                if(colType.equalsIgnoreCase("CALCULATED")){
                    elementId = request.getParameter("eleId");
                    PbReportViewerDAO pb = new PbReportViewerDAO();
                    filePath = pb.getFilePath(session);
                    PbReportViewerDAO bd = new PbReportViewerDAO();
                    formulaVal=bd.readFormulaFromFile(elementId,filePath);
                }
                ...
                if (colList.equalsIgnoreCase("")) {
                    if(colType.equalsIgnoreCase("CALCULATED")){
                        parentGrpValQuery = "select distinct " + formulaVal + "  from " + tableName+" order by "+formulaVal;
                    } else {
                    ...
                    if(colType.equalsIgnoreCase("CALCULATED")){
                         parentGrpValQuery = "select distinct " + formulaVal + "  from " + tableName+ " where " + formulaVal + " not in(" + colListNew + ") order by "+formulaVal;
                     //   parentGrpValQuery = "select distinct " + formulaVal + "  from " + tableName + " where " + formulaVal + " not in(" + colListNew + ") order by "+colName;
                    } else {
                        parentGrpValQuery = "select distinct " + colName + "  from " + tableName + " where " + colName + " not in(" + colListNew + ") order by "+colName;
                    }
                }

               // //////////////////////////////////////////////////////////.println("parentGrpValQuery===" + parentGrpValQuery);
                con = new BusinessGroupDAO().getConnectionIdConnection(connectionId);
                st = con.createStatement();
                rs = st.executeQuery(parentGrpValQuery);
    ...
    <body>
        <%  }%>
<br><br>
        <table style="width:85%;height:200px" border="solid black 1px">
                        ...
                        <div style="height:190px;overflow-y:auto">
                            <ul id="myList3" class="filetree treeview-famfamfam">
                                ...
                                if(colType.equalsIgnoreCase("DATE")){
                                   ...
                                   // //////////////////////////////////////////////////////////.println("dateval--"+dateval1);
                                    if(!dateval1.equalsIgnoreCase("")){
                                %>
                                <li><span class="myDragTabs" class="ui-state-default" id="<%=dateval1%>"><%=dateval1%></span></li>
                                <%}
                                    }else{
                                     if(!memdetPbro.getFieldValueString(i, 0) .equalsIgnoreCase("")){
                                    %>
                                <li><span class="myDragTabs" class="ui-state-default" id="<%=memdetPbro.getFieldValueString(i, 0)%>"><%=memdetPbro.getFieldValueString(i, 0)%></span></li>
