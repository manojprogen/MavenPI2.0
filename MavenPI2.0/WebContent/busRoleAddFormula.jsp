...
<%@page contentType="text/html" pageEncoding="UTF-8" import="prg.db.PbReturnObject,java.sql.*,prg.db.PbDb" %>
...
<head>
    ...
    <script src="<%=conPath%>/javascript/treeview/jquery.cookie.js" type="text/javascript"></script>
    <script src="<%=conPath%>/javascript/treeview/jquery.treeview.js" type="text/javascript"></script>
    <script type="text/javascript" src="<%=conPath%>/javascript/treeview/demo.js"></script>
    <script type="text/javascript" src="<%=conPath%>/javascript/draggable/ui.core.js"></script>
    <script type="text/javascript" src="<%=conPath%>/javascript/draggable/ui.draggable.js"></script>
    <script type="text/javascript" src="<%=conPath%>/javascript/draggable/ui.droppable.js"></script>

    <script type="text/javascript" src="<%=conPath%>/javascript/ui.resizable.js"></script>
    <script type="text/javascript" src="<%=conPath%>/javascript/ui.accordion.js"></script>
    <script type="text/javascript" src="<%=conPath%>/javascript/ui.dialog.js"></script>
    <script type="text/javascript" src="<%=conPath%>/javascript/ui.sortable.js"></script>
    <script type="text/javascript" src="<%=conPath%>/javascript/effects.core.js"></script>
    <script type="text/javascript" src="<%=conPath%>/javascript/effects.explode.js"></script>
    <script language="JavaScript" src="<%=conPath%>/querydesigner/JS/jquery.columnfilters.js"></script>
    <link type="text/css" href="<%=conPath%>/stylesheets/demos.css" rel="stylesheet" />
    <link type="text/css" href="<%=conPath%>/stylesheets/ui.all.css" rel="stylesheet" />
        <!--addeb by bharathi reddy fro search option-->
        <script type="text/javascript" src="<%=conPath%>/javascript/quicksearch.js"></script>
    <link rel="stylesheet" href="<%=conPath%>/stylesheets/treeviewstyle/jquery.treeview.css" />
    <link rel="stylesheet" href="<%=conPath%>/stylesheets/treeviewstyle/screen.css" />

    <link type="text/css" href="<%=conPath%>/stylesheets/metadataButton.css" rel="stylesheet" />
...
<%
        ...
        String Query = "SELECT distinct t.disp_name FROM PRG_USER_SUB_FOLDER_ELEMENTS e, PRG_USER_SUB_FOLDER_TABLES t  where e.SUB_FOLDER_ID=" + folderIds + " AND " +
                 " t.sub_folder_tab_id = e.sub_folder_tab_id and e.ref_element_id= e.element_id and t.disp_name not in('Calculated Facts','Time')order by t.disp_name";
        //////////////////////////////////////////////////////////////.println.println("Query---" + Query);
        String Query1 = "SELECT e.ELEMENT_ID, t.disp_name   ,nvl(e.USER_COL_NAME,e.USER_COL_DESC), e.user_col_type,nvl(e.BUSS_COL_NAME,e.user_col_name)  FROM ";
        Query1 += " PRG_USER_SUB_FOLDER_ELEMENTS e, PRG_USER_SUB_FOLDER_TABLES t  where e.SUB_FOLDER_ID=" + folderIds + " AND ";
        ...
        String Query2 = "select  distinct connection_id,grp_id from prg_user_all_info_details  where SUB_FOLDER_ID =" + folderIds;
        //////////////////////////////////////////////////////////////.println.println("Query2---" + Query2);
        PbReturnObject pbro3 = pbdb.execSelectSQL(Query2);
        String connectionId = "";
        String grpId = "";
        if (pbro3.getRowCount() == 0) {

            Query2 = " select connection_id,grp_id from  prg_grp_master where grp_id in";
            Query2 += "(select grp_id from prg_user_folder where folder_id in";
            Query2 += "(select folder_id from prg_user_folder_detail where ";
            Query2 += "sub_folder_id in(" + folderIds + " )))";
            //////////////////////////////////////////////////////////////.println.println("Query2---" + Query2);
             pbro3 = pbdb.execSelectSQL(Query2);
            connectionId = String.valueOf(pbro3.getFieldValueInt(0, 0));
            grpId = String.valueOf(pbro3.getFieldValueInt(0, 1));
        } else {
            connectionId = String.valueOf(pbro3.getFieldValueInt(0, 0));
            grpId = String.valueOf(pbro3.getFieldValueInt(0, 1));
        }
       ...
       // String tableName = pbro2.getFieldValueString(0, 1);
//  PbReturnObject pbro3 = pbdb.execSelectSQL(Query2);
        ...
        String Query = "SELECT  t.disp_name   ,nvl(e.USER_COL_NAME,e.USER_COL_DESC), e.user_col_type,nvl(BUSS_COL_NAME,user_col_name)  FROM ";
        Query += " PRG_USER_SUB_FOLDER_ELEMENTS e, PRG_USER_SUB_FOLDER_TABLES t  where e.SUB_FOLDER_ID=" + folderIds + " AND ";
        Query += " t.sub_folder_tab_id = e.sub_folder_tab_id and e.ref_element_id= e.element_id ";
        Query += " order by nvl(e.USER_COL_NAME,e.USER_COL_DESC),ref_element_id, ref_element_type";
...
<form id="f1" name="myForm">
        ...
        <tr style="width:100%">
            <td style="width:100%">
                <table style="width:100%" border="0px solid">
                    <tr style="width:100%">
                        <td width="40%" style="height:150px" valign="top">
                        ...
                        <td width="60%">
                          ...
                          <%--  <div style="height:250px;overflow-y:scroll">
                                ...
                                <%for (int i = 0; i< pbro2.getRowCount(); i++) {%>
                                        <ul>
                                            <li class="closed"><img src='icons pinvoke/table.png'></img><span ><font size="1px" face="verdana"><%=pbro2.getFieldValueString(i)%></font></span>
                                                <ul>
                                                    <%for (int j = 0; j < pbro2.getRowCount(); j++) {

                                                    %>

                                                    <li><img src='icons pinvoke/report.png'></img><a href="javascript:void(0)" id="<%=pbro2.getFieldValueString(j, 2)%>" onclick="addValue('<%=pbro2.getFieldValueString(j, 4)%>','~<%=pbro2.getFieldValueString(j, 0)%>','Query')"><%=pbro2.getFieldValueString(j, 2)%></a></li>
    ...
    <div style="height:200px;overflow-y:auto;overflow-x:hidden">
        ...
        if (String.valueOf(pbro2.getFieldValueString(i, 1)).equalsIgnoreCase(String.valueOf(pbro1.getFieldValueString(j, 0)))) {
            ...
            if (pbro2.getFieldValueString(i, 3).equalsIgnoreCase("calculated") != true && pbro2.getFieldValueString(i, 3).equalsIgnoreCase("summarised") != true) {
                                                            %>

                                                    <li>
                                                      <%if(pbro2.getFieldValueString(i, 3).equalsIgnoreCase("NUMBER")){%>
                                                                 <label class="label" style="color:green"><b>N</b></label>
                                                                <%}else if(pbro2.getFieldValueString(i, 3).equalsIgnoreCase("VARCHAR2")){%>
                                                                 <label class="label" style="color:green"><b>T</b></label>
                                                                <%}else if(pbro2.getFieldValueString(i, 3).equalsIgnoreCase("DATE")){%>
                                                                    <label class="label" style="color:green"><b>D</b></label>
                                                                <%}%>

                                                    <a href="javascript:void(0)" style="font-family:verdana;font-size:12px;text-decoration:none;color:#369" id="<%=pbro1.getFieldValueString(j, 0)%>" onclick="addValue('<%=pbro1.getFieldValueString(j, 0)%>.<%=pbro2.getFieldValueString(i, 4)%>','~<%=pbro2.getFieldValueString(i, 0)%>','Query')"><%=pbro2.getFieldValueString(i, 2)%></a></li>
                 ...
                 <%--    <%  else {%>
                                                    ...
                                                    <li>
                                                        <%if(pbro2.getFieldValueString(i, 3).equalsIgnoreCase("CALCULATED")){%>
                                                                 <label class="label" style="color:green"><b>C</b></label>
                                                                <%}else if(pbro2.getFieldValueString(i, 3).equalsIgnoreCase("SUMMARISED")){%>
  ...
  <%      }
    ...
    <table align="center" width="100%" border="0">
        <tr align="center">
            <td align="center">
                <center>
                    <input type="button" class="navtitle-hover" style="width:auto" value="Save" onclick="saveRoleFormula('<%=connectionId%>','<%=grpId%>')">&nbsp;&nbsp;
...
<script type="text/javascript">
        ...
        function saveRoleFormula(ConnectionId,grpId)
        {
             ...
             //   success: function(data) {
                   ...
                   // {
                        if(document.getElementById("columnName").value==""){
                            ...
                            // alert('userLayerAction.do?userParam=saveRoleFormula&txt2='+txt2val+'&folderTabId='+folderTabIdval+'&iscalculate='+iscalculateval+'&tArea1='+tArea1val+'&tArea='+tareaval)

                            $.ajax({
                                url: 'userLayerAction.do?userParam=saveRoleFormula&connectionId'+ConnectionId+'&txt2='+txt2val+'&folderTabId='+folderTabIdval+'&iscalculate='+iscalculateval+'&tArea1='+tArea1val+'&tArea='+tareaval+'&columnName='+columnNameval+'&grpId='+grpId,
                                success: function(data) {
                                   // alert(data)
                                    if(data=="Correct"){
