                   ...
                   for(var l=0;l<viewBys.length;l++){
                    if (typeof drilledvalue !== 'undefined' && drilledvalue.length > 0 && drilledvalue.indexOf(d[viewBys[l]]) !== -1) {
                           ...
                           htmlstr = htmlstr + "<td nowrap id='"+d[columns[l]]+":"+d[measureArray[0]]+"' onclick='drillWithinchart(this.id,\""+chartId+"\")'  style=\"background-color:" + drillShade + ";cursor:pointer;color:black\" width=" + trWidth + " >" + d[columns[l]] + "</td>";
                               ...
                               htmlstr = htmlstr + "<td nowrap id='"+d[columns[l]]+":"+d[measureArray[0]]+"' onclick='drillWithinchart(this.id,\""+chartId+"\")'  style=\"background-color:" + color + ";cursor:pointer;\" width=" + trWidth + " ><u>" + d[columns[l]] + "</u></td>";
               }
                    }
                   for(var j=0;j<measureArray.length;j++){
                   if(j==0){
                   htmlstr = htmlstr + "<td nowrap style=\"background-color:" + color + ";cursor:pointer;\" width=" + trWidth*.20 + " >" + addCurrencyType(div, getMeasureId(measureArray[j])) + addCommas(numberFormat(d[measureArray[j]],yAxisFormat,yAxisRounding,chartId)) + "</td>";
                   }
                    }
                    var emojiWdith=divwidth/27;
            emojiWdith=emojiWdith>22?22:emojiWdith;
            for(var j=0;j<measureArray.length;j++){
               var scale = d3.scale.linear().domain([minMaxMap[measureArray[j]]["max"], minMaxMap[measureArray[j]]["min"]]).range(["90%", "1%"]);
//    dataTable +="<td>"+displayData[i][measureArray[k]]+"</td>";
            var wid = scale(d[measureArray[j]]);
            ...
            if(j==0){
            htmlstr += "<td nowrap height='15px'><canvas id='" + data[chartId][i][columnInd] + "_" + measureArray[j].replace(/[^a-zA-Z0-9]/g, '', 'gi') + "' title='" + measureArray[j] + ":" + addCurrencyType(div, getMeasureId(measureArray[j])) + addCommas(numberFormat(d[measureArray[j]],yAxisFormat,yAxisRounding,chartId)) + "' style='width:"+wid+";height:18px;margin-left:5px;margin-right:5px;background:" + getDrawColor(chartId,j) + "' onmouseove=''></canvas></td>";//width='"+parseFloat(tdwid-2)+"'  //linear-gradient(to right,"+color(k)+" 0%,  rgb(250,250,250) 85%)
            }else {
...
//  context.fillText(addCommas(numberFormat(data[chartId][z][measureArray[j]],yAxisFormat,yAxisRounding)));
//}
//}
}
...
//        vis.append("g")
//        .attr("class", ".x axis")
//        .attr("transform", "translate(250," +(h+top_margin-20 +offset)+ ")")
//        .attr("x", 5)
//        .call()
//        .selectAll('text')
//        .style('text-anchor', 'end')
...
//        vis.append("g")
//        .attr("class", ".x axis")
//        .attr("transform", "translate(0," +(h+top_margin-20 +offset)+ ")")
//            
//        .call(xAxis2)
//        .attr("x",function(d,i) {
//            return right_margin*2.5;
//        })
//        .selectAll('text')
//        .style('text-anchor', 'end')
//        .text(function(d,i) {
...
//    .attr("width",0)
    ...
    .attr("x", function(d,i) {
        if(i>=5){
          ...
          return left_margin*1.3-(x(d[measureArray[0]]))
    ...
    .text(function(d, i) {
        if(typeof displayY !=="undefined" && displayY =="Yes"){
            if (data[i][columns[0]].length < 1)
                return data[i][columns[0]];//buildXaxisFilterhorizontal
            else
    ...
    .attr('transform', function(d,i) {
        if(i>=5){
            return 'translate(165,0) rotate(0)';
        }else{
    ...
    .attr("transform", function(d,i) {
        if(i>=5){
            if(typeof chartData[div]["LabelPos"]!="undefined" && chartData[div]["LabelPos"]!="" && chartData[div]["LabelPos"]!="Top"){
                var xvalue = (left_margin*2.40);
            }else{
                var xvalue = left_margin*1.5-(x(d[measureArray[0]]));
            }  
        }else{
            if(typeof chartData[div]["LabelPos"]!="undefined" && chartData[div]["LabelPos"]!="" && chartData[div]["LabelPos"]!="Top"){
                var xvalue = (left_margin*2.6);
            }else{
                var xvalue = (x(d[measureArray[0]])+left_margin*2.65);
            }   
        }
        return "translate(" + xvalue + ", " + (y.rangeBand() / 1.7) + ")";
    })
    .attr("text-anchor", "middle")
    .attr("class", "valueLabel")
    .attr("fill", function(){
            ...
            .attr("transform", function(d) {
                    var yValue = "-5";
                     ...
                     return "translate("+divWid*.925+", " + (yValue) + ")";      
                          ...
                          if(divWid < 600){
                     return "translate("+divWid*.835+", " + (yValue) + ")";      
                     }else{
...
//alert(JSON.stringify(dataList[m][measureArray[0]]))
//var list = dataList[m][measureArray[0]];
//        var sum = 0;
//        for(var z in list){
//            
//        }
          html += "<tr style='background-color:rgb(227, 243, 222)'>";
          html += "<td style='font-family: helevetica;font-size: 14px;'>"+keys[m]+"</td>";
          html += "<td></td>";
          html += "</tr>";
          for(var k=0;k<5;k++){
          html += "<tr>";
              html += "<td style='padding-left:2%'>"+dataList[m][keys[m]][k];
              html += "</td>";
              var msrData = addCurrencyType(div, getMeasureId(measureArray[0])) + addCommas(dataList[m][measureArray[0]][k]);
              html += "<td>"+msrData;
              html += "</td>";
          html += "</tr>";
          }
...
//          "order": [[ 1, "desc" ]]  
   ...
   function buildTopAnalysis(div, data, columns, measureArray, divWidth, divHeight,KPIresult,drillReportType,viewByIconMap) {
//  alert(columns)
//alert(JSON.stringify(data))  
    var appendDiv = "";
...
htmlvar += "</div>"
htmlvar += "<div id='Top1Div_"+div+"' style='margin-left:15px;float:left;width:"+divWidth*.3+"px;height:"+divHeight+"px'></div>";
htmlvar += "<div id='Top2Div_"+div+"' style='float:left;width:"+divWidth*.3+"px;height:"+divHeight+"px'></div>";
...
parent.$("#chartData").val(JSON.stringify(chartData));
 ...
 $.ajax({
      ...
      buildTopAnalysis("Top1Div_"+div, JSON.parse(data2["data"])[div], chartData[div]["viewBys"], measureArray, divWidth*.3, divHeight,KPIresult,"drill2",viewByIconMap)
...
buildTopAnalysis("Top2Div_"+div, JSON.parse(data1["data"])[div], chartData[div]["viewBys"], measureArray, divWidth*.3, divHeight,KPIresult,"drill1",viewByIconMap)
       } })  
...
chartData[div]["viewIds"] = viewIdList1;
        ...
        buildTopAnalysis("Top1Div_"+div, dataList1, chartData[div]["viewBys"], measureArray, divWidth*.3, divHeight,KPIresult,"drill2")
      var viewList = [];
   var viewIdList = [];
   try{
   viewList.push(viewOvName[viewOvIds.indexOf(dimensions2[0])]);
   ...
   viewIdList.push(viewOvIds[viewOvIds.indexOf(dimensions2[1])]);  
     ...
     chartData[div]["viewBys"] = viewList;
chartData[div]["viewIds"] = viewIdList;
buildTopAnalysis("Top2Div_"+div, dataList2, chartData[div]["viewBys"], measureArray, divWidth*.3, divHeight,KPIresult,"drill1")
    }
})
...
if (typeof chartData[div]["trendViewMeasures"] !== "undefined" && chartData[div]["trendViewMeasures"] === "ViewBys") {
//buildGroupedLine("trendcombo_"+div, data, columns, measureArray, divWidth, subDivHgt,KPIresult)
...
//var datavar = data; 
//data = [];
//for(var datalength=0;datalength<((datavar.length<10)?datavar.length:10);datalength++){
//data.push(datavar[datalength])
//}
...
//                    return "gradient_" + div + "_" + (d[columns[0]]).replace(/[^a-zA-Z0-9]/g, '', 'gi');
        ...
        gradient.append("svg:stop")
                ...
                .attr("stop-color", function(d, i) {
                    var colorShad;
//                    if (isShadedColor) {
//                        colorShad = color(data[j][shadingMeasure]);
//                    } else if (conditionalShading) {
...
//                    }//parent.$("#colorMap").val(JSON.stringify(colorMap));
	...
	var offset1=divHgt/2-(ramswap_r+heigth/50);
        ram_arcs = ram_arcs_group.selectAll("g.ram_arc")
                ...
                .enter().append("svg:g")
                .attr("class", "ram_arc")
                .attr("transform", "translate(" + (ramswap_r+offset) + "," + (ramswap_r+heigth/50+offset1-divHgt/30) + ")")
//                .attr("margin-left",)
//                 .attr("transform", "translate(" + divHgt / 3.2 + "," + divHgt / 3.2 + ")")
                .attr("id", function(d, i) {
                    return label[i];
                })
                .attr("onclick", fun)
...
//            return d.endAngle - d.startAngle > 0.05;
//        })
//                .append("svg:text")
//                .attr("dy", ".35em")
...
//                .attr("transform", function(d) {
//                      d.outerRadius = ramswap_r * 0.8;
//                       var a = angle(d);
////                    return "translate(" + arcFinal.centroid(d) + ")rotate(" + angle(d) + ")";
//                      if (a < 90) {
//                    a = a + 180;
//                    d.innerRadius = ramswap_r * 1.6;
//                } else {
//                    d.innerRadius = ramswap_r * 0.8;
//                }
...
//                .attr("fill", function(d, i) {
//                    //                    return swap_colors(i);
////                    return "url(#gradient_" + div + "_" + (label[i]).replace(/[^a-zA-Z0-9]/g, '', 'gi') + ")";
//                    return getDrawColor(div, parseInt(i));
//                })
.attr("fill", function(d,i) {  // add for graph color pattern by mayank sh.
                var drilledvalue;
        ...
        var rectsize = parseInt(width/60);
         ...
         else
         {
            if(width<height)
            {
                ...
                rectyvalue = parseInt(height*0.735);
            }
            else
            {
                rectyvalue = parseInt(height*0.83);
                yvalue = parseInt(height*.85);
            }
         }
         if(fontsize1>15){
                  fontsize1 = 15;
...
//            .attr("x",rectlen)
//            .attr("y",rectyvalue)
            .attr("transform", "translate(" + width*.71  + "," + rectyvalue + ")")
            .attr("width", rectsize)
            .attr("height", rectsize)
            .attr("fill", function(){
			var colorfill = getcolorValueFunction(div,chartData,drillShade,data,columns,measureArray,i,color)
