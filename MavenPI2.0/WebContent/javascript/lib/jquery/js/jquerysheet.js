...
debugTxt =debugTxt + "In Sheet Start : "+new Date()+"\n";
		settings = jQuery.extend({
			...
			newColumnWidth: 150, 							//int, the width of new columns or columns that have no width assigned
			...
			colMargin: 		20, 							//int, the height and the width of all bar items, and new rows
		...
		if (jQuery.sheet.instance) {
			...
			jQuery.sheet.instance = [o.sheetInstance];
                ...
                debugTxt =debugTxt + "In Sheet End : "+new Date()+"\n";
                jQuery.sheet.instance[0].obj.debugArea().val(debugTxt);
		return o;
	}
});
		...
		var jS = {
			...
			obj: {//obj = object references
				...
				uiActive:			function() {return s.parent.find('div.' + jS.cl.uiActive);},
                                copyArea:               function()  {return jQuery('#' + jS.id.copyArea);},
                                debugArea:               function()  {return jQuery('#' + jS.id.debugArea);}
			},
			id: {//id = id's references
				...
				ui:					'jSheetUI_' + I,
                                copyArea:               'jSheetCopyArea_' + I,
                                debugArea:              'jSheetDebugArea_' + I
			},
			...
			kill: function() {
				...
				delete origParent.sheetInstance;
                        ...
                        getBarTopHTML: function(text, colNumber) {
                            var html = "";
                            var id = "barTopMenuTD"+colNumber;
                            html = html + "<table><tr><td id=\""+id+"\"width=\"100%\" style=\"background-color:transparent\">";
                            html = html + text;//"<div>" + text + "</div>";
                            html = html + "</td>";
                            //html = html + "<td width=\"10%\" style=\"background-color:transparent\" id=\""+id+"\">";
                            //html = html + "<div>";
                            //html = html + "<ul class=\"dropDownMenu\"><li style=\"width:100%\"><a href=\"#\" style=\"color:#369;background-color:#369\"></a><ul style=\"width:110px\"><li><a href=\"#\"  onClick=\"#\">Insert</a><ul><li><a href=\"#\" onClick=\"rajeshAddCol("+ colNumber + ",'before')\">Insert Before</a></li><li><a href=\"#\" onClick=\"rajeshAddCol("+ colNumber + ",'after')\">Insert After</a></li></ul></li></ul></li></ul>";
                            //html = html + "</div>";
                            //html = html + "</td>";
                            html = html + "</tr></table>";
                            return html;
			...
			controlFactory: {
				...
				addCells: function(eq, isBefore, eqO, qty, type) {
					jS.setDirty(true);
//jS.obj.debugArea().val(" in addCells add "+type+" before :"+isBefore+" where :"+eq);
					...
					switch (type) {
						...
						case "col":
							o = {
								...
								newCells: function() {
									return '<td editable="true"/>';
								},
								reLabel: function() {
									o.barParent.children().each(function(i) {
                                                                                var html = jS.getBarTopHTML(cE.columnLabelString(i + 1), i);
				...
				addColumn: function(atColumn, isBefore, atColumnQ) {
                                    alert("in addColumn : "+atColumn+" before : "+isBefore+" colQ : "+atColumnQ);
...
//debugTxt =debugTxt + "Going to remove old bartop : "+new Date()+"\n";
					jS.obj.barTop().remove();
//debugTxt =debugTxt + "Old Bartop removed : "+new Date()+"\n";
...
//debugTxt =debugTxt + "Going to get the parents : "+new Date()+"\n";
						parents = o.find('col');
//debugTxt =debugTxt + "Got the parents : "+new Date()+"\n";
						widthFn = function(obj) {
					...
					parents.each(function(i) {
						...
						var child = jQuery("<div id=\""+id+"\">" + v + "</div>")
							.width(w)
							.height(s.colMargin);

						barTop.append(child);
					});
//debugTxt =debugTxt + "Prepared the parents : "+new Date()+"\n";
/*
                                        barTop.children().each(function(i){
                                            var html = jS.getBarTopHTML(cE.columnLabelString(i + 1), i);
                                            jQuery(this).html(html);
                                        });
*/
				...
				header: function() {    /* This function is used for creating the basic structure of the spreadsheet.
                                                        *  Four Regions are created. First Title and Menu Region if any.
                                                        *  Second is the formula bar region.
                                                        *  Third is the region where the excel body content is displayed.
                                                        *  Fourth is the region where option for creating new sheet is given.
                                                        */
					...
					var firstRow = jQuery('<table id="firstRowTable" cellpadding="0" cellspacing="0" border="0" style="display:none;"><tr /></table>').prependTo(header);
					var firstRowTr = jQuery('<tr />');
                                        var buttonsTd = jQuery('<td style="width:16%" id="buttonsDiv"/>');
                                        var targetTd = jQuery('<td id="targetDiv"/>');
//                                        var buttonsDiv = jQuery('<div id="buttonsDiv"/>');
                                        var bnHtml = "<input type=\"button\"   class=\"navtitle-hover\" style=\"width:auto\" value=\"Save Data\" onclick=\"saveExcel()\">";
                                        bnHtml = bnHtml + "<input type=\"button\" class=\"navtitle-hover\" style=\"width:auto\" value=\"Export\" onclick=\"exportExcel()\">";
                                        bnHtml = bnHtml + "<input type=\"button\" class=\"navtitle-hover\" style=\"width:auto\" value=\"Import\" onclick=\"importExcel()\">";

//                                        var targetDiv = jQuery('<div id="targetDiv"/>');
                                        var targetBnHtml = "<input type=\"button\" class=\"navtitle-hover\" style=\"width:auto\" value=\"Add Target\" onclick=\"addTargetColumn("+s.reportId+")\">";
					...
					if (s.title) {
						...
						firstRowTr.append(jQuery('<td style="width: auto;text-align: center;" />').html(title));
                                            ...
                                            buttonsTd.append(bnHtml);
                                            targetTd.append(targetBnHtml);
//                                            buttonsTd.append(buttonsDiv);
//                                            targetTd.append(targetDiv);
                                            firstRowTr.append(buttonsTd);
                                            firstRowTr.append(targetTd);
                                        }
					...
					if (s.editable) {
						...
						var secondRow = jQuery('<table cellpadding="0" cellspacing="0" border="0">' +
								'<tr>' +
									...
									'</td>' +
                                                                        '<td>' +
										'<textarea id="' + jS.id.copyArea + '" style="display:none;"></textarea>' +
									'</td>' +
                                                                        '<td>' +
										'<textarea id="' + jS.id.debugArea + '" style="display:none;"></textarea>' +
									'</td>' +
								'</tr>' +
							'</table>')
					...
					firstRowTr.appendTo(firstRow);
                                        ...
                                        // Currently removing the sheet creation region.
/*					var tabParent = jQuery('<div id="' + jS.id.tabContainer + '" class="' + jS.cl.tabContainer + '">' +
				...
				sheetUI: function(o, i, fn, reloadBars) {   /* This function is used for creating the excel content and loading it.
                                                                            *  It embeds the incoming table object into the excel region and creates
                                                                            *  the bars on the top (A,B,C,D,etc.) and bars on the left (1,2,3, etc.)
                                                                            */
					if (!i) {
						jS.sheetCount = 0;
						jS.i = 0;
...
//debugTxt =debugTxt + "Starting sheetUI " + new Date() + "\n";
					var objContainer = jS.controlFactory.table().appendTo(jS.obj.ui());     //Creating the basic layout
					var pane = jS.obj.pane().html(o);
//debugTxt =debugTxt + "Created the table and added html " + new Date() + "\n";


					o = jS.tuneTableForSheetUse(o);     //Changing the stylesheet of the incoming table to jquery sheet
//debugTxt =debugTxt + "Tuned table for sheet use " + new Date() + "\n";
					jS.sheetDecorate(o);                        // setting the default row height and column width
//debugTxt =debugTxt + "Decoration done " + new Date() + "\n";
					jS.controlFactory.barTop(reloadBars, o);    // adding the bar top
//debugTxt =debugTxt + "Added bartop " + new Date() + "\n";
					jS.controlFactory.barLeft(reloadBars, o);   // adding the bar left
//debugTxt =debugTxt + "Added barleft " + new Date() + "\n";
					jS.sheetTab(true);              // To set the sheet name
					...
					jS.themeRoller.start(i);        // To set the styles of all the components in the spreadsheet
//debugTxt =debugTxt + "Event Handlers set " + new Date() + "\n";
					jS.setTdIds(o);                 // To set the id of all the td elements (cells in spreadsheet)
//debugTxt =debugTxt + "TD Ids Set " + new Date() + "\n";
					jS.evt.scrollBars();            // To set the scrolling behaviour. On horizontal scroll, bartop should also
                                                                        // scroll. On vertical scroll, barleft should scroll.

					jS.addTab();                    // To add the sheet list and option for adding a new list
//debugTxt =debugTxt + "Scrollbar and add sheet option added " + new Date() + "\n";
					if (fn) {
						fn();
					}

					jS.log('Sheet Initialized');
//debugTxt =debugTxt + "Completed SheetUI " + new Date() + "\n";
					return objContainer;
				},
				table: function() {     /*  This function is used for creating the layout of the excel region.
                                                        *   Totally 4 Regions. First region can be clicked for selecting the complete excel
                                                        *   Second is bartop. Third is Bar Left. Fourth is data region(Pane).
                                                        */
				...
				keyDownHandler: {
						...
						if (e.ctrlKey) {
							...
							jQuery(document).one('keyup', function() {
								...
								//var val = formula.val(); //once ctrl+v is hit formula now has the data we need
                                                                var val = jS.copyContent;
								var firstValue = '';
								formula.val('');
								var tdsBefore = jQuery('<div />');
								var tdsAfter = jQuery('<div />');

								var row = val.split("<NEXTROW>"); //break at rows
								for (var i = 0; i < row.length; i++) {
									var col = row[i].split("<NEXTCOL>"); //break at columns
									for (var j = 0; j < col.length; j++) {
										if (col[j]) {
											var td = jQuery(jS.getTd(jS.i, i + loc[0], j + loc[1]));
                                                                                        var currRow = i+loc[0]+1;
                                                                                        var currCol = j+loc[1]+1;

											tdsBefore.append(td.clone());
                                                                                        var temp = col[j].split("~");
                                                                                        var origRow = cE.parseLocation(temp[0])[0];
                                                                                        var origCol = cE.parseLocation(temp[0])[1];

                                                                                        col[j] = temp[1];

											if ((col[j] + '').charAt(0) == '=') { //we need to know if it's a formula here

                                                                                                if (col[j].match("[A-Za-z]+[0-9]+") != null){
                                                                                                    var tempVal = col[j];
                                                                                                    while(true){
                                                                                                        var matches = tempVal.match("[A-Za-z]+[0-9]+");
                                                                                                        if (matches != null){
                                                                                                            var formulaRow = cE.parseLocation(matches[0])[0];
                                                                                                            var formulaCol = cE.parseLocation(matches[0])[1];

                                                                                                            //Check Row Difference
                                                                                                            var rowDiff = origRow - formulaRow;
                                                                                                            //Check Column Difference
                                                                                                            var colDiff = origCol - formulaCol;

                                                                                                            var tempMatch = cE.columnLabelString(currCol-colDiff);
                                                                                                            tempMatch = tempMatch + (currRow-rowDiff);
                                                                                                            col[j] = col[j].replace(matches[0],tempMatch);
                                                                                                            tempVal = tempVal.replace(matches[0],"");
                                                                                                        }
                                                                                                        else{
                                                                                                            break;
                                                                                                        }
					...
					formulaOnKeyDown: function(e) {
							...
							case key.V:return jS.evt.keyDownHandler.pasteOverCells(e);
								break;
                                                        case key.C:jS.copyContent = jS.obj.copyArea().val();
...
jS.obj.debugArea().val("Key received on "+r+ " "+c);
					switch (e.keyCode) {
						case key.UP:r--;break;
						...
						case key.ENTER:r++;
							...
							break;
						case key.TAB:
jS.obj.debugArea().val("Tab Key received on "+r+ " "+c);
							overrideIsEdit = true;
							if (e.shiftKey) {
								c--;
							} else {
				...
				cellOnMouseDown: function(e) {
					...
					if (e.shiftKey) {
						jS.getTdRange(e, jS.obj.formula().val());
                                                ...
                                                jS.populateCopyArea();
/*                                                var copyText = "";
//alert(jS.highlightedLast.rowStart+" "+jS.highlightedLast.colStart+" "+jS.highlightedLast.rowEnd+" "+jS.highlightedLast.colEnd);
                                                for (var i=jS.highlightedLast.rowStart;i<=jS.highlightedLast.rowEnd;i++){
                                                    for (var j=jS.highlightedLast.colStart;j<=jS.highlightedLast.colEnd;j++){
                                                        var td = jQuery(jS.getTd(jS.i, i, j));
                                                        var cellName = cE.columnLabelString(j+1) + "" + (i+1);
				...
				scrollBars: function() {
					...
					jS.obj.pane().scroll(function() {
						...
						o.barLeft.scrollTop(o.pane.scrollTop());
                                                jS.obj.debugArea().val("scroll position : "+o.pane.scrollLeft()+" max : "+o.pane.attr("scrollWidth")+" "+o.pane.width());
...
//jS.obj.debugArea().val("Row:"+row+" Col:"+col+"Row Offset:"+rowOffset+" Col Offset:"+colOffset+" isBefore:"+isBefore);
				var shiftedRange = {
			...
			formatSheet: function(o) {  /* This function is used for setting the width and height of all the columns and rows of
...
var cols = jS.sheetSize()[1] + 1;
if (s.width/cols > 100)
    s.newColumnWidth = (s.width)/cols;
			...
			themeRoller: {
				...
				cell: {
					...
					clearHighlighted: function() {
                                            jS.obj.debugArea().val(jS.highlightedLast.rowStart+" "+jS.highlightedLast.colStart+" "+jS.highlightedLast.rowEnd+" "+jS.highlightedLast.colEnd);
						jS.obj.cellHighlighted()
							.removeClass(jS.cl.cellHighlighted + ' ' + jS.cl.uiCellHighlighted);
jS.obj.debugArea().val("got the highlighted cell");
			...
			cellEdit: function(td, isDrag) {
				...
				jS.evt.cellEditDone();      // To store the value of the earlier edited cell
				jS.followMe(td);

				var loc = jS.getTdLocation(td);
//jS.obj.debugArea().val(loc[0]+ " " +loc[1]+ " "+td.attr("rowNum")+" "+td.attr("colNum"));
                                var currentCell = cE.columnLabelString(loc[1] + 1) + (loc[0] + 1);
				...
				if (!v) {
					v = jS.manageHtmlToText(td.html());
                                ...
                                if (editable){
                                    var formulaObj = jS.obj.formula();
                                    var readOnly = formulaObj.attr('readonly');
                                    if(readOnly)
                                        formulaObj.removeAttr('readonly');
                                    formulaObj
					.val(v)
					.focus()
					.select();
					...
					.val(v)
                                ...
                                jS.obj.copyArea().val(currentCell+"~"+v);
...
//jS.obj.debugArea().val("Starting Location : "+loc[0]+" " +loc[1]+" ");
					jS.cellLast.row = jS.rowLast = loc[0];
					jS.cellLast.col = jS.colLast = loc[1];
//jS.obj.debugArea().val("going to clear");
					jS.themeRoller.bar.clearActive();
//jS.obj.debugArea().val("cleared bar active");
					jS.themeRoller.cell.clearHighlighted();
//jS.obj.debugArea().val("cleared");
...
//jS.obj.debugArea().val("theme roller done");
					var selectModel;
...
//jS.obj.debugArea().val("going to switch");
...
//jS.obj.debugArea().val("finished switch. Going to drag : "+isDrag);
					if (isDrag) {
						jS.obj.pane()
							.mousemove(function(e) {
								var endLoc = jS.getTdLocation([e.target]);
//jS.obj.debugArea().val(loc[0]+" " +loc[1]+" "+endLoc[0]+" "+endLoc[1]);
...
//jS.obj.debugArea().val(jS.highlightedLast.rowStart+" "+jS.highlightedLast.colStart+" "+jS.highlightedLast.rowEnd+" "+jS.highlightedLast.colEnd);
			...
			openSheet: function(o) {    /* This is the function that will be called for opening a new spreadSheet.
                                                    * Input will be the html table content. This html table content will be converted into
                                                    * Excel format and it'll be displayed. Every table tag in the html will be displayed as a
                                                    * new sheet in Excel. */
					...
					if (!o) {
						jQuery('<div />').load(s.urlGet, function() {
							var sheets = jQuery(this).children('table');
							...
							jS.controlFactory.sheetUI(jQuery(this), i,  function() {
								fnAfter(i, sheets.length);
                                        ...
                                        var paneRightClickMenu = jQuery("<ul id=\"contextmenu2\" class=\"jqcontextmenu\">"+
                                                    ...
                                                    "<li><a href=\"javascript:insertComment(this)\">Insert Comment</a></li>"+
                                                    "<li><a href=\"javascript:clearComment(this)\">Clear Comment</a></li>"+
                                        ...
                                        jS.obj.pane().addcontextmenu('contextmenu2');
...
/*
                                        var formatCellsPopUp = jQuery("<div id=\"formatCell\" style=\"display: none;height:100px;width:100px;\" title=\"Format Cell\">"+
                                         "<table><tr>"+
                                         "<td>Change Background Color</td>"+
                                         "<td>Change Font Color</td>"+
                                         "</tr></table>"+
                                        "</div>");
                                        jQuery("#dummyArea").append(formatCellsPopUp);
*/
					jS.setDirty(false);
			...
			importColumn: function(columnArray) {
				...
				jS.calc(jS.i);
                        ...
                        dynamicAddRow: function(){
                            //jS.controlFactory.addRowMulti(1,1);
                            var row = jS.rowLast;
                            jS.controlFactory.addCells(row+1,false,null,1,'row');
                        },
                        dynamicAddCol: function(colName){
                            $("#addExcelColumnDiv").dialog('close');
                            /*if (where == 'before')
                                jS.controlFactory.addCells(col+1,true,null,1,'col');
                            else*/
                            var col = jS.colLast;
                            jS.controlFactory.addCells(col+1,false,null,1,'col');

                            var body = jS.obj.pane().find("thead");
                            var selector = "#"+jS.i+"_table"+jS.i+"_cell_c"+(col+1)+"_r0";

                            var td = body.find(selector);
                            if (td){
                                td.html(colName);
                                td.css("font-size","11px");
                                td.css("background-color","#b4d9ee");
                            }

                            /* Make ajax call to store the new column information in Container */
                            $.ajax({
                                url:s.contextPath+'/reportTable.do?reportBy=addRTExcelColumn&REPORT_ID='+s.reportId+'&col='+col+'&columnName='+colName,
                                success: function(data){
                                    jS.addToDynamicColumn(col);
                                }
                            });
                        },
                        dynamicAddTargetCol: function(colIndex, colName){
                            ...
                            /* Make ajax call to store the new column information in Container */
                            $.ajax({
                                url:s.contextPath+'/reportTable.do?reportBy=addRTTargetColumn&REPORT_ID='+s.reportId+'&col='+col+'&columnName='+colName,
                                success: function(data){
                                    jS.addToDynamicColumn(col);
                                }
                            });
                        },
                        addToDynamicColumn: function(col){
                        ...
                        openFormatCellDialog: function(){
                            $("#formatCell").dialog('open');
                        },
                        openAddColumnDialog: function(){
                            $("#addExcelColumnDiv").dialog('open');
                        },
                        formatExcelCell: function( bgColor,fontColor){
                            $("#formatCell").dialog('close');
                            var row = jS.rowLast;
                            var col = jS.colLast;
                            var td = jQuery("#"+jS.getTdId(jS.i, jS.rowLast, jS.colLast));
                            ...
                            row = td.attr("rowNum");
                            var reqString = s.contextPath+'/reportTable.do?reportBy=storeFormat&REPORT_ID='+s.reportId+'&row='+row+'&col='+col+'&bgColor='+bgColor+'&fontColor='+fontColor;
                        ...
                        clearFormat: function(){
                            ...
                            row = td.attr("rowNum");
                            $.ajax({
                                url:s.contextPath+'/reportTable.do?reportBy=clearFormat&REPORT_ID='+s.reportId+'&row='+row+'&col='+col,
                                success: function(data){
                                }
                            });
                        ...
                        insertComment: function(){
                            ...
                            commentArea.focus();
                            commentArea.attr("row", row);
                            commentArea.attr("col", col);
                        ...
                        addComment: function(){
                            var commentWindow = jQuery("#commentWindow");
                            var commentArea = jQuery("#commentArea");
                            var row = commentArea.attr("row");
                            var col = commentArea.attr("col");
                            var commentText = commentArea.val();
                            commentWindow.hide();
                            jS.commentWindowOpen = false;
                            if (commentText != "Type your comment here"){
                                var td = jQuery("#"+jS.getTdId(jS.i, row, col));
                                td.attr("comment", commentText);
                                td.attr("title", commentText);
                                td.addClass('jSheetCommentWindow');
                                row = td.attr("rowNum");
                                $.ajax({
                                    url:s.contextPath+'/reportTable.do?reportBy=storeComment&REPORT_ID='+s.reportId+'&row='+row+'&col='+col+'&comment='+commentText,
                                    success: function(data){
                                    }
                                });
                            }
                        ...
                        clearComment: function(){
                            ...
                            row = td.attr("rowNum");
                            $.ajax({
                                url:s.contextPath+'/reportTable.do?reportBy=clearComment&REPORT_ID='+s.reportId+'&row='+row+'&col='+col,
                                success: function(data){
                                }
                            });
                        },
                        setExcelWidth: function(width){
                            alert(width+" "+jS.sheetSize());
                        },
                        addBarTopMenu: function(menuContent){
                            /*var test = document.getElementById("barTopMenuTD0");
                            ...
                            jQuery(menuContent).find("td").each(function(i){
                                ...
                                var id = "#barTopMenuDiv"+i;
                                var td = jS.obj.barTop().find(id);
                                $("#dummyArea").append(ul);
                                td.addcontextmenu(ctxMenuId);
                            });
                        },
                        saveSheet: function(){
                            var xml = "";
                            var body = jS.obj.pane().find("tbody");
                            var size = jS.sheetSize();
                            var fromRow = 1;
                            var toRow = s.toRow - s.fromRow + 1;        // To avoid header. Need to find a correct logic

                            //alert(body.html());

                            for (var i=0;i<jS.insertedCols.length;i++){
                                xml = "";
                                //var saveTds = body.find("td:nth-child("+(jS.insertedCols[i]+1)+")");
                                xml = xml + "<ExcelColumn>";
                                /*saveTds.each(function(i) {
                                    xml = xml + "<data>"+jQuery(this).html()+"</data>";
                                });*/

                                var col = jS.insertedCols[i];
                                var offSet = 0;
                                for (var j=fromRow;j<toRow;j++){        // This needs to be changed if the report is a crosstab
                                    var selector = "#"+jS.i+"_table"+jS.i+"_cell_c"+col+"_r"+(j+offSet);
                                    var td = body.find(selector);
                                    if (td){
                                        var rowIndex = td.attr('rowNum');
//                                        if (rowIndex){
                                            var v = td.attr('formula');
                                            if (!v)
                                                v = td.html();
                                            xml = xml + "<data>"+v+"</data>";
//                                        }
//                                        else{   //The table cell doesn't correspond to any data. It might be a subtotal cell
//                                            j--;
//                                            offSet++;
//                                        }
                                    }
                                ...
                                alert(xml);
                                $.ajax({
                                    url:s.contextPath+'/reportTable.do?reportBy=updateRTExcelColumn&REPORT_ID='+s.reportId+'&col='+jS.insertedCols[i]+'&data='+xml+'&fromRow='+s.fromRow+'&toRow='+s.toRow,
                                    success: function(data){
                                        alert("Data Saved");
                                    }
                                });
                        ...
                        exportExcel: function(filename){
                            $.ajax({
//                                url:s.contextPath+'/reportTable.do?reportBy=exportRTExcelColumn&REPORT_ID='+s.reportId+"&fileName=",
                                url:s.contextPath+'/ExporttoExcel.jsp?REPORT_ID='+s.reportId,
                                success: function(data){
                                }
                            });
                        },
                        importExcel: function(){
                            $.ajax({
                                url:s.contextPath+'/reportTable.do?reportBy=importRTExcelColumn&REPORT_ID='+s.reportId,
                                success: function(data){
                                }
                            });
                        },
			...
			sheetSyncSize: function() {
				...
				h = h - jS.attrH.height(jS.obj.controls()) - jS.attrH.height(jS.obj.barTopParent()) - (s.boxModelCorrection * 2);  // for the table controls
			...
			cellSetActiveBar: function(type, start, end) {
				...
				var loc = jS.sheetSize();
                                ...
                                if (type == 'row' && (start > loc[0] || end > loc[0]))
                                    return;
                                else if (type == 'col' && (start > loc[1] || end > loc[1]))
                                    return;
                            //jS.obj.debugArea().val(" in cell set active bar "+type+" "+start+" "+end);
				...
				cycleFn();
                                ...
                                if (type == 'col'){
                                    ...
                                    jS.highlightedLast.colStart = first;
                                    jS.highlightedLast.rowEnd = loc[0];
                                    jS.highlightedLast.colEnd = last;
                                }
                                else if (type == 'row'){
                                    jS.highlightedLast.td = td;
                                    jS.highlightedLast.rowStart = first;
                                    jS.highlightedLast.colStart = 0;
                                    jS.highlightedLast.rowEnd = last;
                                    jS.highlightedLast.colEnd = loc[1];
                                }
                                else if (type == 'all'){
                                    jS.highlightedLast.td = td;
				...
				if (e) { //if from an event, we use mousemove method
					...
					if (cells.length) {
						...
						return '';
                                ...
                                jS.highlightedLast.rowEnd = jS.getTdLocation([e.target])[0];
                                jS.highlightedLast.colEnd = jS.getTdLocation([e.target])[1];
			},
			...
			getTdLocation: function(td) {
				...
				//var row = parseInt(td[0].parentNode.rowIndex);
                                ...
                                var col, row;
                                var jQTd = jQuery(td[0]);
                                /*if (jQTd.attr("colNum"))
                                    col = parseInt(jQTd.attr("colNum"));
                                else*/
                                    col = parseInt(td[0].cellIndex);
                                /*if (jQTd.attr("rowNum"))
                                    row = parseInt(jQTd.attr("rowNum"));
                                else*/
...
//                alert("h : "+ s.height + " w : "+s.width);
		...
		jS.openSheet(o);
                for (var index=0;index<s.rtColumns.length;index++){
                    jS.insertedCols.push(s.rtColumns[index]);
                }

                if (jS.insertedCols.length > 0 || s.targetEntryApplicable){    //Runtime columns present. Show the save button
                    $("#firstRowTable").show();
...
function insertComment(t){
    jQuery.sheet.instance[0].insertComment();
}
function clearComment(t){
    jQuery.sheet.instance[0].clearComment();
}
function openFormatCellDialog(t){
    jQuery.sheet.instance[0].openFormatCellDialog();
}
function formatExcelSheetCell(bgColor, fontColor){
    jQuery.sheet.instance[0].formatExcelCell( bgColor,fontColor);
}
function openAddColumnDialog(t){
    jQuery.sheet.instance[0].openAddColumnDialog();
}
function addCol(colName){
    jQuery.sheet.instance[0].dynamicAddCol(colName);
}
function addRow(t){
    jQuery.sheet.instance[0].dynamicAddRow();
}
function clearFormat(t){
    jQuery.sheet.instance[0].clearFormat();
}
