jQuery(function ($) {
	$.fn.quicksearch = function (opt) {
		...
		{ //alert('single'+k+"---"+value)
           //alert(value.indexOf(k))
					...
					{   //alert('if')
						...
						{// alert('in else')
							$(el).each(function (i) {
                               // alert(score[k][i]+"--"+i)
								if (score[k][i])
								{  //alert(select_element(this))
									select_element(this).show();
                                    //alert( document.getElementById("test1"))
                                   // var clsId=document.getElementById("test1");
                                 // clsId.className='closed collapsable lastCollapsable';
								}
								else
								{
