<%-- 
    Document   : pbAddMoreAODimensionsRep
    Created on : Dec 10, 2015, 2:24:00 PM
    Author     : Bhargavi Parsi
--%>

<%@page contentType="text/html" pageEncoding="UTF-8" import="com.progen.reportdesigner.db.ReportTemplateDAO,com.progen.report.PbReportCollection,com.progen.report.display.DisplayParameters,prg.db.PbReturnObject,utils.db.ProgenConnection,prg.db.PbDb,prg.db.Container,java.math.*,java.util.*,java.text.*"%>
...
<html>
    <head>
        ...
        <script src="<%=contxtPath%>/javascript/lib/jquery/js/jquery-1.8.3.min.js" type="text/javascript"></script>
        <link type="text/css" href="<%=contxtPath%>/stylesheets/themes/<%=themeColor%>/jquery-ui-1.7.3.custom_new.css" rel="stylesheet" />
        <script type="text/javascript" src="<%=contxtPath%>/javascript/lib/jquery/js/jquery.editablediv.js"></script>
        <script type="text/javascript" src="<%=contxtPath%>https://code.jquery.com/jquery-1.11.2.min.js"></script>
        <script type="text/javascript" src="<%=contxtPath%>/javascript/lib/jquery/js/jquery-1.8.23-custom.min.js"></script>
        <link type="text/css" href="<%=contxtPath%>/stylesheets/themes/<%=themeColor%>/jquery-1.8.23-custom.min.css" rel="stylesheet" />
        ...
        <script type="text/javascript" src="<%=contxtPath%>/javascript/quicksearch.js"></script>
        <script type="text/javascript" src="<%=contxtPath%>/javascript/reportDesign.js"></script>
        <link type="text/css" href="<%=contxtPath%>/stylesheets/themes/<%=themeColor%>/metadataButton.css" rel="stylesheet" />
        <link href="<%=contxtPath%>/stylesheets/jquery.contextMenu.css" rel="stylesheet" type="text/css" />
        <script  type="text/javascript" src="<%=contxtPath%>/javascript/jquery.contextMenu.js" ></script>
        <script  type="text/javascript" src="<%=contxtPath%>/javascript/pbReportViewerJS.js" ></script>
        <%
                    ...
                    session.setAttribute("aoId",aoid);
  ...
  $.ajax({
                            url: 'reportTemplateAction.do?templateParam=getAOMasterMap&aoid='+<%=aoid%>,
...
<!--                                        <td width="25%" valign="top"  id="draggableKpis" valign="top"style="border: 1px solid black; ">
            ...
            <input type="button"  class="navtitle-hover" onclick="createreport2()" style="margin-top: 10px;width:auto; position: fixed;left: 620px;" value="Done" >
        </form>
                                                         <script type="text/javascript">
...
//            }
                        ...
                        function setValueToContainerInDesigner(ctxpath,repId,aoid,tabLst){
                                    //alert("Ctx:"+ctxpath+"RepID..:"+repId+"role....:"+bizRoles);
                            $("#loading").show();
                           // var frameObj=document.getElementById("dataDispmem");
            $.post(ctxpath+"/reportViewer.do?reportBy=setTableListToContainer&repId="+repId+'&tabLst='+tabLst,
                function(data){
                     ...
                     $.post(ctxpath+"/reportTemplateAction.do?templateParam=getMeasures2&aoid="+aoid+'&REPORTID='+repId+'&tableList=true',
                       function(data1){
            ...
            function showListInDesigner(ctxpath,paramslist){
                 ...
                 // alert("Params : "+paramslist+"ctx:"+ctxPath);
                       var tablist='';
                $.post(ctxpath+"/reportViewer.do?reportBy=getTableList&paramslist="+paramslist+'&aoid='+<%=aoid%>,
                function(data){
                    //                    alert(data);
                    var jsonVar=eval('('+data+')')
                 ...
                 setValueToContainerInDesigner(ctxpath,<%=reportId%>,<%=aoid%>,tablist);
            }
   ...
   //Added By Ram
      ...
      var c = document.getElementById("sortable").childNodes;
       if(c.length==1){
          ...
          alert("Report is being Creating....\n\
                ...
                if(msrIds.length!=0){
                    ...
                    if(action!=null && action=="measChange"){
                        $.ajax({
                          ...
                          url: '<%=request.getContextPath()%>/reportViewer.do?reportBy=tableMeasureChanges&tableMsrs='+msrs+'&REPORTID=<%=reportId%>',
                            success: function(data){
                              setTimeout(parent.PreviewTable(), 1000);
                            }
                        });
                    }else{
                        $.ajax({
...
//                                parent.PreviewTable();
            ...
            {
             //   alert("Inside Create Report");
//                getAOMasterMap(<%=aoid%>);
//alert("bcallfunctionefore::"+callfunction);
...
//                  setTimeout(saveMeasuresAo(), 1000);
...
window.parent.document.getElementById('desginerDiv11').style.display='none';
            ...
            //added by mohit for ao
                ...
                function getAOMasterMap(aoid)
            {
                alert("getAOMasterMap::"+aoid)
...
//               alert("saveMeasuresAo")
               if(From != null && From=="busTemplateView"){
                    dispBusTemplateMesures1();
                }else{
                    //                 parent.cancelTabMeasure();
//setTimeout(dispMeasures2(), 200);
                   dispMeasures2();
