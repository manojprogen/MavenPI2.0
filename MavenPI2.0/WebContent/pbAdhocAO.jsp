<%--
    Document   : pbAdhocAO
    Created on : Nov 28, 2015, 17:13:25 PM
    Author     : amar.pal@progenbusiness.com
--%>


<%@page contentType="text/html" pageEncoding="UTF-8" import="com.progen.report.display.DisplayParameters,prg.db.PbDb,java.util.HashMap,prg.db.Container,java.util.ArrayList,prg.util.screenDimensions,com.progen.report.PbReportCollection,com.progen.action.UserStatusHelper,utils.db.ProgenParam,prg.db.PbReturnObject,com.progen.reportdesigner.db.ReportTemplateDAO"%>
...
//                         params=displayParameters.designerParams(request.getContextPath());
                    ...
                    String isFromAOEdit =(String)request.getAttribute("isFromAOEdit");
                    System.out.println(isFromAOEdit);
...
#center250b { width: auto;height: 65px;   background:none; }
            ...
            <script type="text/javascript">
               $(document).ready(function() {
                ...
                var frameObj=document.getElementById("addmoreParamFrame1");

                var IsrepAdhoc='<%=IsrepAdhoc%>';
                var isFromAOEdit1='<%=isFromAOEdit%>'
                var source="reportTemplateAction.do?templateParam=addMoreDimensionsAO&foldersIds="+roleId+"&IsrepAdhoc="+IsrepAdhoc+"&isFromAOEdit="+isFromAOEdit1+"&REPORTID="+document.getElementById("REPORTID").value;
                frameObj.src=source;
...
<!--                          <td valign="top" style="width:3%;position: relative">
                             <a href="javascript:void(0)" class="ui-icon ui-icon-plusthick" onclick="AddMoreDims(<%=IsrepAdhoc%>)"></a>
                              ...
                              <td  id="saveadhoc" valign="top" style="width:1%;position: relative;display: ''">
...
<!--                         <div class="flagDiv" id="paramDesign" style="display:none;width:15%;height:100%;background-color:white; direction: ltr; float: right; position:absolute;text-align:left;border: 1px solid LightGrey;left:0px;top:112px;z-index: 10000;">
                      <%--<%=params %>--%>
                         ...
                         <div id="measuresDialogAO" style="display:none" title="Add Measures">
    <table><tr><td>
            <input id="tableList" type="checkbox" onclick="getDisplayTablesInDesigner('<%=request.getContextPath() %>','','<%=ReportId%>')">All</td>
                    ...
                    <td id="goButton" onclick="setValueToContainerInDesignerAO('<%=request.getContextPath() %>','<%=ReportId%>')"><input type="button" value="GO" class="navtitle-hover"></td>
            </tr></table>
...
//                        alert(jsonname[i])
    ...
    function setValueToContainerInDesignerAO(ctxpath,repId,bizRoles){
       var frameObj=document.getElementById("dataDispmem");
       $("#paramVals").hide();
       ...
       $("#tabListDiv").hide();
           ...
           $("#tabsListVals").val("");
           alert("Inside Adhoc");
            $.post(ctxpath+"/reportViewer.do?reportBy=setTableListToContainerAO&repId="+repId+'&tabLst='+tabLst,
                function(data){
                    var source="reportTemplateAction.do?templateParam=getMeasuresAO&foldersIds="+document.getElementById("roleid").value+'&REPORTID='+document.getElementById("REPORTID").value+'&tableList=true';
                    frameObj.src=source;
                });
   }
    function cancelDim(){
