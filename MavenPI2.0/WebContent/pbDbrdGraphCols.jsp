...
<%@page contentType="text/html" pageEncoding="UTF-8" import="java.sql.*,com.progen.report.PbReportCollection,prg.db.Container,com.progen.report.pbDashboardCollection"%>
<%@page import="utils.db.ProgenConnection,com.progen.reportview.db.PbReportViewerDAO,java.util.HashMap,java.util.ArrayList,com.progen.reportdesigner.db.ReportTemplateDAO,prg.db.PbReturnObject,prg.db.PbDb" %>
...
<%
            ...
            if(grpType.equals("undefined"))
                {
                grpType=(String)session1.getAttribute("grptype");
                
                }
            String grpId = request.getParameter("gid");
            String divId = request.getParameter("divId");
            String dashboardId=request.getParameter("dashboardId");
            String selectedDim = request.getParameter("selectedDim");
...
<!--           <script type="text/javascript" src="<%=contextPath%>/javascript/ui.datepicker.js"></script>
        ...
        <%
                ...
                String[] ids = collect.reportBizRoles;
                    ...
                    factsHtml = templateDAO.getMeasuresforGrpInsights(folderIds, Parameters, request.getContextPath());
                    } else {
          ...
          <script>
            ...
            <%--                $("#measures").draggable({
                ...
                function saveCols(grpId,grpType,divId){
                    ...
                    var grpSize="Medium";
                      var urlVar=""
                      // add by ramesh janakuttu on jun 20 2012
//                 if(grpType=="textkpi")
//                     {
//                       
//                          urlVar='dashboardTemplateAction.do?templateParam2=buildDbrdGraphs&grpColumns='+cols+'&grpColumnsNames='+grpColNames+'&grpType='+grpType+'&grpSize='+grpSize+'&grpId='+grpId+'&divId='+divId+'&dashboardId='+parent.document.getElementById("dbrdId").value+'&viewby='+viewby+'&DashboardGraphName='+parent.document.getElementById("HiddenNewDbrdGraphName").value+'&displayType=textkpi'
//                       
//                     }
                 if(grpType!="table" && grpType!="groupMeassure"){
                      urlVar='dashboardTemplateAction.do?templateParam2=buildDbrdGraphs&grpColumns='+cols+'&grpColumnsNames='+grpColNames+'&grpType='+grpType+'&grpSize='+grpSize+'&grpId='+grpId+'&divId='+divId+'&dashboardId='+parent.document.getElementById("dbrdId").value+'&viewby='+viewby+'&DashboardGraphName='+parent.document.getElementById("HiddenNewDbrdGraphName").value+'&selectedDim='+<%=selectedDim%>
                  }
                  else if(grpType=="groupMeassure")
                  {
                        urlVar='dashboardTemplateAction.do?templateParam2=buildDbrdGraphs&Ids='+cols+'&Names='+grpColNames+'&grpType='+grpType+'&grpSize='+grpSize+'&grpId='+grpId+'&divId='+divId+'&dashboardId='+parent.document.getElementById("dbrdId").value+'&viewby='+viewby+'&DashboardGraphName='+parent.document.getElementById("HiddenNewDbrdGraphName").value+'&selectedDim='+<%=selectedDim%>+'&displayType=groupMeassure'
                  }
                      else{
                      urlVar='dashboardTemplateAction.do?templateParam2=buildDbrdGraphs&grpColumns='+cols+'&grpColumnsNames='+grpColNames+'&grpType='+grpType+'&grpSize='+grpSize+'&grpId='+grpId+'&divId='+divId+'&dashboardId='+parent.document.getElementById("dbrdId").value+'&viewby='+viewby+'&DashboardGraphName='+parent.document.getElementById("HiddenNewDbrdGraphName").value+'&displayType=table'
                  }
