...
<%@page contentType="text/html"  pageEncoding="UTF-8" import="com.progen.i18n.TranslaterHelper,java.util.Locale,com.progen.report.query.PbReportQuery,java.text.DecimalFormat,com.progen.reportview.bd.PbReportViewerBD,com.progen.reportview.db.ProgenReportViewerDAO"%>
<%@page import="com.progen.reportview.bd.PbReportViewerBD,com.progen.report.display.DisplayParameters,java.text.DateFormat,com.progen.users.PrivilegeManager,com.progen.users.ProgenUser,com.progen.charts.JqplotGraphProperty,com.progen.reportview.bd.PbReportViewerBD,com.progen.action.UserStatusHelper,com.progen.users.UserLayerDAO,com.progen.reportview.db.PbReportViewerDAO"%>
<%@page import="prg.db.Container,com.progen.report.segmentation.Segment,com.progen.report.whatIf.WhatIfScenario,com.progen.graph.info.ProgenGraphInfo,java.awt.*,utils.db.*,java.io.*,java.util.*,com.progen.db.*,prg.db.DataTracker,prg.util.screenDimensions"%>
<%@page import="prg.db.*,prg.db.PbDb,prg.db.Session,prg.db.PbReturnObject,java.text.SimpleDateFormat,java.sql.*,com.progen.report.PbReportCollection,com.progen.reportdesigner.db.DashboardTemplateDAO,com.progen.reportdesigner.db.ReportTemplateDAO"%>
...
String defaulttab = String.valueOf(session.getAttribute("DEFAULT_TAB"));    
   ...
   String aoAsGoId = (String)session.getAttribute("aoAsGoId");
    ...
    ArrayList<String> timeinfo = collect.timeDetailsArray;String qtrtype=collect.Qtrtype;String newUIyear=container.getNewUIyr();
    String[] vals1 = null;
    String autoDate = "";
    String vals = " ";
...
<%
    ...
    PbDb pbdb = new PbDb();
//    String rolesidqry = "select folder_id,ao_id from prg_ar_report_details where report_id=" + PbReportId;
//    PbReturnObject roleobj = pbdb.execSelectSQL(rolesidqry);
//    String rolesid = roleobj.getFieldValueString(0, 0);
//      String aoid = roleobj.getFieldValueString(0, 1);
//       String folderName="";
//   if(aoid==null || aoid.equalsIgnoreCase(""))
//     folderName = collect.getReportBizRoleName(rolesid);
//    else{
//    container.setAOId(aoid);}
//       collect.getReportBizRoles();
  String rolesid = collect.reportBizRoles[0]; 
    String folderName = collect.getReportBizRoleName(rolesid);
     String aoid = collect.AOId;
    if(aoid!=null && !aoid.equalsIgnoreCase(""))
        container.setAOId(aoid);
...
<style type="text/css">
.loading_image{display:block;position:absolute;top:0;left:-10%;width:150%;height:170pc;background-color:#000;z-index:1001;-moz-opacity:.8;opacity:.8;filter:alpha(opacity=50);z-index:1001;overflow:hidden}
#ui-datepicker-div{z-index:9999999;display:none}
#selectable{background:none repeat scroll 0 0 #fff;list-style:none outside none;width:60%}
#selectable li{font-family:Helvetica,Arial,sans-serif;font-size:11px;margin-bottom:1px;margin-top:1px;width:120px;background:#b4d9ee;cursor:pointer;color:#000;width:auto;padding:.4em}
#selectable li:hover{background:#fff}
</style>
<%
...
<!--        <link rel="stylesheet" type="text/css" href="<%= ctxpath%>/stylesheets/MultiPurposeCss/mbContainer.css" title="style"  media="screen"/>-->
        <link rel="stylesheet" href="<%= ctxpath%>/stylesheets/treeviewstyle/jquery.treeview.css" />
        ...
        <!--<link type="text/css" href="<%=ctxpath%>/styleshee //ts/themes/<%=themeColor%>/css.css" rel="stylesheet" />-->
        <link type="text/css" href="<%=ctxpath%>/stylesheets/themes/<%=themeColor%>/ReportCss.css" rel="stylesheet"/>
<!--        <link type="text/css" href="<%=ctxpath%>/stylesheets/base/BreadCrumb.css" rel="stylesheet" />-->
        ...
        <script type="text/javascript" src="<%=ctxpath%>/javascript/lib/jquery/js/jquery-1.7.2.min.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/JS/jquery-ui.min.js"></script>
        <script type="text/javascript" src="<%= ctxpath%>/JS/reportgbl/jquery.ui.dialog.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/javascript/pbReportViewerJS.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/TableDisplay/JS/whatIfScenario.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/JS/stickyNote.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/javascript/reportviewer/ReportViewer.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/JS/defineDialog.js"></script>
        ...
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/d3/ion.rangeSlider.js"></script>
        ...
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/d3/progenChartType.js"></script>
        <%  if (status.equalsIgnoreCase("OK")) {%>
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/d3/chartTypesGroupVeraction.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/d3/chartTypeOthersVeraction.js"></script>
        <%} else {%>
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/d3/chartTypesGroup.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/d3/chartTypeOthers.js"></script>
        <%}%>
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/d3/chartTypeBars.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/d3/chartTypeCircular.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/d3/chartTypeLine.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/d3/chartTypeMap.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/d3/chartTypeDashboard.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/d3/chartTypeCombo.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/jquery/ui/jquery.jBreadCrumb.1.1.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/jquery.mb.containerPlus.2.5.1/inc/jquery.metadata.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/jQuery/jquery.mb.containerPlus.2.5.1/inc/mbContainer.js"></script>
<!--        <script type="text/javascript" src="<%=ctxpath%>/javascript/ui.slider.js"></script>-->
        <script type="text/javascript" src="<%= ctxpath%>//dragAndDropTable.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/javascript/quicksearch.js"></script>
        <script src="<%=ctxpath%>/javascript/treeview/jquery.treeview.js" type="text/javascript"></script>
        <script src="<%=ctxpath%>/javascript/treeview/jquery.cookie.js" type="text/javascript"></script>
        <script type="text/javascript" src="<%=ctxpath%>/JS/shrink.js"></script>
        <script type="text/javascript" src="<%=ctxpath%>/JS/bootstrap.min.js"></script>
        <script src="<%=ctxpath%>/javascript/languageResource/MessageBundle_<%=session.getAttribute("UserLocaleFormat")%>.js" type="text/javascript"></script>
        <% if (PrivilegeManager.isModuleEnabledForUser("MAP", USERID) && container.isMapEnabled()) {%>
        <%}%>
        <link type="text/css" href="<%=ctxpath%>/datedesign.css" rel="stylesheet"/>
        <link type="text/css" href="<%=ctxpath%>/css/d3/jquery.dataTables.css" rel="stylesheet"/>
        <link type="text/css" href="<%=ctxpath%>/css/global.css" rel="stylesheet" />
        <!--<link type="text/css" href="<%=ctxpath%>/css/d3.slider.css" rel="stylesheet" />-->
        <link type="text/css" href="<%=ctxpath%>/css/ion.rangeSlider.css" rel="stylesheet" />
        <link type="text/css" href="<%=ctxpath%>/css/ion.rangeSlider.skinFlat.css" rel="stylesheet" />
        ...
        <style type="text/css">
           .mycls{background-color:#FFF;border:0 solid #d7faff;height:180px;overflow:auto;width:180px}
            .suggestLink{position:relative;background-color:#FFF;border:0 solid #000;border-top-width:0;padding:2px 6px;left:3px;min-width:20px;max-width:150px}
            .innerDiv{overflow:auto;height:95%;width:95%}
            .suggestLinkOver{background-color:#09C;padding:2px 6px}
            #cboRegionsuggestList{position:absolute;background-color:#FFF;text-align:left;border:1px solid #000;border-top-width:0;width:160px}
            .imageStyle{position:absolute;width:12px;height:16px;display:inline}
            .ajaxboxstyle{position:relative;background-color:#FFF;text-align:left;border:1px solid #000;border-top-width:1px;height:150px;width:230px;overflow:auto;overflow:hidden;margin:0 .5em;z-index:9999999}
            .myAjaxTable{table-layout:fixed;background-color:#FFF;text-align:left;border:0 solid #000;font-size:10px;left:4px;height:auto;border-collapse:separate;border-spacing:5px}
            .More:hover{color:red;text-decoration:none}
            .myAjaxTable td{min-width:30px;max-width:100px;text-align:left}
            #wrapper{display:inline}
            #cbostate{width:160px}
            #cboRegion{width:160px}
            .black_overlay{display:none;position:absolute;top:0;left:0;width:100%;height:200%;background-color:#000;z-index:1001;-moz-opacity:.5;opacity:.5;filter:alpha(opacity=50);overflow:auto}
            .black_start{display:none;position:absolute;top:0;left:0;width:100%;height:200%;background-color:#000;z-index:1001;-moz-opacity:.5;opacity:.5;filter:alpha(opacity=50);overflow:auto}
            .white_content{display:none;position:absolute;top:25%;left:25%;width:25%;padding:16px;border:16px solid #308dbb;background-color:#fff;z-index:1002;overflow:auto;-moz-border-radius-bottomleft:5px;-moz-border-radius-bottomright:5px;-moz-border-radius-topleft:5px;-moz-border-radius-topright:5px}
            white_contentcolor{display:none;position:absolute;top:25%;left:25%;width:25%;padding:16px;border:16px solid #308dbb;background-color:#fff;z-index:1002;overflow:auto;-moz-border-radius-bottomleft:5px;-moz-border-radius-bottomright:5px;-moz-border-radius-topleft:5px;-moz-border-radius-topright:5px}
            .white_content1{position:absolute;top:50px;left:25%;width:700px;height:400px;padding:16px;border:16px solid silver;background-color:#fff;z-index:1002;-moz-border-radius-bottomleft:5px;-moz-border-radius-bottomright:5px;-moz-border-radius-topleft:5px;-moz-border-radius-topright:5px}
            .white_content2{position:absolute;top:30%;left:38%;width:500px;height:300px;padding:16px;border:16px solid silver;background-color:#fff;z-index:1002;-moz-border-radius-bottomleft:4px;-moz-border-radius-bottomright:4px;-moz-border-radius-topleft:4px;-moz-border-radius-topright:4px}
            .ParamRegion{background-color:#e6e6e6}
            .inputbox{font-size:10px;background-color:#fff;border-right:#000 1px outset;border-right:#000 1px inset;border-top:#fff 1px inset;border-left:#fff 1px inset;border-bottom:#000 1px inset;font-family:Helvetica,Arial,sans-serif;-moz-border-radius-bottomleft:2px;-moz-border-radius-bottomright:2px;-moz-border-radius-topleft:2px;-moz-border-radius-topright:2px}
            .test{width:150px;height:150px;border:1px solid #ff9;background-color:#ff9}
            .paramTable{width:99%;height:auto}
            .colorButton{width:20px;height:20px;background-color:green}
             a{cursor:pointer}
            .leftcol{clear:left;float:left;width:100%}
            .label{background-position:8px center;background-repeat:no-repeat;border:0 solid #252525;clear:both;height:auto;width:auto;cursor:pointer;display:block;margin-bottom:0;margin-right:0;padding:0 .3em .3em 22px;font-family:Helvetica,Arial,sans-serif;font-size:100.01%}
            .MeasuresULClass{-moz-border-radius-bottomleft:4px;-moz-border-radius-bottomright:4px;-moz-border-radius-topleft:4px;-moz-border-radius-topright:4px;background:#79C9EC url(../jQuery/images/ui-bg_glass_75_79c9ec_1x400.png) repeat-x scroll 50% 50%;border:1px solid #448DAE;color:#000;font-size:11px;font-family:Helvetica,Arial,sans-serif;vertical-align:middle;height:20px;width:auto;cursor:pointer}
            .selectedMeasuresULClass{-moz-border-radius-bottomleft:4px;-moz-border-radius-bottomright:4px;-moz-border-radius-topleft:4px;-moz-border-radius-topright:4px;background:#79C9EC url(../jQuery/images/ui-bg_glass_75_79c9ec_1x400.png) repeat-x scroll 50% 50%;border:1px solid #448DAE;color:#000;font-size:11px;font-family:Helvetica,Arial,sans-serif;vertical-align:middle;height:20px;width:auto;cursor:pointer}
            .TopButtons0{-moz-border-radius:3px;background-color:#8fbc8f;color:#FFF;float:left;font-family:helvetica,arial,sans-serif;font-size:13px;font-weight:700;margin-left:10px;padding:10px 25px;text-align:center;text-decoration:none;text-shadow:none;-moz-border-radius:3px;-webkit-border-radius:3px;-webkit-transition:all .1s ease-in-out;width:70px}
            .TopButtons0:hover{text-shadow:none;text-align:center;font-family:helvetica,arial,sans-serif;color:#FFF;font-size:13px;margin-left:10px;background:#069;text-decoration:none;font-weight:700;-moz-border-radius:3px;-webkit-border-radius:3px;-webkit-transition:all .1s ease-in-out}
            .TopButtons1{-moz-border-radius:3px;color:#FFF;float:left;font-family:helvetica,arial,sans-serif;font-size:13px;font-weight:700;padding:10px 25px;text-align:center;margin-left:10px;text-decoration:none;text-shadow:none;-moz-border-radius:3px;-webkit-border-radius:3px;-webkit-transition:all .1s ease-in-out;width:70px;background-color:#069}
            .TopButtons1:hover{text-shadow:none;text-align:center;font-family:helvetica,arial,sans-serif;color:#FFF;font-size:13px;margin-left:10px;background:#8fbc8f;text-decoration:none;font-weight:700;-moz-border-radius:3px;-webkit-border-radius:3px;-webkit-transition:all .1s ease-in-out}
            .TopButtonsNames{-moz-border-radius:3px;color:#000;float:left;font-family:helvetica,arial,sans-serif;font-size:13px;font-weight:700;padding:0 15px;text-align:center;text-decoration:none;text-shadow:none;-moz-border-radius:3px;-webkit-border-radius:3px;-webkit-transition:all .1s ease-in-out;width:100px;background-color:#fff}
        </style>
...
//            }
    ...
    %>
    <body id="mainBody" onload='dbconremoveimage()' style="background-color:#fff; overflow:hidden;">
        <iframe name="widgetframe" id="widgetframe" style="display:none" ></iframe>
        <%
                    ...
                    <table style="width:100%;">
                        <tr>
                            <td valign="top" style="width:50%;">
                                <%  long st56=System.currentTimeMillis();
                                    System.out.println("*******************start time before headerPage and reportPage*******************************"+st56);
                                    if (piVersion.equalsIgnoreCase("piVersion2014")) {
        ...
        <%}%>
                                ...
                                long etime56=System.currentTimeMillis();
                                System.out.println("*******************end time after headerPage and reportPage*******************************"+(etime56-st56));
                                %>
                            </td>
                        </tr>
                    </table>
...
<!--                                 <span id="reportName" style="color: #4F4F4F;font-family:verdana;font-size:<%=reportTitleSize%>px;font-weight:bold"  title="<%=reportDesc%>"><%=reportName%></span>-->
            ...
            <tr id="menuBarUITR" style="width:100%;height:544px;max-height:100%">
                <td id="menuBarUITRTD">
                    <table width="100%" id="menuBarUI"  style="height:100%;margin-top:30px; width: 100%;border-top: 1px solid rgb(244, 244, 244); "  cellpadding="0" cellspacing="0">
                        <tr>
                             <% if(defaulttab!=null && (defaulttab.equalsIgnoreCase("Report") || defaulttab.equalsIgnoreCase("TABLE") || defaulttab.equalsIgnoreCase(""))) {    %>
                          <%--  <td id="leftTd" valign="top" style="width:10%;display:none;border:0px" align="top" >
                                <table style="width:210px;"  >
                                    <tr>
                                        <td>
                                            <div class="navsection">
                                                <% if (userdao.getFeatureEnable("Favorite Links") || userType.equalsIgnoreCase("SUPERADMIN")) {%>
                                                <div class="navtitle" id="displayfavlink" onclick="displayfavlink()" title="<bean:message bundle="Tooltips" key="reportViewer.Favlink"/>">&nbsp;<b style="font-weight:bold">Favourite Links</b>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <div class="navsection">
                                                <div class="navtitle" id="messageslink" onclick="dispScheduleReports()" title="Click here to View the schedule Reports" >&nbsp;<b style="font-weight:bold">Scheduled Reports</b>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <div class="navsection">
                                                <div class="navtitle" id="snaplink" onclick="dispSnapShots()" title="<bean:message bundle="Tooltips" key="reportViewer.Snapshot"/>">&nbsp;<b style="font-weight:bold">Snapshot</b>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <div class="navsection">
                                                <div class="navtitle" id="displayManageSticky" onclick="displayManageSticky()" title="<bean:message bundle="Tooltips" key="reportViewer.ManageStickyNotes"/>" >&nbsp;<b style="font-weight:bold">Manage Sticky Notes</b>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="navsection">
                                                <div class="navtitle" id="displayTopbottom" onclick="displayTopbottom()" title="<bean:message bundle="Tooltips" key="reportViewer.TopBottom"/>" >&nbsp;<b style="font-weight:bold">Top 5/ Bottom 5</b>
                                                    <span id="TopbottomImg" class='ui-icon ui-icon-circle-triangle-s'   style='float:right;'></span>
                                                </div>
                                            </div>
                                        </td>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="navsection">
                                                <div class="navtitle" id="displayBusRoles" onclick="displayBusRoles()">&nbsp;<b style="font-weight:bold">Business Roles</b>
                                        ...
                                        <td>
                                            <div class="navsection" style="background-color:#e6e6e6">
                                                <div class="navtitle" id="drillDownAssist" title="<bean:message bundle="Tooltips" key="reportViewer.DrillAssis"/>">&nbsp;
                                                    <b style="font-weight:bold">Drill Down Viewer</b></div>
                                                    <%
                                        ...
                                        <td>
                                            <div class="navsection" style="background-color:#e6e6e6">
                                                <div class="navtitle" id="advanceParamater" title="Advanced Parameter" onclick="advanceParamter('<%=PbReportId%>','<%=request.getContextPath()%>')">&nbsp;
                                                    <b style="font-weight:bold;">Advanced Parameter</b>
                                                </div>
                                            </div>
                                            </div>
                                        </td>
...
%>
                                        ...
                                        <td>
                                            <div class="navsection" style="background-color:#e6e6e6">
                                                <div class="navtitle" id="paramDrillDownAssist" title="<bean:message bundle="Tooltips" key="reportViewer.ParamDrill"/>">&nbsp;
                                                    <b style="font-weight:bold">Parameter Assistance</b></div>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="navsection">
                                                <div class="navtitle1" id="displayRepTimeInfo" onclick="displayRepTimeInfo('<%=PbReportId%>')" >&nbsp;<b style="font-weight:bold;">Report Time Info</b> </div>
...
<input height="100px" width="100px" type="hidden" id="perioddate" name="perioddate" onchange=""  onclick=""/><input height="100px" width="100px" type="hidden" id="gCompq" name="gCompq"  value="<%=timeinfo.get(4)%>"  onclick=""/>
...
<input height="100px" width="100px" type="hidden" id="compareto" name="compareto" onchange="" value="" onclick=""/>
 <input height="100px" width="100px" type="hidden" id="qtrdate" name="qtrdate" onchange="" value="<%=qtrtype%>" onclick=""/>
 <input height="100px" width="100px" type="hidden" id="newUIyear" name="newUIyear" onchange="" value="<%=newUIyear%>" onclick=""/>
 <input type="hidden" name="AOId" id="AOId" value="<%=aoid%>">
 <input type="hidden" name="dateenable" id="dateenable" value="<%=container.getdateenable()%>">
                                    ...
                                    <table style="width:99%">
                                        ...
                                        <tr id="topDisplayParams" valign="top" class="DisplayParams" style="display: ''">
                                            <td valign="top" width="100%">
                                                ...
                                                <div id="tableOption" class="navsectionDiv"  style="height: auto; width: 150px; position: absolute; margin-top: 5px;">
                                                    <table width="100%"><tr>
                                                            ...
                                                            <td>
                                                                ...
                                                                <i class="fa fa-reply" style='cursor:pointer;margin:8px 0px 0px 8px;color:#4d4d4d;font-size: 1.2em' onclick="showRepPrevState1('<%=PbReportId%>')" title="<%=TranslaterHelper.getTranslatedInLocale("Drill_Up", cL)%>"></i>
                                                                <i class="fa fa-clock-o" style='cursor:pointer;margin:8px 0px 0px 8px;color:#4d4d4d;font-size: 1.2em' onclick="openScheduleReport()" title="<%=TranslaterHelper.getTranslatedInLocale("Scheduler", cL)%>"></i>
                                                                <i class="fa fa-envelope-o" style='cursor:pointer;margin:8px 0px 0px 8px;color:#4d4d4d;font-size: 1.2em' onclick="openShareReportDiv()" title="<%=TranslaterHelper.getTranslatedInLocale("Send_Mail", cL)%>"></i>
                                                                <i class="fa fa-download" style='cursor:pointer;margin:8px 0px 0px 8px;color:#4d4d4d;font-size: 1.2em' onclick="parent.showExports('<%= PbReportId%>','<%=request.getContextPath()%>')" title="<%=TranslaterHelper.getTranslatedInLocale("Downloads_Report", cL)%>"></i>
                                                                <i class="fa fa-filter" style='cursor:pointer;margin:8px 0px 0px 8px;color:#4d4d4d;font-size: 1.2em' onclick="tabparamListDispTop()"  title="<%=TranslaterHelper.getTranslatedInLocale("Click_Here_For_Filters", cL)%>"></i>
                                                                <i class="fa fa-list-ul" style='cursor:pointer;margin:8px 0px 0px 8px;color:#4d4d4d;font-size: 1.2em' onclick="moreListDispTop()"  title="<%=TranslaterHelper.getTranslatedInLocale("Click_Here_For_More_Options", cL)%>"></i>


                                                                <% if (isGraphThere != null && !isGraphThere.equalsIgnoreCase("Yes")) {%>
                                                                <%}%>

                                                                <div id="datalist"  class="hideAllDiv"  onmouseleave='hideme(id)' style="display:none;position:absolute;height:5%;float: right;">

                                                                    <!--Edited by Faiz Ansari-->
                                                                        <table style="background-color:#fff;box-shadow: 0px 0px 2px 2px #d2d2d2;margin-top: 5px;border-radius:5px">
                                                                        <tr>
                                                                            <td>  <a id="whatifSc" class="gFontFamily gFontSize12" href="javascript:openWhatIfDiloge('<%=request.getContextPath()%>','<%=container.getReportId()%>')"  title="<%=TranslaterHelper.getTranslatedInLocale("Perform_What_If", cL)%>" ><%=TranslaterHelper.getTranslatedInLocale("What_If_Analysis", cL)%></a></td>
                                                                        </tr>
                                                                        <tr>
                                                                            <td> <a class="gFontFamily gFontSize12" href="javascript:openGroupMeasureList('<%=request.getContextPath()%>','<%=container.getReportId()%>')"  title="<%=TranslaterHelper.getTranslatedInLocale("Group_Measures", cL)%>"><%=TranslaterHelper.getTranslatedInLocale("Group_Measures", cL)%></a></td>
                                                                        </tr>
                                                                        <tr>
                                                                            <td><a class="gFontFamily gFontSize12" href="javascript:factFormulaDiv()"><%=TranslaterHelper.getTranslatedInLocale("View_Fact_Formula", cL)%></a></td>
                                                                        </tr>
                                                                        <tr>
                                                                            <td><a class="gFontFamily gFontSize12" onclick="paramListDisp('<%=PbReportId%>')" href="javascript:void(0)"><%=TranslaterHelper.getTranslatedInLocale("View_Favorite_Link", cL)%></a></td>
                                                                        </tr>
                                                                        <tr>
                                                                            <td><a class="gFontFamily gFontSize12" onclick="callReportLevelGraph('<%=PbReportId%>')" href="javascript:void(0)"><%=TranslaterHelper.getTranslatedInLocale("Graph_On_Report", cL)%></a></td>
                                                                        </tr>
                                                                        <tr>
                                                                            <td><a class="gFontFamily gFontSize12" onclick="importExcelFile1('<%=request.getContextPath()%>','<%=container.getReportId()%>')" href="javascript:void(0)"><%=TranslaterHelper.getTranslatedInLocale("Upload_Template_Excel", cL)%></a></td>

                                                                        </tr>
                                                                        <tr>
                                                                            <td id="showsql" style="width:100%;">
                                                                                <a id="showSql" href="javascript:showSqlQuery('<%=sqlString%>')" title="<%=TranslaterHelper.getTranslatedInLocale("Click_to_See_SQL_Query", cL)%>" ><%=TranslaterHelper.getTranslatedInLocale("Display_SQL_Query", cL)%></a>
                                                                            </td></tr>
                                                                        <tr>                                                                    

                                                                        </tr>
                                                                        <tr>
                                                                            <td><a class="gFontFamily gFontSize12" onclick="hideMeasures('<%=request.getContextPath()%>','<%=PbReportId%>')" href="javascript:void(0)"><%=TranslaterHelper.getTranslatedInLocale("Show_Hide_Measure", cL)%></a></td>

                                                                        </tr>

                                                            <tr><tr>
                                                                              <%if(container.report_Headrvalue != null && (container.report_Headrvalue).equalsIgnoreCase("hideheader")){%>
										 <td><a class="gFontFamily gFontSize12" onclick="saveShowHeader('showheader','<%=PbReportId%>')" ><%=TranslaterHelper.getTranslatedInLocale("Show_Report_Header", cL)%></a></td>

                                                                              <%}else{%>
                                                                              <td><a class="gFontFamily gFontSize12" onclick="saveShowHeader('hideheader','<%=PbReportId%>')" ><%=TranslaterHelper.getTranslatedInLocale("Hide_Report_Header", cL)%></a></td>
                                                                              <%}%>
                                                                        </tr>

                                                                        <tr>
                                                                            <td><a class="gFontFamily gFontSize12" onclick="reportBugMail23('<%=request.getContextPath()%>','<%=container.getReportId()%>','<%=container.getReportId()%>')" href="javascript:void(0)"><%=TranslaterHelper.getTranslatedInLocale("Report_Bug", cL)%></a></td>

                                                                        </tr>
                                                                        <tr>
                                                                            <td><a onclick="reportmulticalendar('<%=request.getContextPath()%>','<%=container.getReportId()%>')" href="javascript:void(0)"><font style="white-space: nowrap" size="2" ><%=TranslaterHelper.getTranslatedInLocale("Report_Multi_Calendar", cL)%> </font></a></td>
...
<!--                                                                            End!!!-->
                                ...
                                <!--</form>-->
                                        ...
                                        <tr valign="top" class="ParamRegion">
                                            <td valign="top" width="100%">
                                                <div class="navsection"  style="height:auto;width:100%;display:none">
                                             ...
                                             if (fromCmpYear.equalsIgnoreCase(Integer.toString(year))) {%>
                                                                                            ...
                                                                                            <td style="">To Year:&nbsp;<select id="calCmpToYear" name="calCmpYear" style="width:90px" onchange="saveYearDetails('compareto')">
                                                                                                    <%
                                                                                                        ...
                                                                                                        System.out.println("toCmpYear:" + toCmpYear);
                                                                                                        for (int year = 2005; year <= 2020; year++) {
                                                                                                            int y = year + 1;
                                                                                                            String years = Integer.toString(y);
                                                                                                            String yearSec = years.substring(2);
               ...
               if (userdao.getFeatureEnable("Hide Date Section") || userType.equalsIgnoreCase("SUPERADMIN")) {%>
                                               ...
                                               <div id="tabParametersTopParent" style="position: fixed;z-index: 1000; display: none;">
                                                <div id="tabParametersTop" title="show_param" style="background-color:white;width:0px;height:0px; position:absolute;text-align:left;border: 1px solid LightGrey;top:122px;z-index: 1000;">
                                                    <%=ParamSectionDisplay%>
                                                </div>
                                               </div>
                                                <% if (isGraphThere != null && !isGraphThere.equalsIgnoreCase("Yes")) {%>
                                            ...
                                            <td>
                                                ...
                                                <div id="tabGraphs1" style="background-color:#fff;display:none;">
                                                    <table  width="100%" style="height:auto;border:0px solid #369;">
                                                        <tr style="width:100%">
                                                            <td valign="top" width="100%">
                                                                <IFRAME NAME='iframe5' ID='iframe5'   STYLE='width:100%;height:500px' frameborder="0"></IFRAME>
                                                            </td>
                                                        </tr>
                                                    </table>
                                                </div>
                                                <div id="tabGraphs" style="background-color:#fff;display:none;">
                                                    <table  width="100%" style="height:auto;border:0px solid #369;">
                                                        <tr style="width:100%">
                                                            <td valign="top" width="100%">
                                                                <IFRAME NAME='iframe5' ID='iframe4'   STYLE='width:100%;height:400px' frameborder="0"></IFRAME>
                                                            </td>
                                                        </tr>
                                                    </table>
                                                </div>
                                        ...
                                        <tr valign="top">
                                            <td valign="top" width="100%">
                                                <div class="navsection" >
                                                    ...
                                                    <div id="tabGraphs" style="background-color:#fff;display:<%=String.valueOf(session.getAttribute("GraphStatus"))%>;display:none;">
                                                        <table  width="100%" style="height:auto;border:0px solid #369;">
                                                            <tr style="width:100%">
                                                                <td valign="top" width="100%">
                                                                    <IFRAME NAME='iframe4' ID='iframe4'   STYLE='width:100%;overflow:auto;height:467px' frameborder="0"></IFRAME>
                                                ...
                                                <div class="navsection" >
                                                    <div style="width:100%" onclick="displayMap()" title="<bean:message bundle="Tooltips" key="reportViewer.DispMap"/>">
                                                        <table width="100%" class="navtitle1">
                                                            <tr>
                                                                <td><b style="font-weight:bold">Map Region</b></td>
                                        ...
                                        <tr valign="top">
                                            <td valign="top" width="100%">
                                                <div id="tableDivPro" class="navsection" >
     ...
     <% if(defaulttab!=null && (defaulttab.equalsIgnoreCase("Report") || defaulttab.equalsIgnoreCase("TABLE") || defaulttab.equalsIgnoreCase(""))) {    %>
        ...
        <iframe id="Scheduler" height="380px" width="660px"  style="display:none"></iframe>
        <iframe id="cstLinksFrame" height="380px" width="660px" src="getAllReports.do" style="display:none"></iframe>
        <iframe id="prtLinksFrame" height="380px" width="660px" style="display:none"></iframe>
        ...
        <div id="replyMessageDialog" title="" style="display:none">
            <iframe id="replyMessageFrame" width="100%" height="100%" frameborder="0" ></iframe>
        </div>
        ...
        <div id="saveNewReport" style="display:none" title="<%=TranslaterHelper.getTranslatedInLocale("Save_Report", cL)%>">
            <table border="0" style="width:100%;">
                <tr valign="top">
                    <td valign="top"   style="width:40%;">
                        <label class="gFontFamily gFontSize12 label"><%=TranslaterHelper.getTranslatedInLocale("Report_Name", cL)%></label>
                    </td>
                ...
                <tr valign="top">
                    <td valign="top"  style="width:40%;"><label class="gFontFamily gFontSize12 label"><%=TranslaterHelper.getTranslatedInLocale("Description", cL)%></label> </td>
                    <td   valign="top" style="width:60%;">
                ...
                <tr valign="top">
                    <td valign="top"  style="width:40%;"> <label class="gFontFamily gFontSize12 label"><%=TranslaterHelper.getTranslatedInLocale("With_Graph", cL)%></label> </td>
                ...
                <tr><td  valign="top" colspan="2" align="center">
                        <%if (isGraphThere.equalsIgnoreCase("Yes")) {%>
                        <input type="button"  class="gFontFamily gFontSize12 navtitle-hover" style="width:auto" value="<%=TranslaterHelper.getTranslatedInLocale("next", cL)%>" id="save1" onclick="saveNewRepGrpTab('<%=PbReportId%>','<%=request.getContextPath()%>');">
                        <%} else {%>
                        <input type="button"  class="gFontFamily gFontSize12 navtitle-hover" style="width:auto" value="<%=TranslaterHelper.getTranslatedInLocale("next", cL)%>" id="save1" onclick="saveAsNewReport('<%=PbReportId%>','<%=request.getContextPath()%>','<%=reportName%>');">
                        <%}%>
                    </td>
                </tr>
            </table>
        ...
        <div id="DefinecustSeqDialog" title="Define Custom Sequence" style="display:none">
            <form id="custseqForm">
                ...
                <!--By Ram-->
                <div id="editMeasureAsViewBysDiv" title="<%=TranslaterHelper.getTranslatedInLocale("Edit_Measure_As_ViewBy", cL)%>" style="display:none">
             <table><tr><td>
                        <input id="tableListFacts" type="checkbox" onclick="getDisplayTablesFacts('<%=request.getContextPath()%>','<%=params%>')"><%=TranslaterHelper.getTranslatedInLocale("All", cL)%></td>
                    <td id="tabListDivFacts" ><input type="textbox" id="tabsListValsFacts"><input type="textbox" style="display:none;" id="tabsListIdsFacts">
                        <div id="paramValsFacts" class="ajaxboxstyle" style="display:none;overflow: auto;"></div></td>
                    <td id="tablistLinkFacts" ><a href="javascript:void(0)" class="ui-icon ui-icon-note" onclick="showFactsList('<%=request.getContextPath()%>','<%=params%>')" ></a></td>
                    <td id="goButton" onclick="setFactsValueToContainer('<%=request.getContextPath()%>','<%= PbReportId%>','<%=rolesid%>')"><input type="button" value="<%=TranslaterHelper.getTranslatedInLocale("go", cL)%>" class="navtitle-hover"></td>
                </tr></table>
            <iframe  id="editMeasureAsViewBysFrame" name='editMeasureAsViewBysFrame' width="100%" height="100%" frameborder="0"   src='Report/Viewer/ChangeMeasuresAsViewbys.jsp'></iframe>
        </div>
        <!--added by shobhit for multi dashboard on 22/09/15-->        
        <div id="defineParentChildDB" title="<%=TranslaterHelper.getTranslatedInLocale("Define_Multi_Dashboard", cL)%>" style="display:none">
            <iframe  id="defineParentChildDBFrame" name='editViewFrame' width="100%" height="100%" frameborder="0"   ></iframe>
        </div>
        <div id="filterViewByDiv" title="Quickfilters" style="display:none">
            <iframe  id="filterViewByFrame" name='filterViewFrame' width="100%" height="100%" frameborder="0"  ></iframe>
        </div>
        <div id="editViewByDivgbl" title="Resequnce params" style="display:none">
            <iframe  id="editViewByFramegbl" name='editViewFramegbl' width="100%" height="100%" frameborder="0"  ></iframe>
        </div>
        <div id="saveRTD" title="<%=TranslaterHelper.getTranslatedInLocale("Tag_Report", cL)%>" style="display:none">
            <iframe  id="saveReportByFrame" name='saveViewFrame' width="100%" height="100%" frameborder="0"   ></iframe>
        </div>
        <div id="tableColsDialog" title="<%=TranslaterHelper.getTranslatedInLocale("Add_Measures", cL)%>" style="display:none" >
           <% if((container.getReportCollect().AOId!=null && !container.getReportCollect().AOId.equalsIgnoreCase("")) )
            ...
            <table style="display:none;">
                ...
                <tr><td classs="gFontFamily gFontSize12">
                        <input id="tableList"  type="checkbox" onclick="getDisplayTables('<%=request.getContextPath()%>','<%=params%>')"><%=TranslaterHelper.getTranslatedInLocale("All", cL)%></td>
                    ...
                    <td id="goButton" onclick="setValueToContainer('<%=request.getContextPath()%>','<%= PbReportId%>','<%=rolesid%>')"><input type="button" value="<%=TranslaterHelper.getTranslatedInLocale("go", cL)%>" class="navtitle-hover"></td>
                </tr></table>
        ...
        <div id="customReportDrill" title="Edit Report Customize Drill Down" style="display:none">
            <iframe  id="customReportDrillFrame" NAME='customReportDrillFrame' width="100%" height="100%" frameborder="0"   ></iframe>
        </div>
        <div id="fadestart" class="black_start"></div>
        <div id="applycolrdiv" title="Apply Color Based Grouping" style="display:none">
            <iframe id="applycolorframe" name="applycolorframe" frameborder="0" marginheight="0" marginwidth="0" width="100%" height="100%"></iframe>
        </div>
        <div id="dispGrpProp" title="Graph Properties" style="display:none">
            <iframe id="dispGrpPropFrame" NAME='dispGrpPropFrame' width="100%" height="100%"  frameborder="0" ></iframe>
        </div>
        <div id="zoomgraph" title="Zoom Graph" style="display:none">
            <iframe id="zoomgraphFrame" NAME='zoomgraphFrame' width="100%" height="100%"  frameborder="0" ></iframe>
        </div>
<!--        added by sruthi for globalcolumn pro-->
        <div id="dispTabColumnProp" title="<%=TranslaterHelper.getTranslatedInLocale("Table_Column_Properties", cL)%>" style="display:none">
            <iframe id="dispTabColumnPropFrame" NAME='dispTabColumnPropFrame' width="100%" height="100%"  frameborder="0" ></iframe>
        </div>
            <div  id="advanceFilters" title="Advance Filters" style="display:none">
            <iframe id="advanceiframeFilters" name="advanceiframeFilters" frameborder="0" marginheight="0" marginwidth="0"  width="100%" height="100%"></iframe>
        </div>
<!--eneded by sruthi-->
       <div id="ShowFilters" title="<%=TranslaterHelper.getTranslatedInLocale("ShowFilters", cL)%>" style="display:none">
            <iframe id="ShowFiltersFrame" NAME='ShowFiltersFrame' width="100%" height="100%"  frameborder="0" SRC='about:blank'></iframe>
        </div>

        <div id="dispTabProp" title="<%=TranslaterHelper.getTranslatedInLocale("Table_Properties", cL)%>" style="display:none">
            <iframe id="dispTabPropFrame" NAME='dispTabPropFrame' width="100%" height="100%"  frameborder="0" ></iframe>
        </div>
        ...
        <div id="dispgrpAnalysis" title="Dimension Segmentation" style="display:none">
            <iframe id="dispgrpAnalysisFrame" NAME='dispgrpAnalysisFrame' width="100%" height="100%"  frameborder="0" ></iframe>
        </div><div id="dispbucketAnalysis" title="Bucket Analysis" style="display:none">
            <iframe id="dispbucketAnalysisFrame" NAME='dispbucketAnalysisFrame' width="100%" height="100%"  frameborder="0" ></iframe>
        </div>

        <div style="display:none" id="custmemDispDia" title="<%=TranslaterHelper.getTranslatedInLocale("Custom_Measures", cL)%>">
        ...
        <div id="advanceFormulaDiv" title="Advance Formula" style="width:515px;height:200px;overflow-y:auto;overflow-x:auto;display: none">
            <iframe id="advanceFormulaFrame" NAME='advanceFormulaFrame' width="100%" height="100%" frameborder="0" ></iframe>
        </div>
        ...
        <div style="display:none" id="custmemMeasureDispDia" title="Custom Measures">
            <iframe  id="custmemMeasDisp"  height="100%" width="100%" frameborder="0" ></iframe>
        </div>
        <div id="TimeBasedDiaolgDisplay" name="TimeBasedDiaolgDisplay" title="" style="z-index: 1050">
            <iframe id="TimeBasedDisplay" name="TimeBasedDisplay" height="100%" width="100%" frameborder="0" > </iframe>
        </div>
        <div id="showSqlStrDialog" title="SQL Query" style="display:none">
            <iframe  id="sqlQueryStr" NAME='sqlQueryStr' width="100%" height="100%" frameborder="0"   ></iframe>
        </div>
        <!-- added by sruthi to display fullquery       -->
        <div id="showSqlStrDialogfull" title="SQL Query" style="display:none">
            <iframe  id="sqlQueryStrfull" NAME='sqlQueryStr1' width="100%" height="100%" frameborder="0"  ></iframe>
        </div>
        <!--ended by sruthi-->
        <div id="performWhatIfDiv" style="display: none" title="<%=TranslaterHelper.getTranslatedInLocale("Perform_What_If", cL)%>">
            <iframe  frameborder="0" id="performWhatIfFrame" name="performWhatIfFrame" style="width:750px; height:490px; "  ></iframe>
        ...
        <div id="showExports" title="Exports" style="display:none">
            <iframe id="showExportsFrame" NAME='showExportsFrame' width="100%" height="100%"  frameborder="0" ></iframe>
        </div>
        <div id="DefineTarget" title="<%=TranslaterHelper.getTranslatedInLocale("Define_Target", cL)%>" style="display: none">
            <br/>
            <table width="100%" align="center">
                <tbody align="center">
                    <tr>
                        <td>
                            <%=TranslaterHelper.getTranslatedInLocale("Target_Measure_Name", cL)%>
                ...
                <tr  align="center" valign="top">
                            ...
                            <table  align="center" border="0" width="100%">
                                <tr  align="left" valign="top" style="color: black;" class="">
                                    <td style="font-weight:bold" align="center">
                                        <div class="navtitle-hover" style="height: 20px;"><font size="2" style="font-weight: bold;"> <%=TranslaterHelper.getTranslatedInLocale("Target_Measures", cL)%>  </font>
                                        </div></td>
                                </tr></table>
                    ...
                    <td  align="left" valign="top" width="50%">
                        <div style="height:100px; overflow:auto"  align="left" >
                            ...
                            <a href="javascript:clearTextArea()"><font color="blue"><%=TranslaterHelper.getTranslatedInLocale("Clear", cL)%> </font> </a>
                        </div>
                        <div >
                            <table>
                                <tr>
                ...
                <tbody align="center">
                    <tr>
                        <td width="50%" align="right">
                            <input type="button" onclick="saveWhatIfTargetMes('<%= PbReportId%>','<%= request.getContextPath()%>')" name="save" value="<%=TranslaterHelper.getTranslatedInLocale("save", cL)%>"  class="navtitle-hover" style="width: 30%">
                        </td>
                        <td width="50%" align="left">
                            <input type="button"  onclick="saveEditTargetMes('<%= PbReportId%>','<%= request.getContextPath()%>')" id="saveEditTarget" name="saveEditTarget" value="<%=TranslaterHelper.getTranslatedInLocale("done", cL)%>"  class="navtitle-hover" style="width: 30%;display: none" >
                        </td>
                    </tr></tbody>
            </table>
        ...
        <div  id="columnPropertiesdiv" title="Column Properties" style="display:none">
            <iframe id="columnPropertiesframe" name="columnPropertiesframe" frameborder="0" marginheight="0" marginwidth="0"  width="100%" height="100%"></iframe>
        </div>
        ...
        <div id="paramFilterDiv" title="Parameter Filter" style="display:none">
            <iframe  id="paramFilter" NAME='paramFilter' width="100%" height="100%" frameborder="0"   ></iframe>
        </div>
        <div id="paramFilterMemberDiv" title="Parameter Members" style="display:none">
            <iframe  id="paramFilterMember" NAME='paramFilterMember' width="100%" height="100%" frameborder="0"   ></iframe>
        </div>
        ...
        <div id="QuickTimeBasedFormulaDiv" title="Quick TimeBased Formula" style="width:515px;height:300px;overflow-y:auto;overflow-x:auto;display: none">
            <iframe id="QuickTimeBasedFormulaFrame" NAME='QuickTimeBasedFormulaFrame' width="100%" height="100%" frameborder="0" ></iframe>
        ...
        <div id="sensitivityDiv" style="display: none">
            <iframe id="sensitivityiFrame" name="sensitivityiFrame"width="100%" height="100%" frameborder="0"   ></iframe>
        </div>
        <div>
 <div id="demoContainer" class="containerPlus draggable { skin:'stiky', width:'800', closed:'true',rememberMe:true}" style="position:absolute;top:250px;left:400px; height:60%">
                <div id="innerContainer" ></div>
            </div>

            <div  id="applyAlarmdiv"  title="Set Reminder For StickyNote" style="display:none;">
                <iframe id="applyAlarmFrame" name="applyAlarmFrame" frameborder="0" scrolling="" marginheight="0" marginwidth="0"  width="100%" height="100%"></iframe>
            </div>
        </div>
        <div id="groupMeasureDiv" title="<%=TranslaterHelper.getTranslatedInLocale("group_measure", cL)%>" style="display:none;">
            <table align="center">
                <tr>
                    <td>
                        <%=TranslaterHelper.getTranslatedInLocale("Group_Name", cL)%>:
                    </td>
                    <td>
            ...
            <br/><br/><br/>
            <div align="center">
                <input type="button" name="done" value="<%=TranslaterHelper.getTranslatedInLocale("done", cL)%>" onclick="saveMeasureGroup('<%= PbReportId%>')" class="navtitle-hover" >
                <input type="button" name="edit" value="<%=TranslaterHelper.getTranslatedInLocale("Edit_Measure", cL)%>" onclick="editGrpMeasure()" class="navtitle-hover" >
            </div>
        </div>
        ...
        <div id="shareReport" title="<%=TranslaterHelper.getTranslatedInLocale("Share_Report", cL)%>" style="display: none">
            <table>
                <tr>
                    <td width="20%"><b><%=TranslaterHelper.getTranslatedInLocale("Format", cL)%></b> </td>
                    <td>
                ...
                <tr>
                    <td width="20%"><b><%=TranslaterHelper.getTranslatedInLocale("Users", cL)%> </b></td>
                    <td>
                ...
                <tr>
                    <td width="20%" id="share_subject_id" style="display:none"><b><%=TranslaterHelper.getTranslatedInLocale("Subject", cL)%></b> </td>
                    <td colspan="1" >
                ...
                <tr>
                    <td width="20%" id="comments_id" style="display: none" ><%=TranslaterHelper.getTranslatedInLocale("Comment", cL)%></td>
                    <td colspan="1"><textarea  id="comments" name="comments" cols="" rows="" style="width:250px;height:70px;display:none"></textarea></td>
                </tr>
                <tr>
                    <td width="20%" id="tomailtd" style="display:none"><b><%=TranslaterHelper.getTranslatedInLocale("Email_To", cL)%></b> </td>
                    <td colspan="1" >
                        ...
                        <textarea  id="userstextarea" name="userstextarea" cols="" rows=""  style="width:250px;height:80px;display:none"></textarea>
                                        ...
                                        <tr>
                                            <td colspan="2">
                                                <table border="0" style="width:100%">
                                                    <tr style='width:100%'>
                                                        <td style='width:40%'><%=TranslaterHelper.getTranslatedInLocale("Header_Logo", cL)%></td>
                                                        <td style='width:10%'><input type='checkbox' id="headerLogo" name="headerLogo"></td>
                                                        <td style='width:40%'><%=TranslaterHelper.getTranslatedInLocale("Footer_Logo", cL)%></td>
                                         ...
                                         <tr>
                                            <td colspan="2">
                                                <table border="0" style="width:100%">
                                                    <tr style='width:100%'>
                                                        <td style='width:40%'><%=TranslaterHelper.getTranslatedInLocale("Optional_Header", cL)%></td>
                                                        <td style='width:10%'><input type='checkbox' id="optionalHeader" name="optionalHeader"></td>
                                                        <td style='width:40%'><%=TranslaterHelper.getTranslatedInLocale("Optional_Footer", cL)%></td>
                                         ...
                                         <tr>
                                            <td colspan="2">
                                                <table border="0" style="width:100%">
                                                    <tr style='width:100%'>
                                                        <td style='width:40%'><%=TranslaterHelper.getTranslatedInLocale("Signature", cL)%></td>
                ...
                <tr>
                    <td colspan="2" align="center">
                        <input class="alert-button-hover" type="button" id="share_report_button" value="<%=TranslaterHelper.getTranslatedInLocale("Share_Report", cL)%>" onclick="SendReportEmail()">
                    </td>
                </tr>
                <tr> <td colspan="2">&nbsp; </td>  </tr>
                <tr> <td colspan="2"><span style="color:red;font-size:12px;"><sup>*</sup></span> <%=TranslaterHelper.getTranslatedInLocale("Please_separate_multiple_Email_Id_s_by_comma", cL)%>(,). </td>  </tr>
            </table>
        </div>
        ...
        <div id="drillMeasuresId" style="display: none" title="Enable Measure Drill">
            <iframe  id="drillmeasFrame" NAME='drillmeasFrame'  width="100%" height="100%" frameborder="0" ></iframe>
        </div>
        <div id="relatedMeasuresId" style="display: none" title="Related Measure ">
        </div>
        <div id="designerGraphList" style="display:none;"></div>

        <div id="overWriteReport" style="display:none" title="<%=TranslaterHelper.getTranslatedInLocale("OverWriteReport", cL)%>">
            <table>
                <tr><td colspan="2"><font size="2" style="font-weight: bold;"><%=TranslaterHelper.getTranslatedInLocale("Do_you_want_to_over_write_the_report", cL)%></font></td></tr>
                <% if (!alist.get(1).toString().equalsIgnoreCase("PRG_DATE_RANGE")) {%>
                <tr><td><input type="radio" name="Date" id="sysDate" value="sysDate"><%=TranslaterHelper.getTranslatedInLocale("Global_Date", cL)%></td>
                    <td>&nbsp;&nbsp;&nbsp;<input type="radio" name="Date" id="reportDate" value="reportDate" ><%=TranslaterHelper.getTranslatedInLocale("Report_Date", cL)%></td>
                    <td>&nbsp;&nbsp;&nbsp;<input type="radio" name="Date" id="currdetails" value="currdetails" checked><%=TranslaterHelper.getTranslatedInLocale("Current_Details", cL)%></td>
                    <td>&nbsp;&nbsp;&nbsp;<input type="checkbox" name="autometicDate" id="autometicDate" <%=autoDate%> onclick="" value="autometicDate"><%=TranslaterHelper.getTranslatedInLocale("Automatic_Date", cL)%></td></tr>
                <tr><td><br></td></tr>
                <tr><td><input type="radio" name="Date" id="yestrday" value="yestrday"><%=TranslaterHelper.getTranslatedInLocale("Yesterday", cL)%></td><td>&nbsp;&nbsp;&nbsp;<input type="radio" name="Date" id="tomorow" value="tomorow"><%=TranslaterHelper.getTranslatedInLocale("Tomorrow", cL)%></td>
                    <td>&nbsp;&nbsp;&nbsp;<input type="checkbox" name="Cache" id="cacheAO"  onclick="" value="cacheAO"><%=TranslaterHelper.getTranslatedInLocale("Cache_Analytical_Object", cL)%></td></tr>
                <tr><td><br></td></tr>
                <tr><td><input type="radio" name="Date" id="fixeddate" value="fixeddate" ><%=TranslaterHelper.getTranslatedInLocale("Fixed_Date", cL)%></td><td><input id="fdatepicker" type="text" value="" style="width: 120px;" maxlength="100" name="fdate" readonly=""></td>
                </tr>
                ...
                <tr><td><input type="radio" name="Date" id="globalDate" value="globalDate" ><%=TranslaterHelper.getTranslatedInLocale("Global_Date", cL)%></td><td><select id="globalSign" name="globalSign">
                        ...
                        <%} else {%>
                <tr><td><input type="radio" name="Date" id="reportDate" onclick="getCustomDate()" value="reportDate" checked><%=TranslaterHelper.getTranslatedInLocale("Report_Date", cL)%></td><td>&nbsp;&nbsp;&nbsp;<input type="radio" name="Date" id="customDate" onclick="getCustomDate()" value="customDate"><%=TranslaterHelper.getTranslatedInLocale("Custom_Date", cL)%></td>
                    <td>&nbsp;&nbsp;&nbsp;<input type="radio" name="Date" id="currdetails" onclick="getCustomDate()" value="currdetails"><%=TranslaterHelper.getTranslatedInLocale("Current_Details", cL)%></td>
                    <td>&nbsp;&nbsp;&nbsp;<input type="checkbox" name="autometicDate" id="autometicDate" <%=autoDate%> onclick="" value="autometicDate"><%=TranslaterHelper.getTranslatedInLocale("Automatic_Date", cL)%></td>
                    <td>&nbsp;&nbsp;&nbsp;<input type="checkbox" name="Cache" id="cacheAO"  onclick="" value="cacheAO"><%=TranslaterHelper.getTranslatedInLocale("Cache_Analytical_Object", cL)%></td></tr>
                <table id="dateRangeTab" style="display: none;">
                    <tr><td style="font-weight:bold;"><%=TranslaterHelper.getTranslatedInLocale("For_From_Date", cL)%></td></tr>
                    <tr><td><input type="radio" name="FromDate" id="fromfixeddate" value="fromfixeddate"><%=TranslaterHelper.getTranslatedInLocale("Fixed_Date", cL)%></td><td><input id="ffdatepicker" type="text" value="" style="width: 120px;" maxlength="100" name="ffdate" readonly=""></td>
                    </tr>
                    <tr><td><input type="radio" name="FromDate" id="fromyestrday" value="fromyestrday" checked><%=TranslaterHelper.getTranslatedInLocale("Yesterday", cL)%></td><td>&nbsp;<input type="radio" name="FromDate" id="fromtomorow" value="fromtomorow"><%=TranslaterHelper.getTranslatedInLocale("Tomorrow", cL)%></td></tr>
                    <tr><td><input type="radio" name="FromDate" id="fromSysDate" value="fromSysDate"><%=TranslaterHelper.getTranslatedInLocale("System_Date", cL)%></td><td><select id="fromSysSign" name="sign">
                                <option value="+">+</option>
                                <option value="-">-</option>
                            </select></td><td><input id="fromSysVal" type="text"><%=TranslaterHelper.getTranslatedInLocale("Days", cL)%> </td></tr>
                    <tr><td><input type="radio" name="FromDate" id="fromglobalDate" value="fromToglobalDate" ><%=TranslaterHelper.getTranslatedInLocale("Global_Date", cL)%></td><td><select id="fromglobalSign" name="globalSign">
                                <option value="+">+</option>
                                <option value="-">-</option>
                            </select></td><td><input id="fromGlobVal" type="text"><%=TranslaterHelper.getTranslatedInLocale("Days", cL)%> </td></tr>
                    <tr><td style="font-weight:bold;"><%=TranslaterHelper.getTranslatedInLocale("For_To_Date", cL)%></td></tr>
                    <tr><td><input type="radio" name="ToDate" id="tofixeddate" value="tofixeddate"><%=TranslaterHelper.getTranslatedInLocale("Fixed_Date", cL)%></td><td><input id="ftdatepicker" type="text" value="" style="width: 120px;" maxlength="100" name="ftdate" readonly=""></td>
                    </tr>
                    <tr><td><input type="radio" name="ToDate" id="toyestrday" value="toyestrday"><%=TranslaterHelper.getTranslatedInLocale("Yesterday", cL)%></td><td>&nbsp;<input type="radio" name="ToDate" id="totomorow" value="totomorow" checked><%=TranslaterHelper.getTranslatedInLocale("Tomorrow", cL)%></td></tr>
                    <tr><td><input type="radio" name="ToDate" id="toSystDate" value="toSystDate"><%=TranslaterHelper.getTranslatedInLocale("System_Date", cL)%></td><td><select id="toSysSign" name="sign">
                                <option value="+">+</option>
                                <option value="-">-</option>
                            </select></td><td><input id="toSysVal" type="text"><%=TranslaterHelper.getTranslatedInLocale("Days", cL)%> </td></tr>
                    <tr><td><input type="radio" name="ToDate" id="toglobalDdate" value="toglobalDdate" ><%=TranslaterHelper.getTranslatedInLocale("Global_Date", cL)%></td><td><select id="toglobalSign" name="globalSign">
                                <option value="+">+</option>
                                <option value="-">-</option>
                            </select></td><td><input id="toGlobVal" type="text"><%=TranslaterHelper.getTranslatedInLocale("Days", cL)%> </td></tr>
                    <tr><td style="font-weight:bold;"><%=TranslaterHelper.getTranslatedInLocale("For_Compare_From_Date", cL)%></td></tr>
                    <tr><td><input type="radio" name="CmpFrmDate" id="CmpFrmfixeddate" value="CmpFrmfixeddate"><%=TranslaterHelper.getTranslatedInLocale("Fixed_Date", cL)%></td><td><input id="fcfdatepicker" type="text" value="" style="width: 120px;" maxlength="100" name=fcfdate" readonly=""></td>
                    </tr>
                    <tr><td><input type="radio" name="CmpFrmDate" id="CmpFrmyestrday" value="CmpFrmyestrday" checked><%=TranslaterHelper.getTranslatedInLocale("Yesterday", cL)%></td><td>&nbsp;<input type="radio" name="CmpFrmDate" id="CmpFrmtomorow" value="CmpFrmtomorow"><%=TranslaterHelper.getTranslatedInLocale("Tomorrow", cL)%></td></tr>
                    <tr><td><input type="radio" name="CmpFrmDate" id="CmpFrmSysDate" value="CmpFrmSysDate"><%=TranslaterHelper.getTranslatedInLocale("System_Date", cL)%></td><td><select id="CmpFrmSysSign" name="sign">
                                <option value="+">+</option>
                                <option value="-">-</option>
                            </select></td><td><input id="CmpFrmSysVal" type="text"><%=TranslaterHelper.getTranslatedInLocale("Days", cL)%> </td></tr>
                    <tr><td><input type="radio" name="CmpFrmDate" id="CmpFrmglobalDate" value="fromToglobalDate" ><%=TranslaterHelper.getTranslatedInLocale("Global_Date", cL)%></td><td><select id="CmpFrmglobalSign" name="globalSign">
                                <option value="+">+</option>
                                <option value="-">-</option>
                            </select></td><td><input id="CmpFrmGlobVal" type="text"><%=TranslaterHelper.getTranslatedInLocale("Days", cL)%> </td></tr>
                    <tr><td style="font-weight:bold;"><%=TranslaterHelper.getTranslatedInLocale("For_Compare_To_Date", cL)%></td></tr>
                    <tr><td><input type="radio" name="CmpToDate" id="Cmptofixeddate" value="Cmptofixeddate"><%=TranslaterHelper.getTranslatedInLocale("Fixed_Date", cL)%></td><td><input id="fctdatepicker" type="text" value="" style="width: 120px;" maxlength="100" name="fctdate" readonly=""></td>
                    </tr>
                    <tr><td><input type="radio" name="CmpToDate" id="cmptoyestrday" value="cmptoyestrday"><%=TranslaterHelper.getTranslatedInLocale("Yesterday", cL)%></td><td>&nbsp;<input type="radio" name="CmpToDate" id="cmptotomorow" value="cmptotomorow" checked><%=TranslaterHelper.getTranslatedInLocale("Tomorrow", cL)%></td></tr>
                    <tr><td><input type="radio" name="CmpToDate" id="cmptoSysDate" value="cmptoSysDate"><%=TranslaterHelper.getTranslatedInLocale("System_Date", cL)%></td><td><select id="cmptoSysSign" name="sign">
                                <option value="+">+</option>
                                <option value="-">-</option>
                            </select></td><td><input id="cmptoSysVal" type="text"><%=TranslaterHelper.getTranslatedInLocale("Days", cL)%> </td></tr>
                    <tr><td><input type="radio" name="CmpToDate" id="cmptoglobalDate" value="fromToglobalDate" ><%=TranslaterHelper.getTranslatedInLocale("Global_Date", cL)%></td><td><select id="cmptoglobalSign" name="globalSign">
                                <option value="+">+</option>
                                <option value="-">-</option>
                            </select></td><td><input id="cmptoGlobVal" type="text"><%=TranslaterHelper.getTranslatedInLocale("Days", cL)%> </td></tr>
                </table>
                <%}%>
                <tr><td><br></td></tr>
                <tr><td><br></td></tr>
                <tr><td colspan="2" align="center"><input type="button" value="<%=TranslaterHelper.getTranslatedInLocale("Ok", cL)%>" class="navtitle-hover" style="width:40px;height:25px;color:black" onclick="OverWriteReport('<%=request.getContextPath()%>','<%=PbReportId%>',istoggledate,incriment)"/>&nbsp;&nbsp;&nbsp;
                        <input type="button" value="<%=TranslaterHelper.getTranslatedInLocale("cancel", cL)%>" class="navtitle-hover" style="width:50px;height:25px;color:black" onclick="cancelOverWriteReport()"/></td></tr>
            </table>
        </div>
        ...
        <div id="allParametersTab" style="display:none;width:22%;height:140%;background-color:white; direction: ltr; float: right; position:absolute;text-align:left;border:1px solid #000000;left:0px;top:122px;z-index: 10000;overflow: auto;border-color: LightGrey;">
            <table>
                ...
                <tr><td><td style="padding-right:10px"><a href="javascript:void(0)"class='ui-icon ui-icon-star' title="Favourite Links" onclick="remainingRegions('<%=PbReportId%>')"></a></td>
                    ...
                    <% }
                         if (userdao.getFeatureEnable("Top/Bottom") || userType.equalsIgnoreCase("SUPERADMIN")) {%>
                    <td><td style="padding-right:10px"><a href="javascript:void(0)"class='ui-icon ui-icon-mail-open' title="<%=TranslaterHelper.getTranslatedInLocale("Top5_Bottom5", cL)%>" onclick="getdisplayTopBottom()"></a></td>
                    <% }
                          if (userdao.getFeatureEnable("Report Info") || userType.equalsIgnoreCase("SUPERADMIN")) {%>
                    <td><td style="padding-right:10px"><a href="javascript:void(0)"class='ui-icon ui-icon-lightbulb' title="<%=TranslaterHelper.getTranslatedInLocale("Information_about_Report", cL)%>" onclick="getdisplayBusRoles()"></a></td>
                    <% }%>
            ...
            <table width="80%"><tr><td  style="padding-top:6px"></td></tr>
                <tr><td>
                        ...
                        <div id="remainingRegions" style="width:100%;display:none;">
                            <table width="100%">
                                <tr><td ><u><span style="font-size:12px;font-family: helvetica;font-weight:bold;"><%=TranslaterHelper.getTranslatedInLocale("Favorite_Link", cL)%></span></u></td></tr>
        ...
        <div id="ScheduleReport" style="display:none" title="<%=TranslaterHelper.getTranslatedInLocale("Schedule_Report", cL)%>" style="height:360px;">
            <form action="" name="dailyReportScheduleForm" id="dailyReportScheduleForm" method="post">
                <table>
                    ...
                    <tr id="formatId">
                        <td class="myalertheader"><%=TranslaterHelper.getTranslatedInLocale("Format", cL)%></td>
...
<!--edited by shivam-->
                    ...
                    <% if (!alist.get(1).toString().equalsIgnoreCase("PRG_DATE_RANGE")) {%>
                    <tr>
                        <td class="myalertheader"><%=TranslaterHelper.getTranslatedInLocale("Period", cL)%></td>
                        <td class="pdg-left-2">
                 ...
                 <tr>
                    ...
                    <tr><td colspan="2">&nbsp; </td></tr>
                    <tr>
                        <td  id="saveScheduleReport" align="center"><input id="saveScheduleReport" class="alert-button-hover" type="button" onclick="SendScheduleReport()" value="<%=TranslaterHelper.getTranslatedInLocale("Save_Schedule", cL)%>">
                        <td id="updateScheduleReport" align="center"><input class="navtitle-hover" type="button" onclick="updateScheduleReport()" value="<%=TranslaterHelper.getTranslatedInLocale("Update_Schedule", cL)%>">
                    </tr>
                    <tr><td colspan="2">&nbsp; </td></tr>
        ...
        <div id="viewReportSchedule" style="display:none" title="viewSchedules">
            <iframe id="viewScheduleFrame" width="100%" height="100%" frameborder="0" ></iframe>
        </div>
        ...
        <div id="assignheadlineReps" title="Assing Headline Reports" style="display:none">
            <iframe  id="userAssignHeadline" NAME='userAssignHeadline' height="100%" width="100%"  frameborder="0" ></iframe>
        </div>
        ...
        <div id="viewFactformulaDiv" title="<%=TranslaterHelper.getTranslatedInLocale("View_Fact_Formula", cL)%>" style="display: none"></div>
        <div id="viewFilterEdit" name="viewFilterEdit" title="Parameter View/Filter Edit" style="display: none">
            <iframe id="viewFilterEditFrame" NAME='viewFilterEditFrame' width="100%" height="100%" frameborder="0" ></iframe>
        </div>
        <div id="AddMoreParamsDiv" title="Add More Dimension ">
            <iframe  id="addmoreParamFrame" name='addMoreParamFrame' width="100%" height="100%" frameborder="0"   ></iframe>
        </div>
                ...
                <table id="viewResetParamId" align="right">
                    <thead>
                        <tr>
                            ...
                            <th class="navtitle-hover" >Parameter Name</th>
        ...
        <div id="createBucketdiv" style="display: none" title="Add Segment">
            <iframe  id="bucketDisp1" NAME='bucketDisp1'  frameborder="0"   height="100%" width="100%"></iframe>
        </div>
         <% } %>  
 <div id="saveNewReport" style="display:none" title="Save Dashboard">
            <table border="0" style="width:100%;">
                <tr valign="top">
                    <td valign="top"   style="width:40%;">
                        <label class="gFontFamily gFontSize12 label">Dashboard Name</label>
                    </td>
                    <td  valign="top"  style="width:60%;">
                        <input type="text" class="inputbox"  maxlength="200" name="reportName1" style="width:80%" id="reportName1" onblur="tabmsg1()" onkeyup="tabmsg1()" onfocus="document.getElementById('save1').disabled = false;">
                        <br>
                ...
                <tr><td  valign="top" colspan="2" align="center">
                        ...
                        <input type="button"  class="gFontFamily gFontSize12 navtitle-hover" style="width:auto" value="<%=TranslaterHelper.getTranslatedInLocale("next", cL)%>" id="save1" onclick="saveAsNewReport('<%=PbReportId%>','<%=request.getContextPath()%>','<%=reportName%>','Graph');"> 
                   </td>
                </tr>
            </table>
        </div>
...
<!--	<div style="display:none" id="custmemDispDia" title="<%=TranslaterHelper.getTranslatedInLocale("Custom_Measures", cL)%>">
        ...
        <div id="editViewByDiv" title="<%=TranslaterHelper.getTranslatedInLocale("Edit_ViewBy", cL)%>" style="display:none">
            <iframe  id="editViewByFrame" name='editViewFrame' width="100%" height="100%" frameborder="0"  ></iframe>
        </div>
        ...
        <script type="text/javascript">
            ...
            window.onresize = function(event) {
                resizeHichart();
                resizeGridDiv();
                $("#tableOption").css({"margin-left":($(window).width()-155)+"px"});
                $("#iframe1").height($(window).height()-120);
...
//                     resizeTblCont();
            ...
            $(document).ready(function() {
                ...
                if(navigator.userAgent.toLowerCase().indexOf('firefox') > -1 ){
                    $("#tableDivPro").height("400px");
                }else{
                    $("#tableDivPro").height("800px");
                }
            });
                function resizeGridDiv(){
                   if($("#managementTempl").is(":visible")){$("#gridsterDiv").height($(window).height()-300);}
                   else{$("#gridsterDiv").height($(window).height()-220);}
                   $("#gridsterDiv").width($(window).width());
                }
            ...
            //Added by Ram
            function selectFactsTables(tdId,tname){
                document.getElementById(tdId).style.display='none';
                if($("#tabsListValsFacts").val() == ""){
                    $("#tabsListValsFacts").val(tname)
                    $("#tabsListIdsFacts").val(tdId)
                }else{
                    var Ids = $("#tabsListIdsFacts").val()+","+tdId
                    var value = $("#tabsListValsFacts").val()+","+tname
                    $("#tabsListIdsFacts").val(Ids)
                    $("#tabsListValsFacts").val(value)
                }
            }
            
             function showFactsList(ctxpath,paramslist){
                if(document.getElementById("paramValsFacts").style.display=='none'){
                    $("#paramValsFacts").show();
                    $("#tabListDivFacts").show();
                    $("#tablistLinkFacts").show();
                    $("#goButton").show();
                    var htmlVar = "<table>";
                    $.post(ctxpath+"/reportViewer.do?reportBy=getTableList&paramslist="+paramslist+'&currentBizRoles='+'<%=rolesid%>'+'&currentReportId='+'<%= PbReportId%>',
                    function(data){
                        var jsonVar=eval('('+data+')')
                        var json1 = jsonVar.idsList.split(",");
                        var jsonname = jsonVar.namesList.split(",");
                        for(var i=0;i<json1.length;i++){
                            if(json1[i].replace(" ", "", "gi") != "0")
                                htmlVar += "<tr><td id='"+json1[i]+"' onclick=\"selectFactsTables('"+json1[i]+"','"+jsonname[i]+"')\">"+jsonname[i]+"</td></tr>";
                        }
                        htmlVar += "</table>";
                        $("#paramValsFacts").html(htmlVar);
            ...
            function setFactsValueToContainer(ctxpath,repId,bizRoles){
                $("#paramValsFacts").hide();
                var tabLst = $("#tabsListIdsFacts").val();
                $("#tabsListValsFacts").val('')
                $("#tabsListIdsFacts").val('')
                $.post(ctxpath+"/reportViewer.do?reportBy=setTableListToContainer&repId="+repId+'&tabLst='+tabLst,
                function(data){
                    document.getElementById("editMeasureAsViewBysFrame").src="Report/Viewer/ChangeMeasuresAsViewbys.jsp?loadDialogs=true&currentReportId="+repId+"&currentBizRoles="+bizRoles+"";
                });
            }
            function getDisplayTablesFacts(ctxpath,paramslist){
                var check = $("#tableListFacts").is(":checked")
                if($("#tableListFacts").is(":checked")){
                    $("#tabListDivFacts").hide();
                    $("#tablistLinkFacts").hide();
                    $("#goButton").hide();
                    $("#tabsListIdsFacts").val("");
                    $("#tabsListValsFacts").val("");
                    $.post(ctxpath+"/reportViewer.do?reportBy=setTableListToContainer&repId="+'<%= PbReportId%>',
                    function(data){
                        document.getElementById("editMeasureAsViewBysFrame").src="Report/Viewer/ChangeMeasuresAsViewbys.jsp?loadDialogs=true&currentReportId="+'<%= PbReportId%>'+"&currentBizRoles="+'<%=rolesid%>'+'&tableList=true'+"";
                    });
                    ...
                    function(data){
                        ...
                        htmlVar += "</table>";
                        $("#paramValsFacts").html(htmlVar);
                        document.getElementById("editMeasureAsViewBysFrame").src="Report/Viewer/ChangeMeasuresAsViewbys.jsp?loadDialogs=true&currentReportId="+'<%= PbReportId%>'+"&currentBizRoles="+'<%=rolesid%>'+"";
                    });
            ...
            function showList(ctxpath,paramslist){
                if(document.getElementById("paramVals").style.display=='none'){
                    ...
                    $("#goButton").show();
                    var htmlVar = "<table>";
                    $.post(ctxpath+"/reportViewer.do?reportBy=getTableList&paramslist="+paramslist+'&currentBizRoles='+'<%=rolesid%>'+'&aoid='+'<%=collect.AOId%>',
                    function(data){
                        var jsonVar=eval('('+data+')')
                        var json1 = jsonVar.idsList.split(",");
                        var jsonname = jsonVar.namesList.split(",");
...
var htmlSignature=$("input[name='htmlSignature2']:checked").val();
           ...
           function(data){
               ...
               if(data=='Report is Scheduled Sucessfully'){
       ...
       function factFormulaDiv()
       {
           ...
           function(data){
               if(data!='null'){
                   ...
                   var html="";
                   html+="<table width='100%'><thead class='navtitle-hover'><th>&nbsp</th><th><%=TranslaterHelper.getTranslatedInLocale("Filter_Name", cL)%></th><th><%=TranslaterHelper.getTranslatedInLocale("Filter_Formula", cL)%></th></thead><tbody>"
                   for(var i=0;i<filterId.length;i++){
                       html+="<tr><td><input type=\'checkbox\' id=\'filterId"+filterId[i]+"\'class=\'filetrIdClass\' name=\'filetrId\'  value="+filterId[i]+"></td>";
                       html+="<td align='center'>"+filterName[i]+"</td>";
                       html+="<td align='center'>"+filterFormula[i]+"</td></tr>";
                   }
                   html+="<tr><td width=\'20px;\'>&nbsp;</td></tr><tr><td align='center'><input class='navtitle-hover' type=\'button\' value='<%=TranslaterHelper.getTranslatedInLocale("Delete", cL)%>' onclick=deleteFormula('<%=request.getContextPath()%>','<%=PbReportId%>')></td></tr>";
               }
       ...
       function callFilterDiv() {
        ...
        <style>
            #fixedtop1{position:absolute;top:0;left:200px;margin:auto;right:800px;width:50px;border:none;z-index:50}
            #center250a{width:auto;height:65px;background:none}
            #center250b{width:auto;height:65px;background:none}
            #datalist>table tr{border-bottom:1px solid #ccc;curdor:pointer}
            #datalist>table td{padding:8px}
            #datalist>table td:hover{background-image:-moz-linear-gradient(top,rgba(139,195,74,0.8),#8BC34A);background-image:-webkit-gradient(linear,0 0,0 100%,from(rgba(139,195,74,0.8)),to(#8BC34A));background-image:-webkit-linear-gradient(top,rgba(139,195,74,0.8),#8BC34A);background-image:-o-linear-gradient(top,rgba(139,195,74,0.8),#8BC34A);background-image:linear-gradient(to bottom,rgba(139,195,74,0.8),#8BC34A);background-repeat:repeat-x;filter:progid:DXImageTransform.Microsoft.gradient(startColorstr='#ff0088cc',endColorstr='#ff0077b3',GradientType=0)}
            #datalist>table td:hover a{color:#fff}
            #datalist>table td a{color:#555;display:block;text-decoration:none}
        </style>
        <script>
            var fdate,cfdate;
            var fdate1,cfdate1;
		 ...
		 var incriment=0;
            ...
            <%}%>
                                              ...
                                              function viewReset(repId){
                                                  ...
                                                  var elementId="";
                                                  var paramdisId="";
                                                  $.post('reportViewer.do?reportBy=viewParameterReset&reportId='+repId,
                                                  function(data){
                                                      var jsonVar=eval('('+data+')')
...
//                                                   $("#tabParametersTopParent").dialog('open');
                                                   $("#tabParametersTopParent").css({"width":$(window).width(),"height":$(window).height(), "left": 0, " right":0,    "top": 0  });
                                                  if(paramValq=='show_param'){
                                                      var viewbys=$("#numbuerOfViewbys").val();
                                                      var divHeight=(3/5)*($(window).height())+50;
                                                      $("#tabParametersTop").css('height', divHeight);
                                                      var h2=$("#headerTableData").height();
                                                      var heit=(divHeight-h2);
                                                      $(".dynamicClass").css('height', '320px');
                                                      $(".dynamicClass").css('width', widt);
                                                      $(".dynamicClass").css('overflow', 'auto');
                                                      $("#tabParametersTop").animate({right:rght,  height:divHeight, width:widt});
                                                      ...
                                                      $('#tabParametersTop').css('display', 'block');
                                                        $('#tabParametersTop').css('top',($(window).height()*0.2) );
                                                       $("#tabParametersTopParent").css("display","block");
                                                  }
                                                  else{
                                                      $("#tabParametersTop").animate({
                                                       ...
                                                       $("#tabParametersTopParent").css("display","none");
...
// added by krishan pratap
        ...
        <div id="editBucketDiv" title="Edit Bucket Details " STYLE='display:none;'>
            <iframe  id="editBucketframe" NAME='editBucketframe'    frameborder="0" height="100%" width="100%" ></iframe>
        </div>
        ...
        <div id="hideMeasureDiv" style="display:none" title="<%=TranslaterHelper.getTranslatedInLocale("Hiding_Measures", cL)%>">
        ...
        <div id="hideViewByDiv" style="display:none" title="Hiding ViewBys">
            <iframe  id="hideViewbyFrame" NAME='hideViewbyFrame' width="100%" height="100%" frameborder="0"></iframe>
        </div>
<!--        //added by krishan pratap-->
              <div id="importExcelFileDiv123" style="display:none" title="<%=TranslaterHelper.getTranslatedInLocale("Upload_Template_Sheet", cL)%>">
         </div>
 <div  id="report_color_picker" style="display:none"></div>
<div id="ReportMultiCalendar" style="display:none" title="Report Multi Calendar">
        ...
        </div><div id="CustomerReportBugMail" style="display:none" title="<%=TranslaterHelper.getTranslatedInLocale("Customer_Report_Bug_Mail", cL)%>">
        </div>
        ...
        <% } %>
         ...
         <div id="definekpichartalerts" title="Define Alerts" style="display:none">
        </div>
            ...
            <div id="addMailIds" title="Add Mails" style="display:none">
        </div>
	...
	<input type="hidden" id="aoAsGoId" name="aoAsGoId" value="<%=aoAsGoId%>" />
        <input type="hidden" name="AOId" id="AOId" value="<%=aoid%>">
        ...
        <!--script-->
          <script type="text/javascript">
            ...
            var isKPIDashboard1="";
            var fromReport="true";
            var ViewByArrayId=[];var ViewByArrayName=[];var sizeoffilters=6;var sizeoffiltersg=6;
            var ColViewByArrayId=[];var ColViewByArrayName=[];
            var filterMapNew={};
            var filterMapNotin={};  var filterMapNotinDB={};
            var filterMapNewtb={};
            var filterMapgraphs={};
            var nextviewbyid="";
            var isfilteraplied=false;var sigleselectflag=false;
...
$("#advanceParamDiv").dialog({
       ...
       double finalValue = Math.round(total_time * 100.0) / 100.0;
            %>
                ...
                if(isAtLeastIE11)
                    {
                       $("#menuBarUI").css({"margin-top":"60px"}); 
                    }
                        
               if(checkBrowser() == "ie"){  $("#advanceFormulaDiv").css({"display":"none"}); }
       ...
       <script type="text/javascript">
   var defaultTabVar = "<%=defaulttab%>";
   if(typeof defaultTabVar!="undefined" && defaultTabVar!="" && defaultTabVar == "Graph"){
   }else {
...
ProgenReportViewerDAO prdao=new ProgenReportViewerDAO();
prdao.createDefaultRepSavepoint(request,userid,PbReportId);
}
    } else {
        response.sendRedirect(request.getContextPath() + "/baseAction.do?param=logoutApplication");
    }
