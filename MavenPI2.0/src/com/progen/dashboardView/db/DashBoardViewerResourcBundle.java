 ...
 * @author progen
 */
class DashBoardViewerResourcBundle extends ListResourceBundle implements Serializable {
    ...
    static final Object contents[][] = {
        ...
        {"getGraphMaster", "SELECT pagm.graph_id,pagm.graph_name,pagm.GRAPH_WIDTH AS GRAPH_WIDTH,pagm.GRAPH_HEIGHT AS GRAPH_HEIGHT,pagm.graph_class,pagc.graph_class_name,"
            ...
            + "FROM PRG_AR_GRAPH_MASTER pagm ,PRG_AR_GRAPH_SIZES pags,PRG_AR_GRAPH_CLASS pagc,PRG_AR_GRAPH_TYPE pagt "
            + " WHERE pagm.graph_size  = pags.graph_size_id AND pagc.graph_class_id= pagm.graph_class "
            + "AND pagm.graph_type    = pagt.graph_type_id AND pagm.graph_id ='&' "
        ...
        {"getQueryDetails", "select distinct element_id, col_disp_name, ref_element_id, folder_id, sub_folder_id, aggregation_type, column_type"
            + " from prg_ar_query_detail where element_id in (&) and report_id=&"},
        {"getkpiGraphDetails", "SELECT pakgd.KPI_GRP_ID, pakgd.ELEMENT_ID, pakgd.DASHBOARD_ID, pakgd.NEEDLE, pakgd.KPINAME, pakgd.KPIGRAPHTYPE,"
