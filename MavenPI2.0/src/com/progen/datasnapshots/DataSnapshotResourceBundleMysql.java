 ...
 */
public class DataSnapshotResourceBundleMysql extends ListResourceBundle implements Serializable {
    ...
    static final Object contents[][] = {
        {"getDataSnapShotId", "select LAST_INSERT_ID(DATA_SNAPSHOT_ID) from PRG_AR_DATA_SNAPSHOTS order by 1 desc limit 1"},
        {"insertDataSnapShots", "insert into PRG_AR_DATA_SNAPSHOTS(DATA_SNAPSHOT_NAME,REPORT_ID,REFRESH_INTERVAL,CREATED_BY,UPDATED_BY,DATA_HTML_VIEW,CREATE_DATE,UPDATED_DATE,LAST_REFRESHED,HTML_REPORT_TYPE) values('&',&,&,&,&,'&',LOCALTIMESTAMP(),now(),now(),'&')"},
        ...
        {"getHtmlViewClob", "select DATA_HTML_VIEW from PRG_AR_DATA_SNAPSHOTS where DATA_SNAPSHOT_ID=& "},
        {"updateHtmlView", "update PRG_AR_DATA_SNAPSHOTS set DATA_HTML_VIEW=?,LAST_REFRESHED=now() WHERE DATA_SNAPSHOT_ID=?"},
        {"getSysdate", "select now() SYSDATE"},
        ...
        {"updateHtmlView1", "update PRG_AR_DATA_SNAPSHOTS set DATA_HTML_VIEW='na',LAST_REFRESHED=now() WHERE DATA_SNAPSHOT_ID=&"},
        {"getHeaderTitle", "select SETUP_CHAR_VALUE from PRG_GBL_SETUP_VALUES where SETUP_KEY='COMPANY_NAME'"},
        ...
        {"insertHeadlines", "insert into PRG_AR_HEADLINES(HEADLINE_DATA,HEADLINE_NAME,REPORT_ID,DATA_STATUS) values('&','&',&,'&')"},
        {"getHeadlineId", "select LAST_INSERT_ID(HEADLINE_ID) from PRG_AR_HEADLINES order by 1 desc limit 1"},
        {"updateHeadlineView1", "update PRG_AR_HEADLINES set HEADLINE_DATA='na' WHERE HEADLINE_ID=&"},
        {"getHeadlineClob", "select HEADLINE_DATA from PRG_AR_HEADLINES where HEADLINE_ID=& "},
        ...
        {"insertHeadlinesAssignment", "insert into PRG_HEADLINES_ASSIGNMENT(HEADLINE_ID,USER_ID) values(&,&)"},
        ...
        {"getAssignId", "select IDENT_CURRENT('PRG_HEADLINES_ASSIGNMENT')"},
