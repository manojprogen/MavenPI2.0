 ...
 * @author db2admin
 */
public class PbReportCollectionResBunSqlServer extends ListResourceBundle implements Serializable {
    ...
    static final Object contents[][] = {
        ...
        , {"getReportTableInfo", "SELECT parqd.element_id,PARTD. COL_NAME,CASE WHEN parqd.column_type='VARCHAR2' THEN 'C' WHEN parqd.column_type='NUMBER' THEN 'N' "
            ...
            + " , PARTD.NUMBER_FORMAT,PARTD.ROUNDING_PRECISION, PARTD.RUNTIME_MEASURE_ID, PARTD.SHOW_INDICATOR, PARTD.SCRIPT_INDICATOR , PARTD.SCRIPT_ALIGN , PARTD.MEASURE_TYPE , PARTD.MEASURE_ALIGN,PARTD.TIME_CONVERSION,PARTD.DATE_TIME_OPTION,PARTD.DATA_SUBSTRING,PARTD.DATE_FORMAT,PARTD.REPORT_DRILL_ASSIGN_REPORT,PARTD.MULTI_REPORT_DRILL_ASSIGN,PARTD.HIDE_MSR,PARTD.SUBTOTAL_DEVIATION,PARTD.SUMMERIZED_MSR_ENABLE,PARTD.AVGZEROCOUNTTYPE,PARTD.CT_GT_AGG_TYPE,PARTD.NUM_FORMATE,PARQD.drill_message FROM PRG_AR_REPORT_TABLE_DETAILS PARTD left outer join   PRG_AR_QUERY_DETAIL PARQD on (parqd.qry_col_id = partd.qry_col_id) "
            + " where partd.report_id='&' ORDER BY PARTD.disp_seq, PARTD.QRY_COL_ID "}////changed order by becasue %wise meas don't have qry_col_id so were pushed to last always
        , {"getParameterTimeInfo", "select m.rep_time_id , m.time_level , m.time_type , d.column_type, d.column_name, d.sequence, d.form_sequence ,convert(varchar,d.default_date,101),d.default_value"
        ...
        , {"getReportTableMasterInfo", "SELECT REP_TAB_ID, REPORT_ID, TABLE_NAME, TABLE_TYPE, TABLE_SEQ,case when SHOW_GRD_TOTAL='Y' then 'true' else 'false' END AS SHOW_GRD_TOTAL"
            ...
            + ",case when DRILL_ACROSS='Y' then 'true' else 'false'END AS DRILL_ACROSS,case when DRILL_MEASURE='Y' then 'true' else 'false' END AS DRILL_MEASURE,case when SHOW_CATAVG='Y' then 'true' else 'false' END AS SHOW_CATAVG,SUMMERIZED_TABLE_MEASURE,CROSSTAB_MEASURE_MAP,GT_AVERAGE,TOPBOTTOM_VAL,SOURCETYPE,EXCEL_FILE_PATH FROM PRG_AR_REPORT_TABLE_MASTER where report_id='&' "}, {"selectWhatifQuery", "select REPORT_ID,WHATIF_DETAILS from PRG_AR_WHATIFS where report_id =&"}, {"getRefDimTabId", "select REF_DIM_TAB_ID from PRG_USER_ALL_INFO_DETAILS where ELEMENT_ID='&' AND REF_DIM_TAB_ID is not null"}
    };
}
