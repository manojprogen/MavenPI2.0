...
public class CSVTableBodyDisplay extends TableBodyDisplay {

    public CSVTableBodyDisplay(TableBuilder builder) {
        super(builder);
    }
    ...
    public StringBuilder generateHtmlForRows(ArrayList<TableDataRow> rowLst, ArrayList<TableSubtotalRow> subtotalRowLst) {
        ...
        for (TableDataRow tableRow : rowLst) {
            ...
            for (int col = tableBldr.getFromColumn(); col < colCount; col++) {
                ...
                body.append(data.replace(",", "").replace("%", "").replace("$", "").replace("Yen", "").replace("Rs", "").replace("Euro", "").replace("\n", ""));
                if (col != colCount - 1) {
                    body.append(",");
                }
    ...
    public StringBuilder generateSubTotalHTML(TableSubtotalRow[] subTotalRow) {
        CSVTableSubTotalDisplay subTotalDisplay = new CSVTableSubTotalDisplay(tableBldr);
