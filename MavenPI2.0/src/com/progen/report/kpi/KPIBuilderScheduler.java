 ...
 * @author Dinanath
 */
public class KPIBuilderScheduler {

    public static Logger logger = Logger.getLogger(KPIBuilderScheduler.class);
...
//        String kpiOrder = " Case ";
        StringBuilder kpiOrder = new StringBuilder();
...
//        finalQuery = sqlstr;
        finalQuery = " SELECT ELEMENT_ID,AGGREGATION_TYPE,case when USER_COL_DESC is null then USER_COL_NAME else USER_COL_DESC end USER_COL_NAME FROM PRG_USER_ALL_INFO_DETAILS  where ELEMENT_ID in ( " + elementIds + " )  order by " + kpiOrder + " ";
...
//        String kpiCase = " case ";
        ...
        StringBuilder kpiCase = new StringBuilder(400);
        kpiCase.append(" case ");
        PbDb db = new PbDb();
        kpiCols = elementIds.split(",");
...
//        kpiCase += " else 10000 end ";
        ...
        if (ProgenConnection.getInstance().getDatabaseType().equals(ProgenConnection.SQL_SERVER)) {
            sqlstr += " SELECT ELEMENT_ID,AGGREGATION_TYPE,ISNULL(USER_COL_DESC,USER_COL_NAME) USER_COL_NAME FROM PRG_USER_ALL_INFO_DETAILS  where ELEMENT_ID in ( " + elementIds + " )  order by " + kpiCase;

        } else {
            sqlstr += " SELECT ELEMENT_ID,AGGREGATION_TYPE,nvl(USER_COL_DESC,USER_COL_NAME) USER_COL_NAME FROM PRG_USER_ALL_INFO_DETAILS  where ELEMENT_ID in ( " + elementIds + " )  order by decode ( element_id," + kpiOrder + " )";
        }
//        finalQuery = sqlstr;
...
//        String urlElements = "";
        StringBuilder urlElements = new StringBuilder(400);
...
//        String finalQuery = "";
        ...
        for (int j = 0; j < a1.length; j++) {
            ...
            urlElements.append(",").append(kpi.get(0).toString());
...
//            urlElements = urlElements.substring(1);
        ...
        if (kpiType.equalsIgnoreCase("Target")) {
            ...
            dispKpi += "<th width=\"20px\">&nbsp;</th><th width=\"100px\"><strong>Target Value</strong></th><th width=\"70px\"><strong>Deviation%</strong></th><th width=\"180px\"><strong>Comments</strong></th>";
            ...
            if (kpiType.equalsIgnoreCase("PortalKPI")) {
                ...
                dispKpi += "<th ><strong>Current/Prior</strong></th>";
                dispKpi += "<th ><strong>Change%</strong></th>";
                dispKpi += "<th >&nbsp;</th>";
                dispKpi += "<th ><strong>Comments</strong></th>";
                dispKpi += "<th ><strong>Alerts</strong></th>";
                dispKpi += "<th ></th>";
                if (!(kpiType.equalsIgnoreCase("Basic")) || !(kpiType.equalsIgnoreCase("BasicTarget"))) {
                    dispKpi += "<th><strong>Graphs</strong></th>";
                }
            } else {
                dispKpi += "<th ><strong>Current</strong></th>";
                dispKpi += "<th ><strong>Prior</strong></th>";
            }
            dispKpi += "</tr></thead>";
            dispKpi += "<tfoot></tfoot><tbody>";
        }
...
//                finalQuery = sqlstr;
    ...
    public String processSingleKpiScheduler(Container container, String kpiMasterId, HashMap kpiQuery, String kpiDrill, String kpidashid, String dashBoardId, boolean forDesigner, pbDashboardCollection dashboardcollect, String userId, String editDbrd) throws Exception {
                                ...
                                if (flag == 1) {
                                    ...
                                    if (symbols.get(index) != null) {
                                        if (symbols.get(index).equalsIgnoreCase("%")) {
                                            ...
                                            arr.add(gtsb.toString() + symbols.get(index));
                                        } else {
                                            ...
                                            arr.add(symbols.get(index) + gtsb.toString());
                                        }
                                    } else {
                ...
                if (!atrelementIds.isEmpty() && atrelementIds.contains(ElementId) && !background.isEmpty() && background != null) {
                    ...
                    if (negativevalues != null && !negativevalues.isEmpty()) {
                        ...
                        if (istarget) {
                            ...
                            if (flag2 == 1) {
                                dispKpi.append("<Td style=\"cursor:pointer;display:none;color: " + font.get(index) + ";background-color:" + background.get(index) + "\"><a href=\"javascript:void(0)\" onclick=\"changeTargetValue('" + ElementId + ":" + kpiMasterid + ":" + timeLevel + ":" + targetValueStr + ":" + collect.reportId + ":" + dashletId + "','" + kpiType + "')\">" + targetValueStr + "</a></Td>");
                            } else {
                                dispKpi.append("<Td style=\"cursor:pointer;color: " + font.get(index) + ";background-color:" + background.get(index) + "\"><a href=\"javascript:void(0)\" onclick=\"changeTargetValue('" + ElementId + ":" + kpiMasterid + ":" + timeLevel + ":" + targetValueStr + ":" + collect.reportId + ":" + dashletId + "','" + kpiType + "')\">" + targetValueStr + "</a></Td>");
                            }
                            arr.add(targetValueStr);
                        }
                    }
...
//                                String viewbyname1 = "";
                                ...
                                DisplayGT.append("<Td style=\"cursor:pointer;background-color:" + background.get(atrelementIds.indexOf(ElementId)) + "\" align=\"center\" padding=\"0px\"><a onclick= \"viewAlertCondition('" + new Double(oneDForm.format(currVal)) + "','" + ElementId + "','" + kpiMasterid + "','" + dashletId + "','" + collect.reportId + "','" + targetValue + "','" + deviationPercent + "','" + kpiType + "','" + ElementName + "','" + container.getCustomKPIHeaderNames() + "','" + "GTValueAlertsOn" + "','" + "GTValueAlertsOn" + "')\"><span class=\"ui-icon ui-icon-alert\"></span></a></td>");
