 ...
 * @author progen
 */
public class ScorecardResBundleSqlServer extends ListResourceBundle implements Serializable {
    ...
    static final Object contents[][] = {
        {"getScoreCards", "select master.scard_id, master.scard_name, master.scard_area, master.no_of_days, master.lighttype,"
            ...
            + " prg_ar_scard_master master inner join  "
            + " prg_ar_scard_member member on (master.scard_id = member.scard_id) "
            + " left outer join prg_ar_scard_member_kpi kpi on (member.scard_mem_id = kpi.scard_mem_id )"
            + " where  master.scard_id in (&)"},
        ...
        {"saveScardDetails", "Insert into PRG_AR_SCARD_MASTER(SCARD_NAME,SCARD_AREA,NO_OF_DAYS,CREATED_BY,UPDATED_BY,CREATE_DATE,UPDATE_DATE,LIGHTTYPE,FOLDER_ID,TARGET_SCORE) "
            + " values('&','&',&,&,&,&,&,'&','&',&)"},
        {"saveScardMemDetails", "Insert into PRG_AR_SCARD_MEMBER(SCARD_ID,SCARD_TYPE,SCARD_CONTRIBUTION,SCARD_SEQ,UNITS,PERIOD,CHILD_SCARD_ID)"
            + " values(&,'&',&,&,'&','&',&)"},
        {"saveScardMemKpiDetails", "Insert into PRG_AR_SCARD_MEMBER_KPI(SCARD_ID,SCARD_MEM_ID,ELEMENT_ID,SCARD_BASIS,ELEMENT_NAME,PRIOR_ELEMENT_ID,CHANGE_ELEMENT_ID,CHANGEP_ELEMENT_ID) "
            + "values(&,&,&,'&','&',&,&,&)"},
        {"saveScardMemKpiRules", "Insert into PRG_AR_SCARD_MEMBER_KPI_RULE(SCARD_DEATILS_ID,SCARD_ID,SCARD_MEM_ID,SCARD_MEM_VALUE_ST,SCARD_MEM_VALUE_END,SCARD_SCORE,OPERATOR) "
            + " values(&,&,&,&,&,&,'&')"},
        {"saveScardtoUser", "Insert into PRG_AR_SCARD_USERS(SCARD_ID,USER_ID) values(&,&)"},
        {"saveScardColor", "Insert into PRG_AR_SCORECARD_LIGHTS(SCARDID,SCORE_START,SCORE_END,LIGHT_COLOR,OPERATOR) "
            + " values(&,&,&,'&','&')"},
        ...
        {"saveScardMemKpiDetailsForDimension", "Insert into PRG_AR_SCARD_MEMBER_KPI(SCARD_ID,SCARD_MEM_ID,ELEMENT_ID,SCARD_BASIS,ELEMENT_NAME,PRIOR_ELEMENT_ID,CHANGE_ELEMENT_ID,CHANGEP_ELEMENT_ID,DIMENSION_ID,DIMENSION_VALUE,TARGET_ELEMENT_ID,TARGET_ELEMENT_NAME,TARGET_TYPE,TARGET_VALUE) "
            + "values(&,&,&,'&','&',&,&,&,&,'&',&,'&','&',&)"},
        ...
        {"getOriginalMeasQry", "select  member.scard_mem_id,member.scard_contribution, member.scard_seq,"
            ...
            + " where"
            + "  kpi.scard_basis not in('AdHoc') and "
        ...
        {"insertAdhocMemQry", "insert into prg_ar_scard_member(SCARD_ID,SCARD_CONTRIBUTION) values(&,&)"},
        {"insertAdhocMemKpiQry", "insert into PRG_AR_SCARD_MEMBER_KPI(SCARD_ID,SCARD_MEM_ID,SCARD_BASIS,ELEMENT_NAME) "
            + " values(&,&,'&','&')"}
    };
}
