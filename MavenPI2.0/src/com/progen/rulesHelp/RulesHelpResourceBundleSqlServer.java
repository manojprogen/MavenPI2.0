 ...
 * @author progen
 */
public class RulesHelpResourceBundleSqlServer extends ListResourceBundle implements Serializable {
    ...
    static final Object contents[][] = {
        {"getDimentions", "SELECT COALESCE(reld.rel_level,1) level1,key1.element_id KEY_ELEMENT_ID,key1.USER_COL_TYPE Key_col_type,val1.element_id val_element_id,key1.buss_col_id KEY_BUSS_COL_ID,val1.buss_col_id val_buss_col_id,key1.buss_col_name"
            ...
            + "LEFT OUTER JOIN prg_grp_dim_rel relm ON (key1.dim_id = relm.dim_id) LEFT OUTER JOIN prg_grp_dim_rel_details reld ON (key1.member_id    = reld.mem_id) WHERE key1.folder_id IN(&)AND key1.dim_name !='Time'ORDER BY 1"}, {"saveRule", "INSERT INTO PRG_USER_RULES VALUES('&','&','&','&','&')"}, {"getFilterDetails", "SELECT * FROM PRG_USER_RULES WHERE REF_ID=& "}, {"getruleDimMembers", "SELECT distinct COALESCE(reld.rel_level,1) level1,key1.element_id KEY_ELEMENT_ID,key1.buss_col_name,key1.BUSS_TABLE_NAME FROM PRG_GRP_DIM_MAP_KEY_VALUE map"
