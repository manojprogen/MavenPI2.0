...
public class PbUserLayerResourceBundle extends ListResourceBundle implements Serializable {
    ...
    static final Object contents[][] = {
        {"addUserFolder", "INSERT INTO PRG_USER_FOLDER(FOLDER_ID, FOLDER_NAME, FOLDER_DESC,FOLDER_CREATED_ON,FOLDER_UPDATED_ON,GRP_ID) VALUES (PRG_USER_FOLDER_SEQ.NEXTVAL,'&','&',SYSDATE,SYSDATE,&)"}, {"addUserFolderDetails", "insert into PRG_USER_FOLDER_DETAIL (SUB_FOLDER_ID,FOLDER_ID, SUB_FOLDER_NAME, SUB_FOLDER_TYPE) "
            + " values('&',PRG_USER_FOLDER_SEQ.currval,'&','&' )"} /*
         ...
         * and grp_id='&'"}
         *
         */, {"getBussFactsInfoByGrpId", "select buss_table_id, buss_table_name from prg_grp_buss_table where buss_table_id in ( SELECT BUSS_TABLE_ID FROM PRG_GRP_BUSS_TABLE minus "
            + " SELECT DISTINCT TAB_ID FROM PRG_GRP_DIM_TABLES ) and buss_type!='Query' and nvl(is_pure_query,'N')!='Y' and grp_id='&'"} /*
        ...
        , {"addUserSubFolderElements", "INSERT INTO PRG_USER_SUB_FOLDER_ELEMENTS (ELEMENT_ID, SUB_FOLDER_ID, BUSS_TABLE_ID, BUSS_COL_ID, BUSS_COL_NAME, USER_COL_NAME,"
            + " USER_COL_DESC, USER_COL_TYPE, SUB_FOLDER_TAB_ID,REF_ELEMENT_ID,REF_ELEMENT_TYPE,DEFAULT_AGGREGATION,USE_REPORT_FLAG,Display_Formula) SELECT (PRG_USER_SUB_FLDR_ELEMENTS_SEQ.NEXTVAL) as ELEMENT_ID,"
            + "& as SUB_FOLDER_ID,a.BUSS_TABLE_ID, a.buss_column_id, a.column_name,a.column_disp_name,a.column_display_desc,"
            + "a.column_type,& as SUB_FOLDER_TAB_ID,(PRG_USER_SUB_FLDR_ELEMENTS_SEQ.CURRVAL),1 AS REF_ELEMENT_TYPE,a.default_aggregation,'Y',a.display_formula FROM PRG_GRP_BUSS_TABLE_DETAILS a "
        ...
        , {"getElementsInfo", " SELECT & AS SUB_FOLDER_ID,a.BUSS_TABLE_ID,a.buss_column_id, a.column_name AS BUSS_COL_NAME, a.column_disp_name AS USER_COL_NAME, a.column_display_desc AS USER_COL_DESC,"
            + "a.column_type,& AS SUB_FOLDER_TAB_ID,a.default_aggregation, 1 REF_ELEMENT_TYPE,a.reffered_elements, nvl(a.COLUMN_DISPLAY_DESC,a.column_disp_name ) COLUMN_DISPLAY_DESC,a. actual_col_formula,a.display_formula FROM PRG_GRP_BUSS_TABLE_DETAILS a "
            + "where  a.buss_table_id='&'"} //modified by susheela over
        /*
         * ,{"getElementsInfo"," SELECT & AS
         ...
         * b.element_type_name != 'Current' and " + "(a.user_col_type ='NUMBER'
         * OR upper(a.user_col_type)='CALCULATED') " + "and a.element_id in(&)"}
         */ //modified by susheela start18thNov
        , {"insertExtraElementsForFacts", "insert into PRG_USER_SUB_FOLDER_ELEMENTS( ELEMENT_ID, SUB_FOLDER_ID, BUSS_TABLE_ID, BUSS_COL_ID, BUSS_COL_NAME,USER_COL_NAME,"
            ...
            + "and a.element_id in(&)"} //modified by susheela over
        ...
        {"getUserFolderList1", "SELECT f.FOLDER_ID, f.FOLDER_NAME, f.FOLDER_DESC, g.GRP_NAME,G.GRP_ID FROM PRG_USER_FOLDER  f, PRG_GRP_MASTER g"
            + " where g.grp_id=f.grp_id and f.FOLDER_ID='&' and g.grp_id in(select grp_id from prg_grp_master where connection_id in (&))  "},
        {"getUserSubFolderList", "  select sub_folder_id, sub_folder_name, sub_folder_type from PRG_USER_FOLDER_DETAIL where folder_id='&' and sub_folder_name!='Formula'"} /*
         ...
         */ // ,{"getUserFolderDimMbrs"," SELECT sub_folder_tab_id, member_id , nvl(disp_name, member_name) as mem_disp_name ,dim_tab_id FROM PRG_USER_SUB_FOLDER_TABLES WHERE sub_folder_id ='&' AND is_dimension  ='Y' AND dim_id ='&' ORDER BY mem_disp_name"}
        //modified by susheela start 16th Nov
        //        , {"getUserFolderDimMbrs", " SELECT sub_folder_tab_id, member_id , nvl(disp_name, member_name) as mem_disp_name ,dim_tab_id,use_report_member,use_report_dim_member FROM PRG_USER_SUB_FOLDER_TABLES WHERE sub_folder_id ='&' AND is_dimension  ='Y' AND dim_id ='&' ORDER BY mem_disp_name"} //modified by susheela over
        //added by Nazneen
        , {"getUserFolderDimMbrs", "SELECT A.sub_folder_tab_id,A.member_id ,NVL(A.disp_name, A.member_name) AS mem_disp_name ,A.dim_tab_id,A.use_report_member,A.use_report_dim_member,b.element_id FROM PRG_USER_SUB_FOLDER_TABLES A,PRG_USER_SUB_FOLDER_ELEMENTS B,PRG_GRP_DIM_MAP_KEY_VALUE map WHERE map.key1       = B.buss_col_id and A.member_id = map.key_mem_id and A.sub_folder_tab_id = B.sub_folder_tab_id and A.sub_folder_id ='&' AND A.is_dimension    ='Y' AND A.dim_id          ='&' ORDER BY mem_disp_name "} //modified by Nazneen over   
        , {"getUserFolderDimMbrsCols", "SELECT ELEMENT_ID, USER_COL_NAME,USER_COL_DESC FROM PRG_USER_SUB_FOLDER_ELEMENTS where sub_folder_tab_id='&'"}, {"getUserFolderFacts", "SELECT DISTINCT sub_folder_tab_id, nvl(table_disp_name,disp_name) disp_name,USE_REPORT_MEMBER,nvl(table_tooltip_name,disp_name) tooltip_name FROM PRG_USER_SUB_FOLDER_TABLES where is_fact='Y' and sub_folder_id='&' and disp_name not in('Calculated Facts','Formula') order by disp_name "}, {"getUserFolderFactsColumns", "SELECT ELEMENT_ID,USER_COL_NAME  FROM PRG_USER_SUB_FOLDER_ELEMENTS where SUB_FOLDER_TAB_ID='&' order by ref_element_id, ref_element_type"}, {"getUserFolderBuckets", "SELECT DISTINCT sub_folder_tab_id, nvl(table_disp_name,disp_name) disp_name,nvl(table_tooltip_name,disp_name) tooltip_name FROM PRG_USER_SUB_FOLDER_TABLES where is_bucket='Y' and sub_folder_id='&' order by disp_name "}, {"getUserFolderBucketsColumns", "SELECT ELEMENT_ID,USER_COL_NAME,USER_COL_DESC  FROM PRG_USER_SUB_FOLDER_ELEMENTS where SUB_FOLDER_TAB_ID='&' order by ref_element_id, ref_element_type"} //,{"getUserFoldDimNames","SELECT DISTINCT DIM_ID,DIM_NAME FROM PRG_USER_SUB_FOLDER_TABLES where sub_folder_id='&' and is_dimension='Y' order by DIM_NAME"}
        ...
        , {"addUserAllInfoDetails", " insert into PRG_USER_ALL_INFO_DETAILS(GRP_ID ,FOLDER_ID ,FOLDER_NAME ,SUB_FOLDER_ID ,SUB_FOLDER_NAME ,SUB_FOLDER_TYPE ,SUB_FOLDER_TAB_ID,"
            ...
            + " pusft.DIM_ID ,pusft.DIM_TAB_ID ,pusft.DIM_NAME ,pusfe.ELEMENT_ID ,pusfe.BUSS_TABLE_ID ,pusfe.BUSS_COL_ID ,"
            + " pusfe.BUSS_COL_NAME ,pusfe.USER_COL_NAME ,pusfe.USER_COL_DESC,pusfe.USER_COL_TYPE ,pusfe.ref_element_id ,"
            + " pusfe.ref_element_type ,pusft.member_id ,pusft.member_name, busstab.buss_table_name,connTab.conn_id, bussdet.default_aggregation,bussdet.ACTUAL_COL_FORMULA,pusfe.use_report_flag,bussdet.REFFERED_ELEMENTS,bussdet.display_formula,nvl(pusft.TABLE_DISP_NAME,pusft.DISP_NAME),nvl(pusft.TABLE_TOOLTIP_NAME,pusft.DISP_NAME) "
            + " FROM PRG_USER_FOLDER puf ,PRG_USER_FOLDER_DETAIL pufd ,PRG_USER_SUB_FOLDER_TABLES pusft ,PRG_USER_SUB_FOLDER_ELEMENTS pusfe,"
            + " PRG_GRP_BUSS_TABLE bussTab,PRG_GRP_BUSS_TABLE_DETAILS bussDet,(Select min(CONNECTION_ID) conn_id, buss_table_id "
            + " from prg_grp_buss_table_src group by buss_table_id) connTab WHERE puf.folder_id = pufd.folder_id "
            + " AND pufd.SUB_FOLDER_ID =pusft.SUB_FOLDER_ID AND pusfe.sub_folder_tab_id=pusft.sub_folder_tab_id "
        ...
        , {"addUserAllDDIMDetails", "INSERT INTO PRG_USER_ALL_DDIM_DETAILS( INFO_FOLDER_ID, INFO_ELEMENT_ID, INFO_BUSS_COL_ID, REL_LEVEL, INFO_DIM_ID, IMFO_DIM_NAME, INFO_MEMBER_ID, INFO_MEMBER_NAME)"
            ...
            + "info.buss_col_id info_buss_col_id,  nvl(reld.rel_level,1) rel_level, info.dim_id info_dim_id, "
            + "info.dim_name imfo_dim_name,info.member_id info_member_id,info.member_name info_member_name  "
            + "from PRG_USER_ALL_INFO_DETAILS info , prg_grp_dim_rel relm , prg_grp_dim_rel_details reld  "
            + "where info.member_id = reld.mem_id(+) and info.dim_id = relm.dim_id(+) and relm.is_default(+) ='Y'  "
            + "and info.member_id is not null and info.folder_id in(&)"}, {"addUserAllDDIMMaster", "INSERT INTO PRG_USER_ALL_DDIM_MASTER ( INFO_FOLDER_ID, REL_LEVEL, INFO_DIM_ID, INFO_DIM_NAME, INFO_MEMBER_ID, INFO_MEMBER_NAME ) "
            + "select distinct info.folder_id info_folder_id,nvl(reld.rel_level,1) rel_level,info.dim_id info_dim_id, "
            + "info.dim_name info_dim_name,info.member_id info_member_id,info.member_name info_member_name "
            + "from PRG_USER_ALL_INFO_DETAILS info , prg_grp_dim_rel relm , prg_grp_dim_rel_details reld "
            + "where info.member_id = reld.mem_id(+) and info.dim_id = relm.dim_id(+) and relm.is_default(+) ='Y' "
            + "and info.member_id is not null and info.folder_id in(&)"}, {"addUserAllDDIMKeyValEle", "INSERT INTO PRG_USER_ALL_DDIM_KEY_VAL_ELE (LEVEL1,KEY_ELEMENT_ID,KEY_COL_TYPE,VAL_ELEMENT_ID,KEY_BUSS_COL_ID,VAL_BUSS_COL_ID, "
            ...
            + "SELECT NVL(reld.rel_level,1) level1,key1.element_id Key_element_id,key1.USER_COL_TYPE Key_col_type, "
            + "val1.ref_element_id val_element_id ,key1.buss_col_id key_buss_col_id ,val1.buss_col_id val_buss_col_id , "
            ...
            + "FROM PRG_GRP_DIM_MAP_KEY_VALUE map ,PRG_USER_ALL_INFO_DETAILS key1 ,PRG_USER_ALL_INFO_DETAILS val1 , "
            + "prg_grp_dim_rel relm ,prg_grp_dim_rel_details reld WHERE map.key1 = key1.buss_col_id AND key1.member_id = map.key_mem_id "
            + "AND val1.member_id = map.mem_mem_id AND val1.buss_col_id = map.value1 "
            + "AND key1.member_id = reld.mem_id(+)AND key1.dim_id = relm.dim_id(+) "
            + "AND relm.is_default(+) ='Y' and key1.folder_id in(&) AND val1.folder_id  IN(&) ORDER BY 1"}, {"addUserAllADIMDetails", "INSERT INTO PRG_USER_ALL_ADIM_DETAILS(INFO_FOLDER_ID, INFO_ELEMENT_ID, INFO_BUSS_COL_ID, REL_LEVEL, INFO_DIM_ID, INFO_DIM_NAME, INFO_MEMBER_ID, INFO_MEMBER_NAME) "
            + "select info.folder_id info_folder_id, info.element_id info_element_id, info.buss_col_id info_buss_col_id, "
            + "nvl(reld.rel_level,1) rel_level,info.dim_id info_dim_id,info.dim_name info_dim_name,info.member_id info_member_id, "
            + "info.member_name info_member_name from PRG_USER_ALL_INFO_DETAILS info , prg_grp_dim_rel relm , prg_grp_dim_rel_details reld "
            + "where info.member_id = reld.mem_id(+) and info.dim_id = relm.dim_id(+) and info.member_id is not null and info.folder_id in(&)"}, {"addUserAllAdimMaster", "INSERT INTO PRG_USER_ALL_ADIM_MASTER(INFO_FOLDER_ID, REL_LEVEL, INFO_DIM_ID, INFO_DIM_NAME, INFO_MEMBER_ID, INFO_MEMBER_NAME) "
            + "select distinct info.folder_id info_folder_id,nvl(reld.rel_level,1) rel_level,info.dim_id info_dim_id,info.dim_name info_dim_name, "
            + "info.member_id info_member_id,info.member_name info_member_name from PRG_USER_ALL_INFO_DETAILS info  , prg_grp_dim_rel relm "
            + ", prg_grp_dim_rel_details reld where info.member_id = reld.mem_id(+) and info.dim_id = relm.dim_id(+) and info.member_id is not null and info.folder_id in(&)"}, {"addUserAllADIMKeyValEle", "INSERT INTO  PRG_USER_ALL_ADIM_KEY_VAL_ELE( LEVEL1, KEY_ELEMENT_ID, KEY_COL_TYPE, VAL_ELEMENT_ID, KEY_BUSS_COL_ID, "
            ...
            + ", relm.rel_name , relm.is_default FROM PRG_GRP_DIM_MAP_KEY_VALUE map ,PRG_USER_ALL_INFO_DETAILS key1 , "
            + "PRG_USER_ALL_INFO_DETAILS val1 ,prg_grp_dim_rel relm ,prg_grp_dim_rel_details reld WHERE map.key1 = key1.buss_col_id "
            + "AND key1.member_id = map.key_mem_id AND val1.member_id = map.mem_mem_id AND val1.buss_col_id = map.value1 "
            + "AND key1.member_id = reld.mem_id(+) AND key1.dim_id = relm.dim_id(+) and key1.folder_id in(&) AND val1.folder_id  IN(&) ORDER BY 1 "} //added by susheela start
        ...
        , {"checkTargetFolderForGrp", "select puf.folder_name, tmf.bus_folder_id,pgm.grp_name,tmf.bus_group_id from"
            ...
            + " pgm.grp_id= tmf.bus_group_id "}, {"insertTargetRole", "insert into target_measure_folder(TARGET_MEASURE_FOL_DETAIL,BUS_FOLDER_ID,ONLY_FOR_TARGET,UPDATED_BY,CREATED_BY,BUS_GROUP_ID) values(TARGET_MEASURE_FOLDER_SEQ.nextval,'&','&','&','&','&')"} // added by susheela over
        ...
        , {"copyUserFolderTbales", "INSERT INTO PRG_USER_SUB_FOLDER_TABLES(SUB_FOLDER_ID, BUSS_TABLE_ID, IS_DIMENSION, IS_FACT, IS_BUCKET, DISP_NAME, SUB_FOLDER_TAB_ID, DIM_ID, DIM_TAB_ID, DIM_NAME, MEMBER_ID, MEMBER_NAME, USE_DENOM_TABLE, DEFAULT_HIERARCHY_ID, DENOM_TAB_ID, USE_REPORT_MEMBER, USE_REPORT_DIM_MEMBER,TABLE_DISP_NAME,TABLE_TOOLTIP_NAME)"
            + "SELECT (select sub_folder_id from PRG_USER_FOLDER_DETAIL where folder_id=&  and sub_folder_type='&') as sub_folder_id, BUSS_TABLE_ID, IS_DIMENSION, IS_FACT, IS_BUCKET, DISP_NAME,(PRG_USER_SUB_FLDR_ELEMENTS_SEQ.NEXTVAL) as SUB_FOLDER_TAB_ID, DIM_ID, DIM_TAB_ID, DIM_NAME, MEMBER_ID, MEMBER_NAME, "
            + " USE_DENOM_TABLE, DEFAULT_HIERARCHY_ID, DENOM_TAB_ID, USE_REPORT_MEMBER, USE_REPORT_DIM_MEMBER,nvl(TABLE_DISP_NAME,DISP_NAME),nvl(TABLE_TOOLTIP_NAME,DISP_NAME) FROM PRG_USER_SUB_FOLDER_TABLES "
        ...
        , {"getExtraDimsForRole", "select dim_id from prg_grp_dimensions where grp_id=& and dim_name not in('Time') minus select dim_id from prg_user_sub_folder_tables t,prg_grp_buss_table b where sub_folder_id=& and b.buss_table_id=t.buss_table_id"},
        {"getCalculatedFormulas", "SELECT busstab.grp_id , puf.FOLDER_ID ,puf.FOLDER_NAME ,pufd.SUB_FOLDER_ID ,pufd.SUB_FOLDER_NAME ,"
            + " pufd.SUB_FOLDER_TYPE ,pusft.SUB_FOLDER_TAB_ID ,pusft.IS_DIMENSION ,pusft.IS_FACT ,pusft.IS_BUCKET ,pusft.DISP_NAME ,"
            + " pusft.DIM_ID ,pusft.DIM_TAB_ID ,pusft.DIM_NAME ,pusfe.ELEMENT_ID ,pusfe.BUSS_TABLE_ID ,pusfe.BUSS_COL_ID ,"
            + " pusfe.BUSS_COL_NAME ,pusfe.USER_COL_NAME ,pusfe.USER_COL_DESC,pusfe.USER_COL_TYPE ,pusfe.ref_element_id ,"
            + " pusfe.ref_element_type ,pusft.member_id ,pusft.member_name, busstab.buss_table_name,connTab.conn_id, bussdet.default_aggregation,bussdet.ACTUAL_COL_FORMULA,pusfe.use_report_flag ,bussDet.REFFERED_ELEMENTS"
        ...
        , {"getFolderLevelColFormulas", "insert into PRG_USER_ALL_INFO_DETAILS(GRP_ID ,FOLDER_ID ,FOLDER_NAME ,SUB_FOLDER_ID ,SUB_FOLDER_NAME ,SUB_FOLDER_TYPE ,SUB_FOLDER_TAB_ID,"
            ...
            + "pusfe.use_report_flag,pusfe.REFFERED_ELEMENTS,nvl(pusft.TABLE_DISP_NAME,pusft.DISP_NAME),nvl(pusft.TABLE_TOOLTIP_NAME,pusft.DISP_NAME)"
            + " FROM PRG_USER_FOLDER puf ,PRG_USER_FOLDER_DETAIL pufd ,PRG_USER_SUB_FOLDER_TABLES pusft ,PRG_USER_SUB_FOLDER_ELEMENTS pusfe,"
            + " PRG_GRP_BUSS_TABLE bussTab,(Select min(CONNECTION_ID) conn_id, buss_table_id "
            + " from prg_grp_buss_table_src group by buss_table_id) connTab WHERE puf.folder_id = pufd.folder_id "
            + " AND pufd.SUB_FOLDER_ID =pusft.SUB_FOLDER_ID AND pusfe.sub_folder_tab_id=pusft.sub_folder_tab_id "
            + " and busstab.buss_table_id = pusft.buss_table_id "
            + " and connTab.buss_table_id = pusft.buss_table_id AND  puf.FOLDER_ID in(&) and pusfe.buss_col_id=0 AND pufd.sub_folder_type='Facts'  ORDER BY puf.folder_id, "
            + " pufd.SUB_FOLDER_NAME,pusft.SUB_FOLDER_TAB_ID "}, {"getCustomFactElements", "select DISTINCT element_id,nvl(USER_COL_NAME,USER_COL_DESC),REFFERED_ELEMENTS "
        ...
        , {"copyUser", "insert into prg_ar_users (PU_ID,PU_LOGIN_ID,PU_FIRSTNAME,PU_MIDDLENAME,PU_LASTNAME,PU_PASSWORD,PU_TYPE,PU_EMAIL,PU_CONTACTNO,PU_ADDRESS,"
            ...
            + " a.SUB_FOLDER_TYPE,  b.SUB_FOLDER_TYPE,  nvl(a.DISP_NAME,'1'),  nvl(b.DISP_NAME,'1'),  nvl(a.DIM_NAME,'1'),"
            + " nvl( b.DIM_NAME,'1'),  a.BUSS_COL_NAME,  b.BUSS_COL_NAME,  a.USER_COL_NAME,  b.USER_COL_NAME,  a.USER_COL_TYPE,"
            + "  b.USER_COL_TYPE,  nvl(a.MEMBER_NAME,'0'),  nvl(b.MEMBER_NAME,'0'),  nvl(a.BUSS_TABLE_NAME,'0'),  nvl(b.BUSS_TABLE_NAME,'0')"
            ...
            + " ORDER BY a.ref_element_id  ) WHERE old_ele_id IN(&)"}, {"getPriorMeasure", "SELECT USER_COL_DESC,REF_ELEMENT_ID,ELEMENT_ID,REF_ELEMENT_TYPE FROM PRG_USER_ALL_INFO_DETAILS WHERE REF_ELEMENT_ID=& AND REF_ELEMENT_TYPE=2"}, {"getPriorAndChangeMeasures", "select ELEMENT_ID,REF_ELEMENT_ID,REF_ELEMENT_TYPE from prg_user_all_info_details where REF_ELEMENT_ID=& and REF_ELEMENT_TYPE in(2,3,4) "},
        {"getrefElementType", " SELECT REF_ELEMENT_TYPE from PRG_USER_ALL_INFO_DETAILS WHERE ELEMENT_ID=&"}, {"getSubFolderDetails", "SELECT BUSS_TABLE_ID,SUB_FOLDER_ID FROM prg_user_sub_folder_tables WHERE SUB_FOLDER_TAB_ID = &"}, {"deleteElementDetails", "DELETE FROM PRG_USER_SUB_FOLDER_ELEMENTS WHERE BUSS_COL_ID IN (SELECT BUSS_COLUMN_ID FROM PRG_GRP_BUSS_TABLE_DETAILS WHERE BUSS_TABLE_ID = & ) AND SUB_FOLDER_TAB_ID = &  "}, {"subFolderEleDetailsQuery", "INSERT INTO PRG_USER_SUB_FOLDER_ELEMENTS (ELEMENT_ID,sub_folder_id,buss_table_id,buss_col_id,buss_col_name,user_col_name,user_col_desc,user_col_type"
            + ",sub_folder_tab_id,ref_element_id,ref_element_type,default_aggregation,folder_group,use_report_flag,reffered_elements,actual_formula,display_formula,ref_dim_tab_id) (SELECT PRG_USER_SUB_FLDR_ELEMENTS_SEQ.NEXTVAL AS ELEMENT_ID,SUB_FOLDER_ID,BUSS_TABLE_ID,BUSS_COL_ID,BUSS_COL_NAME, "
            ...
            + "bussdet.display_formula,NVL(pusft.TABLE_DISP_NAME,pusft.DISP_NAME),NVL(pusft.TABLE_TOOLTIP_NAME,pusft.DISP_NAME)FROM PRG_USER_FOLDER puf , PRG_USER_FOLDER_DETAIL pufd , PRG_USER_SUB_FOLDER_TABLES pusft , PRG_USER_SUB_FOLDER_ELEMENTS pusfe, PRG_GRP_BUSS_TABLE bussTab,PRG_GRP_BUSS_TABLE_DETAILS bussDet,"
            ...
            + "pufd.SUB_FOLDER_ID,pufd.SUB_FOLDER_NAME,pufd.SUB_FOLDER_TYPE,pusft.SUB_FOLDER_TAB_ID,pusft.IS_DIMENSION,pusft.IS_FACT,pusft.IS_BUCKET,pusft.DISP_NAME,pusft.DIM_ID,pusft.DIM_TAB_ID,pusft.DIM_NAME,pusfe.ELEMENT_ID,pusfe.BUSS_TABLE_ID,pusfe.BUSS_COL_ID,pusfe.BUSS_COL_NAME ,"
            + "pusfe.USER_COL_NAME,pusfe.USER_COL_DESC,pusfe.USER_COL_TYPE,pusfe.ref_element_id,pusfe.ref_element_type,pusft.member_id,pusft.member_name,busstab.buss_table_name,connTab.conn_id,bussdet.default_aggregation,bussdet.ACTUAL_COL_FORMULA,pusfe.use_report_flag,bussdet.REFFERED_ELEMENTS,"
            + "bussdet.display_formula,NVL(pusft.TABLE_DISP_NAME,pusft.DISP_NAME),NVL(pusft.TABLE_TOOLTIP_NAME,pusft.DISP_NAME) FROM PRG_USER_FOLDER puf ,PRG_USER_FOLDER_DETAIL pufd,PRG_USER_SUB_FOLDER_TABLES pusft,PRG_GRP_BUSS_TABLE bussTab,PRG_GRP_BUSS_TABLE_DETAILS bussDet,"
            ...
            + " AND bussdet.buss_column_id = pusfe.buss_col_id AND connTab.buss_table_id  = pusft.buss_table_id AND pusfe.buss_col_id!     =0 AND puf.folder_id  IN(&) AND pusft.BUSS_TABLE_ID = & AND pufd.SUB_FOLDER_ID =& AND PUSFT.SUB_FOLDER_TAB_ID=& AND pusfe.use_report_flag='Y' AND USE_REPORT_DIM_MEMBER='Y' ORDER BY puf.folder_id,  pufd.SUB_FOLDER_NAME,  pusft.SUB_FOLDER_TAB_ID "}, {"updatePrgUserAllinfo", "UPDATE PRG_USER_ALL_INFO_DETAILS SET BUSS_TABLE_NAME =''WHERE  USER_COL_TYPE ='summarized' AND FOLDER_ID =&"}, {"getTableDetails", "select USER_COL_DESC,IS_FACT,IS_DIMENSION,BUSS_TABLE_NAME,BUSS_COL_NAME,USER_COL_NAME,DISP_NAME,AGGREGATION_TYPE,FOLDER_NAME,ACTUAL_COL_FORMULA,DISPLAY_FORMULA from PRG_USER_ALL_INFO_DETAILS where ELEMENT_ID in (&)"}, {"addUserFolderAssignment", "INSERT  INTO PRG_GRP_USER_FOLDER_ASSIGNMENT(USER_ASSI_ID, USER_FOLDER_ID, USER_ID, GRP_ID, START_DATE, END_DATE) values(&,&,&,&,&,'&')"}, {"deleteFolder", "DELETE  FROM PRG_GRP_USER_FOLDER_ASSIGNMENT where user_folder_id in (&) and user_id=&"}, {"getRprt", "select ar.report_id, pm.report_name from account_report ar,prg_ar_users pu,prg_ar_report_master pm where pu.pu_id= & and "
            + "ar.org_id=pu.account_type and pm.report_id= ar.report_id"}, {"insrtreptqry", "insert into prg_ar_user_reports(USER_REP_ID,USER_ID,REPORT_ID,PUR_REPORT_SEQUENCE,PUR_FAV_REPORT,PUR_CUST_REPORT_NAME) values(PRG_AR_USER_REPORTS_SEQ.nextval,'&','&','','N','')"}, {"getRpid", " SELECT REPORT_ID,FOLDER_ID FROM PRG_AR_REPORT_DETAILS where FOLDER_ID in(&)"}, {"deleteUserAssignment", " delete from PRG_GRP_USER_ASSIGNMENT where GRP_ID=&"}, {"deleteUserRoleAssignment", " delete from PRG_GRP_USER_FOLDER_ASSIGNMENT where user_folder_id=&  and grp_id=&"}, {"addUserAssignment", "INSERT INTO PRG_GRP_USER_ASSIGNMENT(USER_GRP_ID, USER_ID, GRP_ID, START_DATE, END_DATE, IS_ACTIVE,USER_NAME,FOLDER_ID) values (&,&,&,&,'&','&','&',&)"}, {"isAssignedGroup", "select * from PRG_GRP_USER_ASSIGNMENT where grp_id=& and user_id=& and folder_id = &"}, {"addExistingDimValues", "insert into  PRG_GRP_DIM_MAP_KEY_VALUE SELECT key.col_id Key1,NVL(member.col_id, key.col_id) value1,key.mem_id key_mem_id,NVL(member.mem_id,key.mem_id ) mem_mem_id FROM prg_grp_dim_member_details KEY,prg_grp_dim_member_details member WHERE key.mem_id       = member.mem_id(+) AND key.col_type       ='KEY' AND member.col_type(+) ='VALUE'"} //added by ramesh janakuttu for partial publish dimentation on 26-jun-12
        , {"insertinUserAllInfoDetails", "INSERT INTO PRG_USER_ALL_INFO_DETAILS(GRP_ID ,FOLDER_ID ,FOLDER_NAME ,SUB_FOLDER_ID ,SUB_FOLDER_NAME ,SUB_FOLDER_TYPE ,SUB_FOLDER_TAB_ID,IS_DIMENSION , IS_FACT ,IS_BUCKET , DISP_NAME , DIM_ID ,DIM_TAB_ID ,DIM_NAME ,ELEMENT_ID ,BUSS_TABLE_ID ,BUSS_COL_ID ,BUSS_COL_NAME ,USER_COL_NAME ,USER_COL_DESC,USER_COL_TYPE , REF_ELEMENT_ID ,REF_ELEMENT_TYPE ,MEMBER_ID ,MEMBER_NAME ,BUSS_TABLE_NAME ,CONNECTION_ID ,AGGREGATION_TYPE ,ACTUAL_COL_FORMULA,USE_REPORT_FLAG,REFFERED_ELEMENTS,Display_Formula,TABLE_DISP_NAME,TABLE_TOOLTIP_NAME)SELECT busstab.grp_id ,puf.FOLDER_ID ,puf.FOLDER_NAME ,pufd.SUB_FOLDER_ID ,pufd.SUB_FOLDER_NAME ,pufd.SUB_FOLDER_TYPE ,pusft.SUB_FOLDER_TAB_ID ,pusft.IS_DIMENSION ,pusft.IS_FACT ,pusft.IS_BUCKET ,pusft.DISP_NAME ,pusft.DIM_ID ,pusft.DIM_TAB_ID ,pusft.DIM_NAME ,pusfe.ELEMENT_ID ,pusfe.BUSS_TABLE_ID ,pusfe.BUSS_COL_ID ,pusfe.BUSS_COL_NAME ,pusfe.USER_COL_NAME ,pusfe.USER_COL_DESC,pusfe.USER_COL_TYPE ,pusfe.ref_element_id ,pusfe.ref_element_type ,pusft.member_id ,pusft.member_name,busstab.buss_table_name,connTab.conn_id,bussdet.default_aggregation,bussdet.ACTUAL_COL_FORMULA,pusfe.use_report_flag,bussdet.REFFERED_ELEMENTS,bussdet.display_formula,NVL(pusft.TABLE_DISP_NAME,pusft.DISP_NAME),NVL(pusft.TABLE_TOOLTIP_NAME,pusft.DISP_NAME)FROM PRG_USER_FOLDER puf ,PRG_USER_FOLDER_DETAIL pufd ,PRG_USER_SUB_FOLDER_TABLES pusft ,PRG_USER_SUB_FOLDER_ELEMENTS pusfe,PRG_GRP_BUSS_TABLE bussTab,PRG_GRP_BUSS_TABLE_DETAILS bussDet,(SELECT MIN(CONNECTION_ID) conn_id,buss_table_id FROM prg_grp_buss_table_src GROUP BY buss_table_id) connTab WHERE puf.folder_id= pufd.folder_id AND pufd.SUB_FOLDER_ID=pusft.SUB_FOLDER_ID AND pusfe.sub_folder_tab_id=pusft.sub_folder_tab_id AND busstab.buss_table_id  = pusft.buss_table_id AND bussdet.buss_column_id = pusfe.buss_col_id AND connTab.buss_table_id  = pusft.buss_table_id AND pusfe.buss_col_id!=0 AND puf.folder_id  IN(&) and pusft.BUSS_TABLE_ID=& AND pusfe.use_report_flag='Y' AND USE_REPORT_DIM_MEMBER='Y' ORDER BY puf.folder_id,pufd.SUB_FOLDER_NAME,pusft.SUB_FOLDER_TAB_ID"}, {"insertintoallAdimDetails", "INSERT INTO PRG_USER_ALL_ADIM_DETAILS(INFO_FOLDER_ID,INFO_ELEMENT_ID,INFO_BUSS_COL_ID,REL_LEVEL,INFO_DIM_ID,INFO_DIM_NAME,INFO_MEMBER_ID,INFO_MEMBER_NAME)"
            + "SELECT info.folder_id info_folder_id,info.element_id info_element_id,info.buss_col_id info_buss_col_id,NVL(reld.rel_level,1) rel_level,info.dim_id info_dim_id,"
            + "info.dim_name info_dim_name,info.member_id info_member_id,info.member_name info_member_name FROM PRG_USER_ALL_INFO_DETAILS info ,"
            + "prg_grp_dim_rel relm ,prg_grp_dim_rel_details reld WHERE info.member_id = reld.mem_id(+) AND info.dim_id = relm.dim_id(+)"
            + "AND info.member_id  IS NOT NULL AND info.folder_id  IN(&) and info.BUSS_TABLE_ID=&"}, {"insertintoallAdimKeyValueElements", "INSERT INTO PRG_USER_ALL_ADIM_KEY_VAL_ELE(LEVEL1,KEY_ELEMENT_ID,KEY_COL_TYPE,VAL_ELEMENT_ID,KEY_BUSS_COL_ID,VAL_BUSS_COL_ID,KEY_BUSS_COL_NAME,VAL_BUSS_COL_NAME,"
            + "KEY_BUSS_TABLE_ID,VAL_BUSS_TABLE_ID,VAL_COL_TYPE,KEY_MEMBER_NAME,VAL_MEMBER_NAME,KEY_SUB_FOLDER_NAME,VAL_SUB_FOLDER_NAME,KEY_SUB_FOLDER_ID,VAL_SUB_FOLDER_ID,KEY_DISP_NAME,"
            + "KEY_DIM_ID,KEY_DIM_NAME, VAL_DISP_NAME,VAL_DIM_ID,VAL_DIM_NAME,KEY_FOLDER_ID,KEY_FOLDER_NAME,REL_NAME,IS_DEFAULT)"
            + "SELECT NVL(reld.rel_level,1) level1,key1.element_id Key_element_id,key1.USER_COL_TYPE Key_col_type,val1.ref_element_id val_element_id ,key1.buss_col_id key_buss_col_id ,"
            + "val1.buss_col_id val_buss_col_id ,key1.buss_col_name Key_buss_col_name,val1.buss_col_name val_buss_col_name ,key1.buss_table_id Key_buss_table_id,val1.buss_table_id val_buss_table_id ,"
            + "val1.USER_COL_TYPE val_col_type,key1.member_name key_member_name ,val1.member_name val_member_name ,key1.SUB_FOLDER_NAME key_SUB_FOLDER_NAME,val1.SUB_FOLDER_NAME val_SUB_FOLDER_NAME ,"
            + "key1.SUB_FOLDER_ID key_SUB_FOLDER_ID,val1.SUB_FOLDER_ID val_SUB_FOLDER_ID ,key1.disp_name key_disp_name ,key1.dim_id key_dim_id,key1.dim_name key_dim_name,val1.disp_name val_disp_name,"
            + "val1.dim_id val_dim_id,val1.dim_name val_dim_name ,key1.folder_id key_folder_id ,key1.folder_name key_folder_name ,relm.rel_name ,relm.is_default FROM PRG_GRP_DIM_MAP_KEY_VALUE map ,"
            + "PRG_USER_ALL_INFO_DETAILS key1 ,PRG_USER_ALL_INFO_DETAILS val1 ,prg_grp_dim_rel relm ,prg_grp_dim_rel_details reld WHERE map.key1= key1.buss_col_id"
            + " AND key1.member_id = map.key_mem_id AND val1.member_id   = map.mem_mem_id AND val1.buss_col_id = map.value1 AND key1.member_id   = reld.mem_id(+) AND key1.dim_id= relm.dim_id(+)"
            + "AND key1.folder_id  IN(&) AND val1.folder_id IN(&) and key1.BUSS_TABLE_ID=& and val1.BUSS_TABLE_ID=& ORDER BY 1"}, {"insertintoallAdimMasterdetails", "INSERT INTO PRG_USER_ALL_ADIM_MASTER(INFO_FOLDER_ID,REL_LEVEL,INFO_DIM_ID,INFO_DIM_NAME,INFO_MEMBER_ID,INFO_MEMBER_NAME)"
            + "SELECT DISTINCT info.folder_id info_folder_id,NVL(reld.rel_level,1) rel_level,info.dim_id info_dim_id,info.dim_name info_dim_name,info.member_id info_member_id,info.member_name info_member_name FROM PRG_USER_ALL_INFO_DETAILS info ,"
            + "prg_grp_dim_rel relm ,prg_grp_dim_rel_details reld WHERE info.member_id = reld.mem_id(+)AND info.dim_id= relm.dim_id(+)"
            + "AND info.member_id  IS NOT NULL AND info.folder_id  IN(&) and info.BUSS_TABLE_ID=&"}, {"insertallDdimDetails", "INSERT INTO PRG_USER_ALL_DDIM_DETAILS(INFO_FOLDER_ID,INFO_ELEMENT_ID,INFO_BUSS_COL_ID,REL_LEVEL,INFO_DIM_ID,IMFO_DIM_NAME,INFO_MEMBER_ID,INFO_MEMBER_NAME)"
            + "SELECT info.folder_id info_folder_id,info.element_id info_element_id,info.buss_col_id info_buss_col_id,NVL(reld.rel_level,1) rel_level,info.dim_id info_dim_id,info.dim_name imfo_dim_name,"
            + "info.member_id info_member_id,info.member_name info_member_name FROM PRG_USER_ALL_INFO_DETAILS info ,prg_grp_dim_rel relm ,prg_grp_dim_rel_details reld WHERE info.member_id   = reld.mem_id(+)"
            + "AND info.dim_id = relm.dim_id(+) AND relm.is_default(+) ='Y' AND info.member_id    IS NOT NULL AND info.folder_id    IN(&) and info.BUSS_TABLE_ID=&"}, {"insertallDdimkeyValueElement", "INSERT INTO PRG_USER_ALL_DDIM_KEY_VAL_ELE(LEVEL1,KEY_ELEMENT_ID,KEY_COL_TYPE,VAL_ELEMENT_ID,KEY_BUSS_COL_ID,VAL_BUSS_COL_ID,KEY_BUSS_COL_NAME,VAL_BUSS_COL_NAME,"
            + "KEY_BUSS_TABLE_ID,VAL_BUSS_TABLE_ID,VAL_COL_TYPE,KEY_MEMBER_NAME,VAL_MEMBER_NAME,KEY_SUB_FOLDER_ID,VAL_SUB_FOLDER_ID,KEY_SUB_FOLDER_NAME,VAL_SUB_FOLDER_NAME,KEY_DISP_NAME,KEY_DIM_ID,"
            + "KEY_DIM_NAME,VAL_DISP_NAME,VAL_DIM_ID,VAL_DIM_NAME,KEY_FOLDER_ID,KEY_FOLDER_NAME,REL_NAME,IS_DEFAULT)"
            + "SELECT NVL(reld.rel_level,1) level1,key1.element_id Key_element_id,key1.USER_COL_TYPE Key_col_type,val1.ref_element_id val_element_id ,key1.buss_col_id key_buss_col_id ,val1.buss_col_id val_buss_col_id ,"
            + "key1.buss_col_name Key_buss_col_name,val1.buss_col_name val_buss_col_name ,key1.buss_table_id Key_buss_table_id,val1.buss_table_id val_buss_table_id ,val1.USER_COL_TYPE val_col_type,"
            + "key1.member_name key_member_name ,val1.member_name val_member_name ,key1.SUB_FOLDER_id key_SUB_FOLDER_id,val1.SUB_FOLDER_id val_SUB_FOLDER_id ,key1.SUB_FOLDER_NAME key_SUB_FOLDER_NAME,"
            + "val1.SUB_FOLDER_NAME val_SUB_FOLDER_NAME ,key1.disp_name key_disp_name ,key1.dim_id key_dim_id,key1.dim_name key_dim_name,val1.disp_name val_disp_name,val1.dim_id val_dim_id,"
            + "val1.dim_name val_dim_name ,key1.folder_id key_folder_id ,key1.folder_name key_folder_name,relm.rel_name,relm.is_default FROM PRG_GRP_DIM_MAP_KEY_VALUE map ,"
            + "PRG_USER_ALL_INFO_DETAILS key1 ,PRG_USER_ALL_INFO_DETAILS val1 ,prg_grp_dim_rel relm ,prg_grp_dim_rel_details reld WHERE map.key1= key1.buss_col_id AND key1.member_id= map.key_mem_id"
            + " AND val1.member_id= map.mem_mem_id AND val1.buss_col_id= map.value1 AND key1.member_id = reld.mem_id(+) AND key1.dim_id= relm.dim_id(+) AND relm.is_default(+) ='Y'"
            + "AND key1.folder_id IN(&) AND val1.folder_id IN(&) and key1.BUSS_TABLE_ID=& and val1.BUSS_TABLE_ID=& ORDER BY 1"}, {"insertallDdimMasterDetails", "INSERT INTO PRG_USER_ALL_DDIM_MASTER(INFO_FOLDER_ID,REL_LEVEL,INFO_DIM_ID,INFO_DIM_NAME,INFO_MEMBER_ID,INFO_MEMBER_NAME)"
            + "SELECT DISTINCT info.folder_id info_folder_id,NVL(reld.rel_level,1) rel_level,info.dim_id info_dim_id,info.dim_name info_dim_name,info.member_id info_member_id,"
            + "info.member_name info_member_name FROM PRG_USER_ALL_INFO_DETAILS info ,prg_grp_dim_rel relm ,prg_grp_dim_rel_details reld WHERE info.member_id= reld.mem_id(+)"
            + "AND info.dim_id= relm.dim_id(+) AND relm.is_default(+) ='Y' AND info.member_id IS NOT NULL AND info.folder_id IN(&) and info.BUSS_TABLE_ID=&"} //added by anil
        , {"getAllFactsNew", "select DISTINCT sub_folder_tab_id,NVL(table_disp_name,disp_name),nvl(table_tooltip_name,disp_name) from PRG_USER_ALL_INFO_DETAILS where folder_id in (&) and sub_folder_type in('Facts') and disp_name is not null and disp_name not in('Calculated Facts','Formula')"} //added by anil
        , {"getFactElements", "select DISTINCT element_id,nvl(USER_COL_DESC, USER_COL_NAME),ref_element_id as column_name,BUSS_COL_NAME, REF_ELEMENT_TYPE,REFFERED_ELEMENTS,disp_name,display_formula   "
            + "from PRG_USER_ALL_INFO_DETAILS where sub_folder_tab_id = '&' and sub_folder_type in('Facts','Formula') and use_report_flag='Y'  order by BUSS_COL_NAME,REF_ELEMENT_TYPE"} //added by Nazneen for security
        , {"insertinuserDenomSecInfo", "insert into PRG_USER_DENOM_SEC_INFO(ID,GRP_ID,DIM_ID,FOLDER_ID,SUB_FOLDER_ID,SUB_FOLDER_TAB_ID,DIM_TAB_ID,USER_ID,MEMBER_ID,ELEMENT_ID,CLAUSE_ID,BASE_ELEMENT_ID,BASE_BUSS_TAB_ID,BASE_BUSS_COL_NAME,ELEMENT_BUSS_TAB_ID,ELEMENT_BUSS_COL_NAME) values(PRG_USER_DENOM_SEC_INFO_SEQ.nextval,&,&,&,&,&,&,&,&,&,&,&,&,'&',&,'&')"} //added by Anil
        , {"saveFavReport1", "update prg_ar_user_reports set pur_fav_report = 'Y' WHERE report_id in (&) AND user_id=&"}, {"saveFavReport2", "update prg_ar_user_reports set pur_fav_report = 'N' WHERE report_id not in (&) AND user_id=&"} //added by Nazneen
        , {"saveFavReportForAllUsers1", "update prg_ar_user_reports set pur_fav_report = 'N' WHERE report_id in (&) AND user_id in (&)"}, {"saveFavReportForAllUsers2", "update prg_ar_user_reports set pur_fav_report = 'Y' WHERE report_id in (&) AND user_id in (&)"}, {"getFavReport", "SELECT DISTINCT b.report_id,a.report_name,a.report_type,b.pur_report_sequence FROM prg_ar_report_master a,prg_ar_user_reports b"
            + " WHERE b.user_id=& AND a.report_id=b.report_id AND b.pur_fav_report ='Y' order by b.pur_report_sequence"} //added by Nazneen
        , {"getFavReportForAll", "SELECT DISTINCT b.report_id,a.report_name,a.report_type,b.pur_report_sequence "
            + " FROM prg_ar_report_master a,prg_ar_user_reports b WHERE a.report_id    = b.report_id AND b.pur_fav_report = 'Y' AND user_id IN (&) "
            + " AND b.report_id IN   ( SELECT DISTINCT report_id FROM prg_ar_user_reports WHERE user_id IN (&) AND pur_fav_report = 'Y' GROUP BY report_id "
            + " HAVING COUNT(DISTINCT user_id) = &) ORDER BY b.pur_report_sequence"} //added by anitha
        , {"getCreateKPIData", "select CREATE_KPI_ID,CREATE_KPI_NAME,CREATE_KPI_DATA from PRG_AR_CREATE_KPI_FROM_REPORT"}, {"insertTargetMeasures", "insert into PRG_AR_TARGET_MEASURES_DETAILS(TARGET_ID,ELEMENT,FOLDER_ID,USER_ID,ELEMENT_ID,TABLE_ID,TARGET_VALUE,TARGET_DURATION,START_DATE,END_DATE,CUSTOM_NAME,START_VALUE,END_VALUE,ELEMENT_NAME) values(PRG_AR_TARGET_MEASURES_SEQ.nextval,'&','&','&','&','&',&,'&','&','&','&','&','&','&')"}, {"insertTargetDayLevelData", "insert into PRG_AR_TARGET_DAYLEVEL_DATA(DAYLEVEL_ID,ELEMENT_ID,MONTH_NAME,MONTH_WEEKS,MONTH_DATES,MONTH_DAYS,TARGET_LEVEL_VALUE,ALLOCATION_TYPE,DAY_ALLOCATION_TYPE) values(PRG_AR_TARGET_DAYLEVEL_SEQ.nextval,'&','&','&','&','&','&','&','&')"}, {"insertTargetWeekLevelData", "insert into PRG_AR_TARGET_WEEK_ALLOCATION(DAYLEVEL_ID,ELEMENT_ID,MONTH_NAME,MONTH_WEEKS,TARGET_WEEK_VALUE,TARGET_PERCENT_VALUE) values(PRG_AR_TARGET_WEEK_ALLOC_SEQ.nextval,'&','&','&','&','&')"}, {"getMemberDetails", " SELECT COALESCE(reld.rel_level,1) level1,key1.element_id KEY_ELEMENT_ID,key1.USER_COL_TYPE Key_col_type,val1.element_id val_element_id,key1.buss_col_id KEY_BUSS_COL_ID,val1.buss_col_id val_buss_col_id,key1.buss_col_name KEY_BUSS_COL_NAME,"
            ...
            + "prg_grp_dim_rel_details reld ON (key1.member_id    = reld.mem_id) WHERE key1.folder_id IN( &)AND val1.folder_id IN( &)  AND key1.member_id IN(&) ORDER BY 1 "} //addded by Nazneen for Publish a Role
        , {"insertPublishInUserAllInfoDetails", "INSERT INTO PRG_USER_ALL_INFO_DETAILS (GRP_ID ,FOLDER_ID ,FOLDER_NAME , SUB_FOLDER_ID , SUB_FOLDER_NAME , SUB_FOLDER_TYPE , SUB_FOLDER_TAB_ID,  IS_DIMENSION ,  IS_FACT ,  IS_BUCKET , DISP_NAME , DIM_ID , DIM_TAB_ID ,"
            ...
            + "bussdet.default_aggregation,bussdet.ACTUAL_COL_FORMULA,pusfe.use_report_flag,bussdet.REFFERED_ELEMENTS,bussdet.display_formula, NVL(pusft.TABLE_DISP_NAME,pusft.DISP_NAME),NVL(pusft.TABLE_TOOLTIP_NAME,pusft.DISP_NAME)"
            ...
            + " SELECT info.folder_id info_folder_id, info.element_id info_element_id,info.buss_col_id info_buss_col_id, NVL(reld.rel_level,1) rel_level,info.dim_id info_dim_id, info.dim_name info_dim_name,info.member_id info_member_id,info.member_name info_member_name "
            ...
            + " SELECT info.folder_id info_folder_id,  info.element_id info_element_id,  info.buss_col_id info_buss_col_id,  NVL(reld.rel_level,1) rel_level,  info.dim_id info_dim_id,  info.dim_name imfo_dim_name,  info.member_id info_member_id,  info.member_name info_member_name "
            ...
            + " SELECT DISTINCT info.folder_id info_folder_id,  NVL(reld.rel_level,1) rel_level,  info.dim_id info_dim_id,  info.dim_name info_dim_name,  info.member_id info_member_id,  info.member_name info_member_name "
        ...
        , {"deleteDimPublishUserAllInfoDetails", "delete from PRG_USER_ALL_INFO_DETAILS where BUSS_TABLE_ID = & "}, {"deleteDimPublishAllADIMDetails", "delete from PRG_USER_ALL_ADIM_DETAILS where INFO_DIM_ID = & "}, {"deleteDimPublishAllADIMKeyValEle", "delete from PRG_USER_ALL_ADIM_KEY_VAL_ELE where KEY_BUSS_TABLE_ID = & "}, {"deleteDimPublishAllADIMMaster", "delete from  PRG_USER_ALL_ADIM_MASTER where INFO_DIM_ID = & "}, {"deleteDimPublishAllDDIMDetails", "delete from PRG_USER_ALL_DDIM_DETAILS where INFO_DIM_ID = & "}, {"deleteDimPublishAllDDIMKeyValEle", "delete from PRG_USER_ALL_DDIM_KEY_VAL_ELE where KEY_BUSS_TABLE_ID = & "}, {"deleteDimPublishAllDDIMMaster", "delete from PRG_USER_ALL_DDIM_MASTER where INFO_DIM_ID = & "}, {"insertDimPublishUserAllInfoDetails", "INSERT INTO PRG_USER_ALL_INFO_DETAILS (GRP_ID ,FOLDER_ID ,FOLDER_NAME , SUB_FOLDER_ID , SUB_FOLDER_NAME , SUB_FOLDER_TYPE , SUB_FOLDER_TAB_ID,  IS_DIMENSION ,  IS_FACT ,  IS_BUCKET , DISP_NAME , DIM_ID , DIM_TAB_ID ,"
            ...
            + " FROM PRG_USER_FOLDER puf , PRG_USER_FOLDER_DETAIL pufd , PRG_USER_SUB_FOLDER_TABLES pusft ,PRG_USER_SUB_FOLDER_ELEMENTS pusfe,PRG_GRP_BUSS_TABLE bussTab, PRG_GRP_BUSS_TABLE_DETAILS bussDet, (SELECT MIN(CONNECTION_ID) conn_id,   buss_table_id"
            + " FROM prg_grp_buss_table_src GROUP BY buss_table_id  ) connTab WHERE puf.folder_id        = pufd.folder_id AND pufd.SUB_FOLDER_ID     =pusft.SUB_FOLDER_ID AND pusfe.sub_folder_tab_id=pusft.sub_folder_tab_id AND busstab.buss_table_id  = pusft.buss_table_id AND bussdet.buss_column_id = pusfe.buss_col_id "
            + "AND connTab.buss_table_id  = pusft.buss_table_id AND pusfe.buss_col_id!     =0 AND puf.folder_id  IN(&) AND  pusft.BUSS_TABLE_ID = & AND pusfe.use_report_flag='Y' AND USE_REPORT_DIM_MEMBER='Y' ORDER BY puf.folder_id,  pufd.SUB_FOLDER_NAME,  pusft.SUB_FOLDER_TAB_ID "}, {"insertDimPublishAllADIMDetails", "INSERT INTO PRG_USER_ALL_ADIM_DETAILS  (INFO_FOLDER_ID, INFO_ELEMENT_ID,INFO_BUSS_COL_ID,REL_LEVEL, INFO_DIM_ID, INFO_DIM_NAME,  INFO_MEMBER_ID,  INFO_MEMBER_NAME) "
            + " SELECT info.folder_id info_folder_id, info.element_id info_element_id,info.buss_col_id info_buss_col_id, NVL(reld.rel_level,1) rel_level,info.dim_id info_dim_id, info.dim_name info_dim_name,info.member_id info_member_id,info.member_name info_member_name "
            + " FROM PRG_USER_ALL_INFO_DETAILS info , prg_grp_dim_rel relm , prg_grp_dim_rel_details reld WHERE info.member_id = reld.mem_id(+) AND info.dim_id      = relm.dim_id(+) AND info.member_id  IS NOT NULL AND info.folder_id  IN(&) AND  info.BUSS_TABLE_ID = & "}, {"insertDimPublishAllADIMKeyValEle", "INSERT INTO PRG_USER_ALL_ADIM_KEY_VAL_ELE ( LEVEL1, KEY_ELEMENT_ID, KEY_COL_TYPE, VAL_ELEMENT_ID, KEY_BUSS_COL_ID,  VAL_BUSS_COL_ID,  KEY_BUSS_COL_NAME,  VAL_BUSS_COL_NAME,  KEY_BUSS_TABLE_ID,  VAL_BUSS_TABLE_ID,  VAL_COL_TYPE,  KEY_MEMBER_NAME,  VAL_MEMBER_NAME,"
            + "  KEY_SUB_FOLDER_NAME,  VAL_SUB_FOLDER_NAME, KEY_SUB_FOLDER_ID,VAL_SUB_FOLDER_ID, KEY_DISP_NAME, KEY_DIM_ID, KEY_DIM_NAME, VAL_DISP_NAME, VAL_DIM_ID, VAL_DIM_NAME, KEY_FOLDER_ID, KEY_FOLDER_NAME,  REL_NAME,  IS_DEFAULT) "
            + " SELECT NVL(reld.rel_level,1) level1,key1.element_id Key_element_id, key1.USER_COL_TYPE Key_col_type,key1.ref_element_id val_element_id ,key1.buss_col_id key_buss_col_id ,key1.buss_col_id val_buss_col_id , key1.buss_col_name Key_buss_col_name, key1.buss_col_name val_buss_col_name ,key1.buss_table_id Key_buss_table_id,key1.buss_table_id val_buss_table_id ,key1.USER_COL_TYPE val_col_type,key1.member_name key_member_name ,"
            + "key1.member_name val_member_name ,key1.SUB_FOLDER_NAME key_SUB_FOLDER_NAME, key1.SUB_FOLDER_NAME val_SUB_FOLDER_NAME , key1.SUB_FOLDER_ID key_SUB_FOLDER_ID, key1.SUB_FOLDER_ID val_SUB_FOLDER_ID ,key1.disp_name key_disp_name , key1.dim_id key_dim_id,key1.dim_name key_dim_name,key1.disp_name val_disp_name, key1.dim_id val_dim_id,key1.dim_name val_dim_name ,"
            + "key1.folder_id key_folder_id , key1.folder_name key_folder_name , relm.rel_name , relm.is_default FROM PRG_GRP_DIM_MAP_KEY_VALUE map ,PRG_USER_ALL_INFO_DETAILS key1 ,prg_grp_dim_rel relm ,prg_grp_dim_rel_details reld "
            + " WHERE map.key1       = key1.buss_col_id AND key1.member_id   = map.key_mem_id AND key1.member_id   = reld.mem_id(+) AND key1.dim_id      = relm.dim_id(+) AND key1.folder_id  IN(&) AND key1.BUSS_TABLE_ID = & "}, {"insertDimPublishAllADIMMaster", "INSERT INTO PRG_USER_ALL_ADIM_MASTER  (INFO_FOLDER_ID,  REL_LEVEL, INFO_DIM_ID, INFO_DIM_NAME, INFO_MEMBER_ID,INFO_MEMBER_NAME) SELECT DISTINCT info.folder_id info_folder_id,NVL(reld.rel_level,1) rel_level,info.dim_id info_dim_id,info.dim_name info_dim_name,info.member_id info_member_id,info.member_name info_member_name "
            + "FROM PRG_USER_ALL_INFO_DETAILS info , prg_grp_dim_rel relm , prg_grp_dim_rel_details reld WHERE info.member_id = reld.mem_id(+) AND info.dim_id      = relm.dim_id(+) AND info.member_id  IS NOT NULL AND info.folder_id  IN(&) AND info.BUSS_TABLE_ID = & "}, {"insertDimPublishAllDDIMDetails", "INSERT INTO PRG_USER_ALL_DDIM_DETAILS  (    INFO_FOLDER_ID,    INFO_ELEMENT_ID,    INFO_BUSS_COL_ID,    REL_LEVEL,    INFO_DIM_ID,    IMFO_DIM_NAME,    INFO_MEMBER_ID,    INFO_MEMBER_NAME  ) "
            + " SELECT info.folder_id info_folder_id,  info.element_id info_element_id,  info.buss_col_id info_buss_col_id,  NVL(reld.rel_level,1) rel_level,  info.dim_id info_dim_id,  info.dim_name imfo_dim_name,  info.member_id info_member_id,  info.member_name info_member_name "
            + " FROM PRG_USER_ALL_INFO_DETAILS info ,  prg_grp_dim_rel relm ,  prg_grp_dim_rel_details reld WHERE info.member_id   = reld.mem_id(+) AND info.dim_id        = relm.dim_id(+) AND relm.is_default(+) ='Y' "
            + " AND info.member_id    IS NOT NULL AND info.folder_id    IN(&) AND info.BUSS_TABLE_ID = & "}, {"insertDimPublishAllDDIMKeyValEle", "INSERT INTO PRG_USER_ALL_DDIM_KEY_VAL_ELE  (    LEVEL1,    KEY_ELEMENT_ID,    KEY_COL_TYPE,    VAL_ELEMENT_ID,    KEY_BUSS_COL_ID,    VAL_BUSS_COL_ID,    KEY_BUSS_COL_NAME,    VAL_BUSS_COL_NAME,    KEY_BUSS_TABLE_ID,    VAL_BUSS_TABLE_ID,    VAL_COL_TYPE,"
            + "    KEY_MEMBER_NAME,    VAL_MEMBER_NAME,    KEY_SUB_FOLDER_ID,    VAL_SUB_FOLDER_ID,    KEY_SUB_FOLDER_NAME,    VAL_SUB_FOLDER_NAME,    KEY_DISP_NAME,    KEY_DIM_ID,    KEY_DIM_NAME,    VAL_DISP_NAME,    VAL_DIM_ID,    VAL_DIM_NAME,    KEY_FOLDER_ID,    KEY_FOLDER_NAME,    REL_NAME,    IS_DEFAULT  ) "
            + " SELECT NVL(reld.rel_level,1) level1,  key1.element_id Key_element_id,  key1.USER_COL_TYPE Key_col_type,  key1.ref_element_id val_element_id ,  key1.buss_col_id key_buss_col_id ,  key1.buss_col_id val_buss_col_id ,  key1.buss_col_name Key_buss_col_name,  key1.buss_col_name val_buss_col_name ,  key1.buss_table_id Key_buss_table_id,  key1.buss_table_id val_buss_table_id ,  key1.USER_COL_TYPE val_col_type,  key1.member_name key_member_name ,  key1.member_name val_member_name ,  key1.SUB_FOLDER_id key_SUB_FOLDER_id,"
            + "  key1.SUB_FOLDER_id val_SUB_FOLDER_id ,  key1.SUB_FOLDER_NAME key_SUB_FOLDER_NAME,  key1.SUB_FOLDER_NAME val_SUB_FOLDER_NAME ,  key1.disp_name key_disp_name ,  key1.dim_id key_dim_id,  key1.dim_name key_dim_name,  key1.disp_name val_disp_name,  key1.dim_id val_dim_id,  key1.dim_name val_dim_name ,  key1.folder_id key_folder_id ,  key1.folder_name key_folder_name,  relm.rel_name,  relm.is_default "
            + " FROM PRG_GRP_DIM_MAP_KEY_VALUE map ,  PRG_USER_ALL_INFO_DETAILS key1 ,  prg_grp_dim_rel relm ,  prg_grp_dim_rel_details reld "
            + " WHERE map.key1         = key1.buss_col_id AND key1.member_id     = map.key_mem_id AND key1.member_id     = reld.mem_id(+) AND key1.dim_id        = relm.dim_id(+) AND relm.is_default(+) ='Y' AND key1.folder_id    IN(&) AND key1.BUSS_TABLE_ID = & "}, {"insertDimPublishAllDDIMMaster", "INSERT INTO PRG_USER_ALL_DDIM_MASTER  (    INFO_FOLDER_ID,    REL_LEVEL,    INFO_DIM_ID,    INFO_DIM_NAME,    INFO_MEMBER_ID,    INFO_MEMBER_NAME  ) "
            + " SELECT DISTINCT info.folder_id info_folder_id,  NVL(reld.rel_level,1) rel_level,  info.dim_id info_dim_id,  info.dim_name info_dim_name,  info.member_id info_member_id,  info.member_name info_member_name "
            + " FROM PRG_USER_ALL_INFO_DETAILS info ,  prg_grp_dim_rel relm ,  prg_grp_dim_rel_details reld WHERE info.member_id   = reld.mem_id(+) AND info.dim_id        = relm.dim_id(+) AND relm.is_default(+) ='Y' AND info.member_id    IS NOT NULL "
            + " AND info.folder_id    IN(&) AND info.BUSS_TABLE_ID = & "} //for Facts
        , {"insertAllFactPublishUserAllInfoDetails", "INSERT INTO PRG_USER_ALL_INFO_DETAILS (GRP_ID ,FOLDER_ID ,FOLDER_NAME , SUB_FOLDER_ID , SUB_FOLDER_NAME , SUB_FOLDER_TYPE , SUB_FOLDER_TAB_ID,  IS_DIMENSION ,  IS_FACT ,  IS_BUCKET , DISP_NAME , DIM_ID , DIM_TAB_ID ,"
            + "  DIM_NAME , ELEMENT_ID , BUSS_TABLE_ID ,BUSS_COL_ID ,BUSS_COL_NAME , USER_COL_NAME ,USER_COL_DESC , USER_COL_TYPE , REF_ELEMENT_ID , REF_ELEMENT_TYPE , MEMBER_ID ,MEMBER_NAME , BUSS_TABLE_NAME ,  CONNECTION_ID ,"
            + " AGGREGATION_TYPE , ACTUAL_COL_FORMULA,  USE_REPORT_FLAG, REFFERED_ELEMENTS, Display_Formula, TABLE_DISP_NAME,  TABLE_TOOLTIP_NAME ) SELECT busstab.grp_id ,puf.FOLDER_ID ,puf.FOLDER_NAME ,pufd.SUB_FOLDER_ID ,pufd.SUB_FOLDER_NAME ,pufd.SUB_FOLDER_TYPE ,pusft.SUB_FOLDER_TAB_ID ,pusft.IS_DIMENSION ,pusft.IS_FACT ,pusft.IS_BUCKET ,"
            + " pusft.DISP_NAME ,pusft.DIM_ID ,pusft.DIM_TAB_ID ,pusft.DIM_NAME ,pusfe.ELEMENT_ID ,pusfe.BUSS_TABLE_ID ,pusfe.BUSS_COL_ID ,pusfe.BUSS_COL_NAME ,pusfe.USER_COL_NAME ,pusfe.USER_COL_DESC,pusfe.USER_COL_TYPE ,pusfe.ref_element_id ,pusfe.ref_element_type ,pusft.member_id ,pusft.member_name,busstab.buss_table_name,connTab.conn_id,"
            + " bussdet.default_aggregation,bussdet.ACTUAL_COL_FORMULA,pusfe.use_report_flag,bussdet.REFFERED_ELEMENTS,bussdet.display_formula, NVL(pusft.TABLE_DISP_NAME,pusft.DISP_NAME),NVL(pusft.TABLE_TOOLTIP_NAME,pusft.DISP_NAME)"
            + " FROM PRG_USER_FOLDER puf , PRG_USER_FOLDER_DETAIL pufd , PRG_USER_SUB_FOLDER_TABLES pusft ,PRG_USER_SUB_FOLDER_ELEMENTS pusfe,PRG_GRP_BUSS_TABLE bussTab, PRG_GRP_BUSS_TABLE_DETAILS bussDet, (SELECT MIN(CONNECTION_ID) conn_id,   buss_table_id"
            + " FROM prg_grp_buss_table_src GROUP BY buss_table_id  ) connTab WHERE puf.folder_id        = pufd.folder_id AND pufd.SUB_FOLDER_ID     =pusft.SUB_FOLDER_ID AND pusfe.sub_folder_tab_id=pusft.sub_folder_tab_id AND busstab.buss_table_id  = pusft.buss_table_id AND bussdet.buss_column_id = pusfe.buss_col_id "
            + " AND connTab.buss_table_id  = pusft.buss_table_id AND pusfe.buss_col_id!     =0 AND puf.folder_id  IN(&) AND  pusft.BUSS_TABLE_ID = & AND pusfe.use_report_flag='Y' AND USE_REPORT_DIM_MEMBER='Y' ORDER BY puf.folder_id,  pufd.SUB_FOLDER_NAME,  pusft.SUB_FOLDER_TAB_ID "}, {"insertRemFactPublishUserAllInfoDetails", "INSERT INTO PRG_USER_ALL_INFO_DETAILS (GRP_ID ,FOLDER_ID ,FOLDER_NAME , SUB_FOLDER_ID , SUB_FOLDER_NAME , SUB_FOLDER_TYPE , SUB_FOLDER_TAB_ID,  IS_DIMENSION ,  IS_FACT ,  IS_BUCKET , DISP_NAME , DIM_ID , DIM_TAB_ID ,"
            + "  DIM_NAME , ELEMENT_ID , BUSS_TABLE_ID ,BUSS_COL_ID ,BUSS_COL_NAME , USER_COL_NAME ,USER_COL_DESC , USER_COL_TYPE , REF_ELEMENT_ID , REF_ELEMENT_TYPE , MEMBER_ID ,MEMBER_NAME , BUSS_TABLE_NAME ,  CONNECTION_ID ,"
            + " AGGREGATION_TYPE , ACTUAL_COL_FORMULA,  USE_REPORT_FLAG, REFFERED_ELEMENTS, Display_Formula, TABLE_DISP_NAME,  TABLE_TOOLTIP_NAME ) SELECT busstab.grp_id ,puf.FOLDER_ID ,puf.FOLDER_NAME ,pufd.SUB_FOLDER_ID ,pufd.SUB_FOLDER_NAME ,pufd.SUB_FOLDER_TYPE ,pusft.SUB_FOLDER_TAB_ID ,pusft.IS_DIMENSION ,pusft.IS_FACT ,pusft.IS_BUCKET ,"
            + " pusft.DISP_NAME ,pusft.DIM_ID ,pusft.DIM_TAB_ID ,pusft.DIM_NAME ,pusfe.ELEMENT_ID ,pusfe.BUSS_TABLE_ID ,pusfe.BUSS_COL_ID ,pusfe.BUSS_COL_NAME ,pusfe.USER_COL_NAME ,pusfe.USER_COL_DESC,pusfe.USER_COL_TYPE ,pusfe.ref_element_id ,pusfe.ref_element_type ,pusft.member_id ,pusft.member_name,busstab.buss_table_name,connTab.conn_id,"
            + " bussdet.default_aggregation,bussdet.ACTUAL_COL_FORMULA,pusfe.use_report_flag,bussdet.REFFERED_ELEMENTS,bussdet.display_formula, NVL(pusft.TABLE_DISP_NAME,pusft.DISP_NAME),NVL(pusft.TABLE_TOOLTIP_NAME,pusft.DISP_NAME)"
            + " FROM PRG_USER_FOLDER puf , PRG_USER_FOLDER_DETAIL pufd , PRG_USER_SUB_FOLDER_TABLES pusft ,PRG_USER_SUB_FOLDER_ELEMENTS pusfe,PRG_GRP_BUSS_TABLE bussTab, PRG_GRP_BUSS_TABLE_DETAILS bussDet, (SELECT MIN(CONNECTION_ID) conn_id,   buss_table_id"
            + " FROM prg_grp_buss_table_src GROUP BY buss_table_id  ) connTab WHERE puf.folder_id        = pufd.folder_id AND pufd.SUB_FOLDER_ID     =pusft.SUB_FOLDER_ID AND pusfe.sub_folder_tab_id=pusft.sub_folder_tab_id AND busstab.buss_table_id  = pusft.buss_table_id AND bussdet.buss_column_id = pusfe.buss_col_id "
            + " AND connTab.buss_table_id  = pusft.buss_table_id AND pusfe.buss_col_id!     =0 AND puf.folder_id  IN(&) AND  pusft.BUSS_TABLE_ID = & AND pusfe.use_report_flag='Y' AND USE_REPORT_DIM_MEMBER='Y' "
            + " AND pusfe.ELEMENT_ID IN (&) ORDER BY puf.folder_id,  pufd.SUB_FOLDER_NAME,  pusft.SUB_FOLDER_TAB_ID "} //end of code by nazneen
