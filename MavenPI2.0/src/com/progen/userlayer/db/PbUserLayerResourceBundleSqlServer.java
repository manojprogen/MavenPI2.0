 ...
 */
public class PbUserLayerResourceBundleSqlServer extends ListResourceBundle implements Serializable {
    ...
    static final Object contents[][] = {
        {"addUserFolder", "INSERT INTO PRG_USER_FOLDER(FOLDER_NAME, FOLDER_DESC,FOLDER_CREATED_ON,FOLDER_UPDATED_ON,GRP_ID) VALUES ('&','&',getdate(),getdate(),&)"}, {"addUserFolderDetails", "insert into PRG_USER_FOLDER_DETAIL (FOLDER_ID, SUB_FOLDER_NAME, SUB_FOLDER_TYPE) "
            + " values(&,'&','&' )"} /*
         ...
         */, {"getBussFactsInfoByGrpId", "select buss_table_id, buss_table_name from prg_grp_buss_table where buss_table_id in ( SELECT BUSS_TABLE_ID FROM PRG_GRP_BUSS_TABLE except "
            + " SELECT DISTINCT TAB_ID FROM PRG_GRP_DIM_TABLES ) and buss_type!='Query' and isnull(is_pure_query,'N')!='Y' and grp_id='&'"} /*
        ...
        , {"addUserSubFolderElements", "INSERT INTO PRG_USER_SUB_FOLDER_ELEMENTS (SUB_FOLDER_ID, BUSS_TABLE_ID, BUSS_COL_ID, BUSS_COL_NAME, USER_COL_NAME,"
            + " USER_COL_DESC, USER_COL_TYPE, SUB_FOLDER_TAB_ID,REF_ELEMENT_ID,REF_ELEMENT_TYPE,DEFAULT_AGGREGATION,USE_REPORT_FLAG,Display_Formula) "
            + "SELECT & as SUB_FOLDER_ID,a.BUSS_TABLE_ID, a.buss_column_id,a.column_name,a.column_disp_name,a.column_display_desc, "
            + " a.column_type ,& as SUB_FOLDER_TAB_ID,&,1 AS REF_ELEMENT_TYPE,a.default_aggregation,'Y',a.display_formula FROM PRG_GRP_BUSS_TABLE_DETAILS a "
        ...
        , {"insertExtraElementsForFacts", "insert into PRG_USER_SUB_FOLDER_ELEMENTS(  SUB_FOLDER_ID, BUSS_TABLE_ID, BUSS_COL_ID, BUSS_COL_NAME,USER_COL_NAME,"
            ...
            + "(a.user_col_type ='NUMBER' OR upper(a.user_col_type)='NUMERIC'"
            + " OR upper(a.user_col_type)='FLOAT' OR upper(a.user_col_type)='DOUBLE' "
            + " OR upper(a.user_col_type)='CALCULATED') "
        ...
        , {"getUserFolderDimMbrs", " SELECT sub_folder_tab_id, member_id , isnull(disp_name, member_name) as mem_disp_name ,dim_tab_id,use_report_member,use_report_dim_member FROM PRG_USER_SUB_FOLDER_TABLES WHERE sub_folder_id ='&' AND is_dimension  ='Y' AND dim_id ='&' ORDER BY mem_disp_name"} //modified by susheela over
        //added by Nazneen
        ...
        , {"getUserFolderDimMbrsCols", "SELECT ELEMENT_ID, USER_COL_NAME,USER_COL_DESC FROM PRG_USER_SUB_FOLDER_ELEMENTS where sub_folder_tab_id='&'"}, {"getUserFolderFacts", "SELECT DISTINCT sub_folder_tab_id, isnull(table_disp_name,disp_name) disp_name,USE_REPORT_MEMBER,isnull(table_tooltip_name,disp_name) tooltip_name FROM PRG_USER_SUB_FOLDER_TABLES where is_fact='Y' and sub_folder_id='&' and disp_name not in('Calculated Facts','Formula') order by disp_name "}, {"getUserFolderFactsColumns", "SELECT ELEMENT_ID,USER_COL_NAME  FROM PRG_USER_SUB_FOLDER_ELEMENTS where SUB_FOLDER_TAB_ID='&' order by ref_element_id, ref_element_type"}, {"getUserFolderBuckets", "SELECT DISTINCT sub_folder_tab_id, isnull(table_disp_name,disp_name) disp_name,isnull(table_tooltip_name,disp_name) tooltip_name FROM PRG_USER_SUB_FOLDER_TABLES where is_bucket='Y' and sub_folder_id='&' order by disp_name "}, {"getUserFolderBucketsColumns", "SELECT ELEMENT_ID,USER_COL_NAME,USER_COL_DESC  FROM PRG_USER_SUB_FOLDER_ELEMENTS where SUB_FOLDER_TAB_ID='&' order by ref_element_id, ref_element_type"} //,{"getUserFoldDimNames","SELECT DISTINCT DIM_ID,DIM_NAME FROM PRG_USER_SUB_FOLDER_TABLES where sub_folder_id='&' and is_dimension='Y' order by DIM_NAME"}
        ...
        , {"saveFavReport1", "update prg_ar_user_reports set pur_fav_report = 'Y' WHERE report_id in (&) AND user_id=&"}, {"saveFavReport2", "update prg_ar_user_reports set pur_fav_report = 'N' WHERE report_id not in (&) AND user_id=&"}, {"getFavReport", "SELECT DISTINCT b.REPORT_ID,a.REPORT_NAME,a.REPORT_TYPE,b.PUR_REPORT_SEQUENCE FROM PRG_AR_REPORT_MASTER a,PRG_AR_USER_REPORTS b"
            + " WHERE b.USER_ID=& AND a.REPORT_ID=b.REPORT_ID AND b.PUR_FAV_REPORT='Y' order by b.PUR_REPORT_SEQUENCE"} //added by anitha
        , {"getCreateKPIData", "select CREATE_KPI_ID,CREATE_KPI_NAME,CREATE_KPI_DATA from PRG_AR_CREATE_KPI_FROM_REPORT"}, {"insertTargetMeasures", "insert into PRG_AR_TARGET_MEASURES_DETAILS(ELEMENT,FOLDER_ID,USER_ID,ELEMENT_ID,TABLE_ID,TARGET_VALUE,TARGET_DURATION,START_DATE,END_DATE,CUSTOM_NAME,START_VALUE,END_VALUE,ELEMENT_NAME) values('&','&','&','&','&',&,'&','&','&','&','&','&','&')"}, {"insertTargetDayLevelData", "insert into PRG_AR_TARGET_DAYLEVEL_DATA(ELEMENT_ID,MONTH_NAME,MONTH_WEEKS,MONTH_DATES,MONTH_DAYS,TARGET_LEVEL_VALUE,ALLOCATION_TYPE,DAY_ALLOCATION_TYPE) values('&','&','&','&','&','&','&','&')"} //addded by Nazneen for Publish a Role
        , {"insertPublishInUserAllInfoDetails", "INSERT INTO PRG_USER_ALL_INFO_DETAILS (GRP_ID ,FOLDER_ID ,FOLDER_NAME , SUB_FOLDER_ID , SUB_FOLDER_NAME , SUB_FOLDER_TYPE , SUB_FOLDER_TAB_ID,  IS_DIMENSION ,  IS_FACT ,  IS_BUCKET , DISP_NAME , DIM_ID , DIM_TAB_ID ,"
            ...
            + " SELECT info.folder_id info_folder_id,  info.element_id info_element_id,  info.buss_col_id info_buss_col_id,  isnull(reld.rel_level,1) rel_level,  info.dim_id info_dim_id,  info.dim_name imfo_dim_name,  info.member_id info_member_id,  info.member_name info_member_name "
