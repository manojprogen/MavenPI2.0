 ...
 */
public class ProgenSpiderChart extends Plot implements Cloneable,
        Serializable {
     ...
     */
    public static final Paint DEFAULT_LABEL_BACKGROUND_PAINT = new Color(
            255, 255, 192);
    /**
    ...
    public static double prg_MaxVal = 0;
    /**
    ...
    private double[] prg_SeriesMaxArray;
    /**
     ...
     * <code>null</code> permitted).
     */
    public ProgenSpiderChart(CategoryDataset dataset) {
     ...
     */
    public ProgenSpiderChart(CategoryDataset dataset, TableOrder extract) {
        super();
        if (extract == null) {
    ...
    //added by kalyan on  26-05-2010
    public double[] getPrg_SeriesMaxArray() {
        return prg_SeriesMaxArray;
    }

    public void setPrg_SeriesMaxArray(double[] prg_SeriesMaxArray) {
        this.prg_SeriesMaxArray = prg_SeriesMaxArray;
    }
     ...
     *
     * @return The data extract order (never
     * <code>null</code>).
     *
    ...
    /**
     * Returns the start angle for the first radar axis. <BR> This is measured
     * in degrees starting from 3 o'clock (Java Arc2D default) and measuring
     * anti-clockwise.
     *
     ...
     * registered listeners. <P> The initial default value is 90 degrees, which
    ...
    /**
     * Sets the maximum value any category axis can take and sends a {@link PlotChangeEvent}
     ...
     *
     * @return The paint used to draw the axis lines (never
     * <code>null</code>).
     *
     ...
     *
     * @return The stroke used to draw the axis lines (never
     * <code>null</code>).
     *
     ...
     *
     * @return The label generator (never
     * <code>null</code>).
     *
     ...
     *
     * @return The tool tip generator (possibly
     * <code>null</code>).
     *
     ...
     *
     * @return The URL generator (possibly
     * <code>null</code>).
     *
     ...
     */
    protected Point2D getWebPoint(Rectangle2D bounds, double angle,
            double length) {
    ...
    public void draw(Graphics2D g2, Rectangle2D area, Point2D anchor,
        ...
        if (!DatasetUtilities.isEmptyOrNull(this.dataset)) {
            ...
            for (int cat = 0; cat < catCount; cat++) {
                ...
                g2.draw(line);
                drawLabel(g2, radarArea, 0.0, cat, angle,
                        360.0 / catCount);
            }
            ...
            for (int series = 0; series < seriesCount; series++) {
                drawRadarPoly(g2, radarArea, centre, info, series,
                        catCount, headH, headW);
            }
        } else {
    ...
    private void calculateMaxValue(int seriesCount, int catCount) {
        double v = 0;
        Number nV = null;
        ////////.println("seriesCount=" + seriesCount);
        ////////.println("catCount=" + catCount);
        double[] Prg_MaxArray = new double[catCount];
        double prg_CheckMax = 0;

        for (int catIndex = 0; catIndex < catCount; catIndex++) {
            for (int seriesIndex = 0; seriesIndex < seriesCount; seriesIndex++) {

                if (prg_CheckMax > Double.parseDouble(String.valueOf(getPlotValue(seriesIndex, catIndex)))) {

                    Prg_MaxArray[catIndex] = prg_CheckMax;
                } else {
                    prg_CheckMax = Double.parseDouble(String.valueOf(getPlotValue(seriesIndex, catIndex)));
                    Prg_MaxArray[catIndex] = Double.parseDouble(String.valueOf(getPlotValue(seriesIndex, catIndex)));
                }
                // ////.print("values are " + getPlotValue(seriesIndex, catIndex));
            }
            prg_CheckMax = 0;
            ////////.println("");
        }
...
//                if (prg_CheckMax > Double.parseDouble(String.valueOf(getPlotValue(seriesIndex, catIndex)))) {
//
//                    Prg_MaxArray[seriesIndex] = prg_CheckMax;
//                } else {
//                    prg_CheckMax = Double.parseDouble(String.valueOf(getPlotValue(seriesIndex, catIndex)));
//                    Prg_MaxArray[seriesIndex] = Double.parseDouble(String.valueOf(getPlotValue(seriesIndex, catIndex)));
//                }
//                ////.print("values are " + getPlotValue(seriesIndex, catIndex));
//            }
//            prg_CheckMax = 0;
...
//        }
        ...
        setPrg_SeriesMaxArray(Prg_MaxArray);


        for (int i = 0; i < catCount; i++) {
            ////////.println("max values are" + Prg_MaxArray[i]);
        }
...
//             }
    ...
    protected void drawRadarPoly(Graphics2D g2, Rectangle2D plotArea,
            Point2D centre, PlotRenderingInfo info, int series,
            int catCount, double headH, double headW) {

        double[] seriesmaxvalues = getPrg_SeriesMaxArray();
        //prg_MaxVal=seriesmaxvalues[series];

        ////////.println("entered into drawradarpoly");
        ...
        for (int cat = 0; cat < catCount; cat++) {
            ...
            if (dataValue != null) {
                ...
                if (value < 0.1) {
                    value = 0.000001D;
                    // //////.println("inner value is "+value);
                }
                //added by k
                this.maxValue = seriesmaxvalues[cat];
                if (this.maxValue < 0.1) {
                    this.maxValue = 1.000001D;
                }
                ...
                if (value >= 0) { // draw the polygon series...
                    ...
                    // ////////.println("this .maxValue="+this .maxValue);
                    ...
                    Ellipse2D head = new Ellipse2D.Double(point.getX() - headW / 2, point.getY() - headH / 2,
                            headW, headH);
                    g2.setPaint(paint);
                    g2.fill(head);
                    ...
                    if (entities != null) {
                        ...
                        if (this.toolTipGenerator != null) {
                            tip = this.toolTipGenerator.generateToolTip(this.dataset,
                                    series, cat);
                        }
     ...
     *
     * @return The value to be plotted (possibly
     * <code>null</code>).
     *
     * @see #getDataExtractOrder()
     */
    ...
    protected void drawLabel(Graphics2D g2, Rectangle2D plotArea,
        ...
        if (this.dataExtractOrder == TableOrder.BY_ROW) {
            ...
            label = this.labelGenerator.generateColumnLabel(
                    this.dataset, cat);
        } else {
            // if series are in columns, then the categories are the row keys
            label = this.labelGenerator.generateRowLabel(this.dataset,
                    cat);
        }

        Rectangle2D labelBounds = getLabelFont().getStringBounds(label,
                frc);
        LineMetrics lm = getLabelFont().getLineMetrics(label, frc);
        double ascent = lm.getAscent();

        Point2D labelLocation = calculateLabelLocation(labelBounds,
                ascent, plotArea, startAngle);
    ...
    protected Point2D calculateLabelLocation(Rectangle2D labelBounds,
        ...
        Arc2D arc1 = new Arc2D.Double(plotArea, startAngle, 0,
                Arc2D.OPEN);
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof ProgenSpiderChart)) {
            return false;
        }
        if (!super.equals(obj)) {
            return false;
        }
        ProgenSpiderChart that = (ProgenSpiderChart) obj;
        ...
        if (!PaintUtilities.equal(this.axisLinePaint,
                that.axisLinePaint)) {
            return false;
        }
        ...
        if (!ShapeUtilities.equal(this.legendItemShape,
        ...
        if (!PaintUtilities.equal(this.baseSeriesPaint,
                that.baseSeriesPaint)) {
            return false;
        }
    ...
    public Object clone() throws CloneNotSupportedException {
        ProgenSpiderChart clone = (ProgenSpiderChart) super.clone();
    ...
    private void writeObject(ObjectOutputStream stream)
        ...
        SerialUtilities.writeStroke(this.baseSeriesOutlineStroke,
                stream);
