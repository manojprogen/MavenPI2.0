 ...
 * @author Saurabh
 */
public class PbTargetMeasureResourceBundleSqlServer extends ListResourceBundle implements Serializable {

    protected Object[][] getContents() {
        return contents;
    }
    static final Object contents[][] = {
        {"getBusinessGroupFactsList", "select BUSS_TABLE_ID,BUSS_TABLE_NAME,DB_TABLE_ID from  prg_grp_buss_table where BUSS_TABLE_ID in (select BUSS_TABLE_ID  from prg_grp_buss_table where grp_id=&  and buss_type!='Query' except SELECT DISTINCT TAB_ID "
        ...
        , {"getTargetDimMembersGroup", "select pgdm.MEMBER_ID, pgdm.MEMBER_NAME, pgdm.DIM_TAB_ID,pgdt.TAB_ID,pgbt.BUSS_TABLE_NAME, pgdt.DIM_ID "
            ...
            + " pgbt.buss_table_id=pgdt.tab_id and pgbt.buss_type not in ('Query') order by pgdt.dim_id , pgdm.member_name"}, {"getTargetDimMembers", "select member_id,member_name from prg_grp_dim_member where dim_id in(select dim_id from prg_grp_dim_tables where tab_id in(&))"}, {"addTargetTableInDbMaster", "insert into prg_db_master_table(CONNECTION_ID,USER_SCHEMA,TABLE_NAME,TABLE_ALIAS,TABLE_TYPE,CREATED_BY,UPDATED_BY,CREATED_ON,UPDATED_ON,DB_QUERY) values('&','&','&','&','&','&','&',&,&,'&')"}, {"addTargetTableInDbDetails", " insert into prg_db_master_table_details(table_id, table_col_name, table_col_cust_name, col_type, col_length, is_primary_key, is_active) values(&,'&','&','&',&,'&','&')"}, {"getTargetMemberNames", "select MEMBER_ID,MEMBER_NAME from prg_grp_dim_member where member_id in (&)"} // ,{"getDimMemberDet","select distinct buss_column_id,column_name, column_type, column_disp_name from prg_grp_buss_table_details where buss_column_id in(select col_id from prg_grp_dim_tab_details where dim_tab_id in(select dim_tab_id from prg_grp_dim_member where member_id in (&)))"}
         ...
         */, {"getDimMemberDet", "select * from prg_grp_dim_member_details pgdm,prg_grp_buss_table_details pgbt,prg_grp_buss_table pgb where mem_id"
            ...
            + " pgbt.buss_column_id=pgdm.col_id and pgb.grp_id=& and pgbt.buss_table_id=pgb.buss_table_id"}, {"getKeyMembers", "select COL_ID,COL_TYPE  from prg_grp_dim_member_details where mem_id in(&) and col_type='KEY'"}, {"addTargetTabInBussTable", "insert into PRG_GRP_BUSS_TABLE(buss_table_name,buss_desc,buss_type,no_of_nodes,is_pure_query,db_level_hint,db_table_id,grp_id,db_query) values('&','&','&','&','&','&',&,&,'&')"}, {"addTargetTabInBussSrcTable", "insert into PRG_GRP_BUSS_TABLE_SRC(BUSS_TABLE_ID,CONNECTION_ID,DB_TABLE_ID,SOURCE_TYPE,DB_HINT,DB_TABLE_NAME) values(&,&,&,'&','&','&')"}, {"addTargetTabInBussSrcDetailsTable", "insert into prg_grp_buss_table_src_details(BUSS_SRC_ID,DB_TABLE_ID,BUSS_TABLE_ID,DB_COLUMN_ID,COLUMN_ALIAS,COLUMN_TYPE) values(&,&,&,&,'&','&')"}, {"addBussTableDetails", "insert into PRG_GRP_BUSS_TABLE_DETAILS(BUSS_TABLE_ID,COLUMN_NAME,DB_TABLE_ID,DB_COLUMN_ID,BUSS_SRC_TABLE_DTL_ID,IS_UNION, COLUMN_TYPE,ACTUAL_COL_FORMULA,ACTUAL_COL_FORMULA1,DEFAULT_AGGREGATION,BUCKET_ATTACHED,COLUMN_DISP_NAME) values(&,'&',&,&,&,'&','&','&','&','&','&','&')"} // ,{"getMeasureTableCols","select buss_column_id, column_name, buss_column_id, pgdd.buss_table_id, pgbt.buss_table_name from PRG_GRP_BUSS_TABLE_DETAILS pgdd,prg_grp_buss_table pgbt where pgdd.buss_table_id in(&) and pgbt.buss_table_id= pgdd.buss_table_id order by pgdd.buss_table_id"}
        ...
        , {"addFormulaBussMater", "INSERT INTO PRG_GRP_BUSS_TABLE(BUSS_TABLE_ID, BUSS_TABLE_NAME, BUSS_DESC, BUSS_TYPE, NO_OF_NODES, IS_PURE_QUERY,DB_QUERY,"
            ...
            + "'&',&,&,'&',&,'&',&,'&','&','&','&','&','&')"}, {"insertTrgtMes", "INSERT INTO PRG_TARGET_MEASURE VALUES(&)"}, {"insertTrgtMester", "INSERT INTO PRG_TAR_MASTER VALUES(&)"}
    };
}
